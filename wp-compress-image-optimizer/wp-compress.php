...
if (!defined('WPC_PLUGIN_VERSION')) {
    define('WPC_PLUGIN_VERSION', '7.21.27');
}
...
if (!function_exists('wpc_tier_override736')) {
    function wpc_tier_override736($wpc_cap736 = true)
    {
        ...
        $wpc_auth736 = ($wpc_cap736 && function_exists('wp_get_current_user') && function_exists('current_user_can') && current_user_can('manage_options'));
        if (!$wpc_auth736 && isset($_GET['wpc_key']) && class_exists('wps_ic_url_key') && method_exists('wps_ic_url_key', 'tierKey')) {
        ...
        $wpc_tc739 = (class_exists('wps_ic_url_key') && method_exists('wps_ic_url_key', 'tierCacheOn')) ? wps_ic_url_key::tierCacheOn() : false;
        ...
        $wpc_map736 = ['free' => ['live-cdn' => '0', 'modern_image_delivery' => '0', 'used-css' => '0', 'picture_avif' => '0', 'generate_webp' => '0', 'generate_adaptive' => '0'], 'local' => ['live-cdn' => '0', 'modern_image_delivery' => '1', 'used-css' => '1', 'picture_avif' => '1', 'generate_webp' => '1'], 'edge' => ['live-cdn' => '1', 'modern_image_delivery' => '1', 'used-css' => '1', 'picture_avif' => '1', 'generate_webp' => '1'],];
        ...
        if ($wpc_t736 !== 'edge') {
            ...
            add_filter('option_wps_ic_allow_live', function () {
                return '0';
            }, PHP_INT_MAX);
            add_filter('default_option_wps_ic_allow_live', function () {
                return '0';
            }, PHP_INT_MAX);
        }
        return $wpc_t736;
...
if ($wpc_can_shim('getmypid')) {
    function getmypid()
    {
        return 0;
    }
}
if ($wpc_can_shim('set_time_limit')) {
    function set_time_limit($seconds)
    {
        return false;
    }
}
if ($wpc_can_shim('ignore_user_abort')) {
    function ignore_user_abort($enable = null)
    {
        return 0;
    }
}
if ($wpc_can_shim('opcache_reset')) {
    function opcache_reset()
    {
        return false;
    }
}
if ($wpc_can_shim('opcache_invalidate')) {
    function opcache_invalidate($filename, $force = false)
    {
        return false;
    }
}
if ($wpc_can_shim('opcache_get_status')) {
...
if (!empty($_SERVER['REQUEST_URI'])) {
    ...
    if (strpos($wpc_req_uri, '/wp-json/wpc/v2/bg_swap') !== false || strpos($wpc_req_uri, '/wp-json/wpc/v2/healthcheck') !== false || strpos($wpc_req_uri, 'rest_route=/wpc/v2/bg_swap') !== false || strpos($wpc_req_uri, 'rest_route=/wpc/v2/healthcheck') !== false || strpos($wpc_req_uri, 'rest_route=%2Fwpc%2Fv2%2Fbg_swap') !== false || strpos($wpc_req_uri, 'rest_route=%2Fwpc%2Fv2%2Fhealthcheck') !== false) {
...
if (!empty($_POST['action'])) {
    ...
    if ($wpc_ajax_action === 'wps_ic_variant_count' || $wpc_ajax_action === 'wps_ic_media_library_heartbeat' || $wpc_ajax_action === 'wps_ic_bulkCompressHeartbeat' || $wpc_ajax_action === 'wps_ic_image_stats' || $wpc_ajax_action === 'wpc_ic_start_bulk_compress' || $wpc_ajax_action === 'wps_ic_compress_live') {
