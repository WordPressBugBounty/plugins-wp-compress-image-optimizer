...
if (!function_exists('wpc_crit_meta_write')) {
    ...
    function wpc_crit_meta_write($path, $value)
    {
        try {
            ...
            if (@is_file($path) && (int) @filesize($path) === strlen($value)
                && @file_get_contents($path) === $value) {
                return true;
            }
            return wpc_fs_put($path, $value) !== false;
        } catch (\Throwable $e) {
            return false;
        }
    }
    ...
    function wpc_cache_first_log($event, $key = '', $url = '', $layers = [])
    {
        ...
        try {
            ...
            if ($wpc_lf !== '') {
                ...
                if (is_dir(dirname($wpc_lf))) {
                    if (@filesize($wpc_lf) > 4194304) {
                        $wpc_tail = (string) @file_get_contents($wpc_lf, false, null, max(0, filesize($wpc_lf) - 1048576));
                        $wpc_nl   = strpos($wpc_tail, "\n");
                        wpc_fs_put($wpc_lf, $wpc_nl === false ? '' : substr($wpc_tail, $wpc_nl + 1), LOCK_EX);
                    }
                    $wpc_ok = (bool) wpc_fs_put($wpc_lf, wp_json_encode($entry) . "\n", FILE_APPEND | LOCK_EX);
                    if (!$wpc_ok) {
                        $wpc_ok = (bool) wpc_fs_put($wpc_lf, wp_json_encode($entry) . "\n", FILE_APPEND | LOCK_EX);
                    }
                }
    ...
    function wpc_warm_url_fire($url, $only_variants = null)
    {
        try {
            ...
            foreach (wpc_warm_url_variants() as $wpc_variant => $wpc_v) {
                ...
                if (++$wpc_batch117 > (int) apply_filters('wpc_warm_batch_cap',
                    (function_exists('wpc_constrained_host65') && wpc_constrained_host65()) ? 1 : 2)) {
                    break;
                }
                ...
                @usleep(mt_rand(150, 400) * ((function_exists('wpc_constrained_host65') && wpc_constrained_host65()) ? 4 : 1) * 1000);
            }
        } catch (\Throwable $e) {
            ...
            if ($wpc_lurl136 !== '' && !empty($wpc_opt136['api_key'])) {
                ...
                if ($wpc_lc136 === 200) {
                    ...
                    if (function_exists('wpc_rum_sample_land46')) {
                        wpc_rum_sample_land46($wpc_ld136);
                    }
                    if (is_array($wpc_ld136) && !empty($wpc_ld136['ready'])
                        ...
                        if (!@is_readable($dir . 'used_tpl.txt')) {
                            foreach (['used_css_mobile', 'used_css_desktop', 'used_css_sheets', 'used_css'] as $wpc_uk136) {
                            ...
                            if (!@is_readable($dir . 'used_tpl.txt')) {
                                foreach ([$wpc_a136, $wpc_ld136] as $wpc_tsrc217) {
                                    foreach (['tpl_key', 'tplKey', 'used_tpl'] as $wpc_tk217) {
                                        if (!empty($wpc_tsrc217[$wpc_tk217]) && is_string($wpc_tsrc217[$wpc_tk217])
                                            && function_exists('wpc_used_css_key_valid') && wpc_used_css_key_valid(trim($wpc_tsrc217[$wpc_tk217]))) {
                                            wpc_crit_meta_write($dir . 'used_tpl.txt', trim($wpc_tsrc217[$wpc_tk217]));
                                            break 2;
                                        }
                                    }
                                }
                            ...
                            if (!@is_readable($dir . 'used_tpl.txt') && !empty($wpc_a136['used_css'])) {
                                $wpc_ltpl218 = trim((string) @file_get_contents($dir . 'tpl.txt'));
                                if ($wpc_ltpl218 !== '' && function_exists('wpc_used_css_key_valid')
                                    && wpc_used_css_key_valid($wpc_ltpl218)) {
                                    wpc_crit_meta_write($dir . 'used_tpl.txt', $wpc_ltpl218);
                                    if (function_exists('wpc_cache_first_log')) {
                                        wpc_cache_first_log('v2-latest-tpl-from-local', (string) $urlKey, '', ['tpl' => $wpc_ltpl218]);
                                    }
                                }
                            }
                        }
                        ...
                        $wpc_tplv217 = trim((string) @file_get_contents($dir . 'used_tpl.txt'));
                        $wpc_ucv217  = trim((string) @file_get_contents($dir . 'used_css_url.txt'));
                        if ($wpc_tplv217 !== '' && $wpc_ucv217 !== '' && function_exists('wpc_used_css_fetch')
                            && function_exists('wpc_used_css_path') && !@is_readable(wpc_used_css_path($wpc_tplv217))
                            && !get_transient('wpc_v2lucf217_' . md5((string) $urlKey))) {
                            set_transient('wpc_v2lucf217_' . md5((string) $urlKey), 1, 300);
                            if (wpc_used_css_fetch($wpc_ucv217, $wpc_tplv217)) {
                                if (function_exists('wpc_used_css_scoped_purge')) { wpc_used_css_scoped_purge($wpc_tplv217); }
                                if (function_exists('wpc_cache_first_log')) {
                                    wpc_cache_first_log('v2-latest-usedcss-landed', (string) $urlKey, '', ['tpl' => $wpc_tplv217]);
                                        if (function_exists('wpc_used_css_sheets_land')) { wpc_used_css_sheets_land($dir, $wpc_tplv217); }
                                }
                            } elseif (function_exists('wpc_cache_first_log')) {
                                wpc_cache_first_log('v2-latest-usedcss-fetch-failed', (string) $urlKey, '', ['url' => substr($wpc_ucv217, -60)]);
                            }
                        }
                        if (function_exists('wpc_cache_first_log')) {
        ...
        if (@is_readable($dir . 'crit_combined_url.txt')) {
            ...
            if ($wpc_cbu154 !== '' && preg_match('#^https://#i', $wpc_cbu154)
                ...
                if (strlen($wpc_cbb154) > 1024 && stripos($wpc_cbb154, '@media') !== false
                    ...
                    wpc_fs_put($dir . 'critical_combined.css', $wpc_cbb154, LOCK_EX);
...
if (!function_exists('wpc_used_css_zoneify')) {
    ...
    function wpc_used_css_zoneify($css)
    {
        try {
            ...
            $wpc_zsrv237 = false;
            $wpc_zset237 = defined('WPS_IC_SETTINGS') ? get_option(WPS_IC_SETTINGS) : [];
            foreach (['jpg', 'png', 'gif', 'svg'] as $wpc_zk237) {
                if (is_array($wpc_zset237) && !empty($wpc_zset237['serve'][$wpc_zk237]) && $wpc_zset237['serve'][$wpc_zk237] == '1') {
                    $wpc_zsrv237 = true;
                    break;
                }
            }
...
if (!function_exists('wpc_used_css_path')) {
        ...
        try {
            ...
            foreach (array_slice($sheets, 0, (int) apply_filters('wpc_used_css_sheets_cap', 200)) as $sh) {
                if (is_array($sh) && !empty($sh['url']) && is_string($sh['url'])) {
                    $clean[] = ['url' => (string) $sh['url'], 'in' => (int) ($sh['in'] ?? 0), 'out' => (int) ($sh['out'] ?? 0), 'skip' => (isset($sh['skip']) && $sh['skip'] !== '' && $sh['skip'] !== false) ? (string) $sh['skip'] : '', 'disposition' => isset($sh['disposition']) ? strtolower(trim((string) $sh['disposition'])) : ''];
                }
            }
    ...
    function wpc_css_host_twin_heal231($css)
    {
        try {
            if (!is_string($css) || $css === '' || stripos($css, 'url(') === false
                || !function_exists('home_url') || !defined('ABSPATH')
                || !apply_filters('wpc_css_host_twin_heal', true)) {
                return $css;
            }
            $set231 = function_exists('get_option') && defined('WPS_IC_SETTINGS') ? get_option(WPS_IC_SETTINGS) : [];
            foreach (['jpg', 'png', 'gif', 'svg'] as $k231) {
                if (is_array($set231) && !empty($set231['serve'][$k231]) && $set231['serve'][$k231] == '1') {
                    return $css;
                }
            }
            $home231 = strtolower((string) parse_url(home_url('/'), PHP_URL_HOST));
            if ($home231 === '') {
                return $css;
            }
            $out231 = preg_replace_callback('#(url\(\s*[\'"]?)https?://([^/\'")\s]+)(/wp-content/[^\'")\s]+?\.(?:png|jpe?g|gif|webp|avif|svg))#i', function ($m) use ($home231) {
                if (strtolower($m[2]) === $home231) {
                    return $m[0];
                }
                $p231 = (string) preg_replace('/[?#].*$/', '', $m[3]);
                if (strpos($p231, '..') !== false || !@is_readable(rtrim(ABSPATH, '/') . $p231)) {
                    return $m[0];
                }
                return $m[1] . 'https://' . $home231 . $m[3];
            }, $css);
            return is_string($out231) ? $out231 : $css;
        } catch (\Throwable $e) {
            return $css;
        }
    }

    function wpc_used_css_sheets_land($cd, $tplKey, $sheetsUrl = '')
    {
        try {
            if (!function_exists('wpc_used_css_load_sheets') || wpc_used_css_load_sheets($tplKey)) {
                return true;
            }
            $u = trim((string) $sheetsUrl);
            if ($u === '' && $cd !== '') {
                $u = trim((string) @file_get_contents(rtrim($cd, '/') . '/used_css_sheets_url.txt'));
            }
            if (($u === '' || strpos($u, 'http') !== 0) && $cd !== '') {
                foreach (['used_css_desktop_url.txt', 'used_css_mobile_url.txt', 'used_css_url.txt'] as $f) {
                    $uu = trim((string) @file_get_contents(rtrim($cd, '/') . '/' . $f));
                    if ($uu !== '' && strpos($uu, 'http') === 0
                        && preg_match('/^(.*tpl-[a-f0-9]{8,24})(?:\.(?:desktop|mobile))?\.css/i', $uu, $m)) {
                        $u = $m[1] . '.sheets.json';
                        break;
                    }
                }
            }
            if ($u === '' || strpos($u, 'http') !== 0 || get_transient('wpc_shland223_' . md5($tplKey))) {
                return false;
            }
            set_transient('wpc_shland223_' . md5($tplKey), 1, 600);
            $r = wp_remote_get($u, ['timeout' => 6]);
            $b = !is_wp_error($r) && (int) wp_remote_retrieve_response_code($r) === 200
                ? (string) wp_remote_retrieve_body($r) : '';
            $j = json_decode($b, true);
            if (is_array($j) && isset($j[0]['url']) && wpc_used_css_store_sheets($tplKey, $j)) {
                if ($cd !== '' && function_exists('wpc_crit_meta_write')) {
                    wpc_crit_meta_write(rtrim($cd, '/') . '/used_css_sheets_url.txt', $u);
                }
                if (function_exists('wpc_cache_first_log')) {
                    wpc_cache_first_log('sheets-manifest-landed', '', substr($u, 0, 90), ['n' => count($j), 'tpl' => $tplKey]);
                }
                return true;
            }
            return false;
...
if (!function_exists('wpc_used_css_process_body')) {
    ...
    function wpc_used_css_disk_resanitize()
    {
        ...
        foreach ((array) @glob(rtrim(WPS_IC_CRITICAL, '/') . '/used-css/tpl-*.css') as $wpc_uf394) {
            ...
            if (is_string($wpc_un394) && $wpc_un394 !== $wpc_ub394) {
                wpc_fs_put($wpc_uf394, $wpc_un394);
                if (function_exists('wpc_cache_first_log')) {
    ...
    function wpc_used_css_process_body($body)
    {
        ...
        if (function_exists('wpc_css_host_twin_heal231')) {
            $body = wpc_css_host_twin_heal231($body);
        }
        ...
        if (function_exists('wpc_used_css_strip_faces245')) {
            $body = wpc_used_css_strip_faces245($body);
        }
        if (function_exists('wpc_used_css_bg_park255')) {
            $body = wpc_used_css_bg_park255($body);
        }
        return $body;
    }
}
if (!function_exists('wpc_used_css_bg_park255')) {
    ...
    function wpc_bgpark_scan255($css, $i, $n)
    {
    ...
    function wpc_bgpark_close255($css, $open, $n)
    {
        $d = 1; $j = $open;
        while ($d > 0) {
            $j = wpc_bgpark_scan255($css, $j + 1, $n);
            if ($j === false) { return false; }
            $d += ($css[$j] === '{') ? 1 : -1;
        }
        return $j;
    }
    function wpc_bgpark_split255($v, $sep)
    {
        
        $out = []; $d = 0; $q = ''; $cur = '';
        for ($j = 0, $n = strlen($v); $j < $n; $j++) {
            $c = $v[$j];
            if ($q !== '') { $cur .= $c; if ($c === '\\') { $cur .= isset($v[$j + 1]) ? $v[++$j] : ''; continue; } if ($c === $q) { $q = ''; } continue; }
            if ($c === "'" || $c === '"') { $q = $c; $cur .= $c; continue; }
            if ($c === '(') { $d++; } elseif ($c === ')') { $d--; }
            if ($c === $sep && $d <= 0) { $out[] = $cur; $cur = ''; continue; }
            $cur .= $c;
        }
        $out[] = $cur;
        return $out;
    }
    function wpc_bgpark_guard255($sel)
    {
        $parts = wpc_bgpark_split255($sel, ',');
        foreach ($parts as $k => $pp) {
            $pp = trim($pp);
            if ($pp === '') { continue; }
            if (preg_match('/^(html|:root)(?![\w-])/i', $pp)) {
                $parts[$k] = preg_replace('/^(html|:root)/i', '$1:where(.wpc-bgl255)', $pp, 1);
            } else {
                $parts[$k] = ':where(html.wpc-bgl255) ' . $pp;
            }
        }
        return implode(',', $parts);
    }
    function wpc_bgpark_rule255($body, &$taken)
    {
        if (stripos($body, 'url(') === false) { return $body; }
        $decls = wpc_bgpark_split255($body, ';');
        $keep = []; $take = [];
        foreach ($decls as $dc) {
            $prop = strtolower(trim((string) strstr($dc, ':', true)));
            if (($prop === 'background' || $prop === 'background-image')
                && preg_match('~url\(\s*[\'"]?(?!data:)~i', $dc)) {
        ...
        if (!empty($take)) { $taken = implode(';', $take); }
        return implode(';', $keep);
    }
    function wpc_bgpark_walk255($css, $ctx, &$moved)
    {
        $out = ''; $i = 0; $n = strlen($css);
        while ($i < $n) {
            $b = wpc_bgpark_scan255($css, $i, $n);
            if ($b === false || $css[$b] === '}') { $out .= substr($css, $i); break; }
            $sel = substr($css, $i, $b - $i);
            $e = wpc_bgpark_close255($css, $b, $n);
            if ($e === false) { $out .= substr($css, $i); break; }
            $inner = substr($css, $b + 1, $e - $b - 1);
            $selT = trim($sel);
            if ($selT !== '' && $selT[0] === '@') {
                $at = strtolower($selT);
                if (strpos($at, '@media') === 0 || strpos($at, '@supports') === 0) {
                    $out .= $sel . '{' . wpc_bgpark_walk255($inner, array_merge($ctx, [$selT]), $moved) . '}';
                } else {
                ...
                $taken = '';
                $out .= $sel . '{' . wpc_bgpark_rule255($inner, $taken) . '}';
                if ($taken !== '') { $moved[] = [$ctx, $selT, $taken]; }
            }
            $i = $e + 1;
        }
        return $out;
    }
    function wpc_used_css_bg_park255($css)
    {
        try {
            ...
            $moved = [];
            $base = wpc_bgpark_walk255($css, [], $moved);
            if (empty($moved)) { return $css; }
            $park = '';
            foreach ($moved as $mv) {
                $w = wpc_bgpark_guard255($mv[1]) . '{' . $mv[2] . '}';
                foreach (array_reverse($mv[0]) as $wrap) { $w = $wrap . '{' . $w . '}'; }
                $park .= $w;
            }
            $out = $base . $park;
            
            
            if ((substr_count($out, '{') - substr_count($css, '{')) !== (substr_count($out, '}') - substr_count($css, '}'))
                || substr_count(strtolower($out), 'url(') !== substr_count(strtolower($css), 'url(')) {
                return $css;
            }
            return $out;
        } catch (\Throwable $e) {
...
if (!function_exists('wpc_used_css_strip_faces245')) {
    function wpc_used_css_strip_faces245($css)
    {
        try {
            if (!is_string($css) || stripos($css, '@font-face') === false
                || !apply_filters('wpc_used_css_strip_url_faces', true)) {
                return $css;
            }
            $wpc_sf245 = preg_replace_callback('/@font-face\s*\{[^}]*\}/is', function ($m) {
                if (stripos($m[0], 'url(') === false || stripos($m[0], 'url(data:') !== false
                    || stripos($m[0], "url('data:") !== false || stripos($m[0], 'url("data:') !== false) {
                    return $m[0];
                }
                return '';
            }, $css);
            return is_string($wpc_sf245) ? $wpc_sf245 : $css;
        } catch (\Throwable $e) {
            return $css;
        }
    }
...
if (!function_exists('wpc_crit_parse_belt198')) {
    ...
    function wpc_crit_parse_belt198($html)
    {
        try {
            if (!is_string($html) || $html === ''
                || strpos($html, 'id="wpc-critical-css"') === false
                || !class_exists('wps_rewriteLogic')
                || !method_exists('wps_rewriteLogic', 'wpc_css_requote_urls194')
                || !apply_filters('wpc_crit_parse_belt', true)) {
                return $html;
            }
            $out = preg_replace_callback('/(<style\b[^>]*id="wpc-critical-css"[^>]*>)(.*?)(<\/style>)/is', function ($m) {
                $fixed = wps_rewriteLogic::wpc_css_requote_urls194($m[2]);
                if (!is_string($fixed) || $fixed === '' || $fixed === $m[2]) {
                    return $m[0];
                }
                if (function_exists('wpc_cache_first_log') && function_exists('get_transient')
                    && !get_transient('wpc_belt198_logged')) {
                    set_transient('wpc_belt198_logged', 1, 3600);
                    wpc_cache_first_log('crit-final-belt-requoted', '', '', ['d' => strlen($fixed) - strlen($m[2])]);
                }
    ...
    function wpc_css_insert_fallbacks($css)
    {
        try {
            ...
            foreach ($table as $fam => $row) {
                ...
                $css2 = preg_replace(
                    '/(font-family\s*:\s*)([\'"]?)' . preg_quote($fam, '/') . '\2\s*(,(?!\s*[\'"]?' . preg_quote($fb, '/') . ')|;|\}|!)/i',
                    '$1$2' . $fam . '$2,"' . $fb . '"$3',
                    $css2
                );
                if (!is_string($css2)) {
...
if (!function_exists('wpc_header_css_slice')) {
    ...
    function wpc_header_css_slice($cssPath, $tokens = [])
    {
        try {
            ...
            wpc_fs_put($cache, $slice, LOCK_EX);
            return $slice;
        } catch (\Throwable $e) {
            return '';
        }
        ...
        try {
            ...
            foreach (['', 'mobile', 'desktop'] as $dev) {
                ...
                if (is_string($new) && $new !== $body) {
                    ...
                    if (wpc_fs_put($tmp, $new, LOCK_EX) !== false && @rename($tmp, $p)) {
                        if (function_exists('wpc_cache_first_log')) {
...
if (!function_exists('wpc_pressure_bounded28')) {
    function wpc_pressure_bounded28($lane, $max = 6)
    {
        if (!(function_exists('wpc_under_pressure') && wpc_under_pressure())) {
            delete_option('wpc_shed28_' . $lane);
            return false;
        }
        $n = (int) get_option('wpc_shed28_' . $lane, 0);
        if ($n < max(1, (int) apply_filters('wpc_pressure_shed_max', $max, $lane))) {
            update_option('wpc_shed28_' . $lane, $n + 1, false);
            return true;
        }
        delete_option('wpc_shed28_' . $lane);
        if (function_exists('wpc_cache_first_log')) {
            wpc_cache_first_log('pressure-forced-through', $lane, '', ['sheds' => $n]);
        }
        return false;
    }
}
if (!function_exists('wpc_opcache_full70')) {
    




    function wpc_opcache_full70()
    {
        if (!function_exists('opcache_get_status')) {
            return false;
        }
        $s = @opcache_get_status(false);
        return is_array($s) && !empty($s['opcache_enabled']) && !empty($s['cache_full']);
    }
    add_action('admin_init', function () {
        if (!function_exists('wpc_state81') || !wpc_opcache_full70()) { if (function_exists('wpc_state_clear81')) { wpc_state_clear81('opcache_full'); } return; }
        if (function_exists('wpc_cache_first_log') && function_exists('wpc_admin_held69') && !wpc_admin_held69('wpc_opcache_log70')) {
            wpc_admin_hold69('wpc_opcache_log70', DAY_IN_SECONDS);
            wpc_cache_first_log('opcache-full', '', '', ['mem' => (string) ini_get('opcache.memory_consumption')]);
        }
        if (function_exists('wpc_state_clear81')) {
            wpc_state_clear81('opcache_full');
        }
    }, 30);
}
if (!function_exists('wpc_state81')) {
    function wpc_states81_list()
    {
        $s = get_option('wpc_states81', []);
        return is_array($s) ? $s : [];
    }
    function wpc_state81($id, $level, $text, $cta_url = '', $cta_label = '')
    {
        ...
        $s = wpc_states81_list();
        $prev = (isset($s[$id]) && is_array($s[$id])) ? $s[$id] : [];
        $s[$id] = [
            'level'     => in_array($level, ['error', 'warning', 'info', 'success'], true) ? $level : 'info',
            'text'      => (string) $text,
            'cta'       => (string) $cta_url,
        ...
        if (function_exists('wpc_cache_first_log') && function_exists('wpc_admin_held69') && !wpc_admin_held69('wpc_state81_' . $id)) {
            wpc_admin_hold69('wpc_state81_' . $id, DAY_IN_SECONDS);
            wpc_cache_first_log('state-' . $id, '', '', ['level' => $s[$id]['level'], 'text' => substr((string) $text, 0, 160)]);
        }
        return true;
    }
    function wpc_state_clear81($id)
    {
        $id = sanitize_key((string) $id);
        $s = wpc_states81_list();
        if (!isset($s[$id])) {
        ...
        update_option('wpc_states81', $s, false);
        if (function_exists('wpc_cache_first_log')) {
            wpc_cache_first_log('state-cleared', '', '', ['id' => $id]);
        }
        return true;
    }
    function wpc_states81_render()
    {
        if (!function_exists('current_user_can') || (!current_user_can('manage_wpc_settings') && !current_user_can('manage_options'))) {
            return;
        }
        $out = '';
        foreach (wpc_states81_list() as $id => $st) {
            if (!is_array($st) || !empty($st['dismissed']) || empty($st['text'])) {
                continue;
            }
            $lvl = isset($st['level']) ? (string) $st['level'] : 'info';
            $dis = wp_nonce_url(admin_url('admin-post.php?action=wpc_state81_dismiss&id=' . rawurlencode((string) $id)), 'wpc_state81_' . $id);
            $out .= '<div class="wpc-state wpc-state--' . esc_attr($lvl) . '" data-wpc-state="' . esc_attr((string) $id) . '"><span class="wpc-state-text">' . esc_html((string) $st['text']) . '</span>';
            if (!empty($st['cta'])) {
                $label = (isset($st['cta_label']) && $st['cta_label'] !== '') ? (string) $st['cta_label'] : __('Fix', 'wp-compress-image-optimizer');
                $out .= ' <a class="wpc-state-cta" href="' . esc_url((string) $st['cta']) . '">' . esc_html($label) . '</a>';
            }
            $out .= ' <a class="wpc-state-dismiss" href="' . esc_url($dis) . '" aria-label="' . esc_attr__('Dismiss', 'wp-compress-image-optimizer') . '">&times;</a></div>';
        }
        if ($out === '') {
            return;
        }
    ...
    add_action('admin_post_wpc_state81_dismiss', function () {
...
if (!function_exists('wpc_admin_held69')) {
    ...
    function wpc_admin_ticks69()
    {
        $t = get_option('wpc_admin_tick69');
        return is_array($t) ? $t : [];
    }
    function wpc_admin_held69($lane)
    {
        $t = wpc_admin_ticks69();
        return isset($t[$lane]) && (int) $t[$lane] > time();
    }
    function wpc_admin_hold69($lane, $secs)
    {
        $t = wpc_admin_ticks69();
        $t[(string) $lane] = time() + max(1, (int) $secs);
        if (count($t) > 64) {
            $t = array_slice($t, -64, null, true);
        }
        update_option('wpc_admin_tick69', $t, true);
    }
}

if (!function_exists('wpc_constrained_host65')) {
    




    function wpc_constrained_host65()
    {
        static $c = null;
        if ($c === null) {
            $c = (bool) apply_filters('wpc_constrained_host',
                @file_exists('/etc/cloudlinux-release') || @file_exists('/proc/lve/list') || @is_dir('/usr/local/cpanel'));
        }
        return $c;
    }
}
...
if (!function_exists('wpc_land_purge_coalesced')) {
    ...
    function wpc_land_purge_coalesced($urlKey, $url, $context)
    {
        ...
        if (strpos((string) $context, 'crit') === 0) {
            update_option($k, ['t' => $now, 'dirty' => 0, 'u' => (string) $url], false);
            wps_ic_cache_integrations::purgeUrlHtml($urlKey, $url, ['context' => $context, 'warm' => true, 'force' => true]);
            return;
        }
        if (!is_array($c) || ($now - (int) ($c['t'] ?? 0)) > 240) {
            ...
            wps_ic_cache_integrations::purgeUrlHtml($urlKey, $url, ['context' => $context, 'warm' => true]);
            return;
        }
        $c['dirty'] = 1;
        if (empty($c['u']) && $url !== '') {
    ...
    function wpc_land_finalize_handler($urlKey)
    {
        ...
        if (is_array($c) && !empty($c['dirty'])
            ...
            wps_ic_cache_integrations::purgeUrlHtml((string) $urlKey, (string) ($c['u'] ?? ''), ['context' => 'land-finalize', 'warm' => true]);
            if (function_exists('wpc_cache_first_log')) {
...
if (!function_exists('wpc_land_repoll55_handler')) {
    function wpc_land_repoll55_handler($urlKey, $announced = '')
    {
        $urlKey = (string) $urlKey;
        $announced = preg_replace('/[^A-Za-z0-9-]/', '', (string) $announced);
        if ($urlKey === '' || $announced === '' || !defined('WPS_IC_CRITICAL') || !defined('WPS_IC_DIR')) {
            return;
        }
        if (function_exists('wpc_under_pressure') && wpc_under_pressure()) {
            return;
        ...
        $before = preg_replace('/[^A-Za-z0-9-]/', '', (string) @file_get_contents($cd . 'land_uuid.txt'));
        $cc = new wps_criticalCss();
        $ok = $cc->pullDerivedArtifacts($urlKey, $announced, true);
        $after = preg_replace('/[^A-Za-z0-9-]/', '', (string) @file_get_contents($cd . 'land_uuid.txt'));
        if ($after === $announced) {
            delete_transient('wpc_land_announced55_' . md5($urlKey));
            if (class_exists('wps_ic_cache_integrations') && method_exists('wps_ic_cache_integrations', 'purgeUrlHtml')) {
                wps_ic_cache_integrations::purgeUrlHtml($urlKey, '', ['context' => 'land-repoll55', 'warm' => true]);
            }
        }
        if (function_exists('wpc_cache_first_log')) {
            wpc_cache_first_log('land-repoll55-done', $urlKey, '', ['ok' => $ok ? 1 : 0, 'before' => substr($before, 0, 8), 'after' => substr($after, 0, 8), 'announced' => substr($announced, 0, 8)]);
        }
    }
    add_action('wpc_land_repoll55', 'wpc_land_repoll55_handler', 10, 2);
}
...
if (!function_exists('wpc_font_metrics_from_artifact')) {
    ...
    function wpc_font_metrics_from_artifact($table)
    {
        ...
        if ($cached === null) {
            ...
            try {
                if ($cached) {
                    if (get_option('wpc_fm_mirror44') !== $cached) {
                        update_option('wpc_fm_mirror44', $cached, false);
                    }
                } elseif (apply_filters('wpc_font_metrics_mirror', true)) {
                    $wpc_mir44 = get_option('wpc_fm_mirror44');
                    if (is_array($wpc_mir44) && !empty($wpc_mir44)) {
                        $cached = $wpc_mir44;
                        if (function_exists('wpc_cache_first_log') && function_exists('get_transient')
                            && !get_transient('wpc_fm_mirror_log44')) {
                            set_transient('wpc_fm_mirror_log44', 1, 3600);
                            wpc_cache_first_log('font-metrics-mirror-served', '', '', ['fams' => count($wpc_mir44)]);
                        }
                    }
                }
            } catch (\Throwable $e) {
...
if (!function_exists('wpc_delay_manifest_self_heal')) {
    function wpc_delay_manifest_self_heal()
    {
        try {
            ...
            if (wpc_admin_held69('wpc_manifest_heal_lock')) {
                return;
            }
            ...
            if ($target_key === '') {
                wpc_admin_hold69('wpc_manifest_heal_lock', 10 * MINUTE_IN_SECONDS);
                return;
            }
            wpc_admin_hold69('wpc_manifest_heal_lock', HOUR_IN_SECONDS);
...
if (!function_exists('wpc_crit_land_self_heal')) {
    function wpc_crit_land_self_heal()
    {
        try {
            ...
            if (wpc_admin_held69('wpc_landheal_lock')) {
                return;
            }
            ...
            if ($target_key === '') {
                wpc_admin_hold69('wpc_landheal_lock', 10 * MINUTE_IN_SECONDS);
                return;
            }
            wpc_admin_hold69('wpc_landheal_lock', 15 * MINUTE_IN_SECONDS);
            $tries[$target_key] = (int) ($tries[$target_key] ?? 0) + 1;
...
if (!function_exists('wpc_font_display_effective')) {
    ...
    function wpc_font_display_effective($raw, $family = '')
    {
        ...
        if ($raw === 'smart' || ($raw === 'swap' && apply_filters('wpc_fd_swap_to_optional_when_validated', false))) {
...
if (!function_exists('wpc_compute_tpl_key')) {
    ...
    function wpc_dispatch_tpl_key($urlKey)
    {
        ...
        if ($tk === '') {
            ...
            if (function_exists('wpc_compute_tpl_key') && function_exists('did_action') && did_action('template_redirect')
                && !(function_exists('is_admin') && is_admin())) {
                try { $tk = trim((string) wpc_compute_tpl_key()); } catch (\Throwable $e) { $tk = ''; }
            }
            if ($tk === '') {
                return '';
            }
...
if (!function_exists('wpc_crit_mark_stale_instead')) {
    function wpc_crit_mark_stale_instead($url_key = '')
    {
        try {
            ...
            if ($k !== 'all' && $k !== 'upgrade' && strpos($k, '..') === false && defined('WPS_IC_CRITICAL') && function_exists('wpc_crit_meta_write')) {
                $wpc_sd52 = rtrim(WPS_IC_CRITICAL, '/') . '/' . ltrim($k, '/') . '/';
                if (@is_file($wpc_sd52 . 'critical_desktop.css') || @is_file($wpc_sd52 . 'critical_mobile.css')) {
                    wpc_crit_meta_write($wpc_sd52 . 'stale.txt', (string) time());
                }
            }
...
if (!function_exists('wpc_crit_delete_files196')) {
    ...
    function wpc_crit_delete_files196()
    {
        $n = 0;
        try {
            if (!defined('WPS_IC_CRITICAL')) {
            ...
            $root = rtrim(WPS_IC_CRITICAL, '/');
            @unlink($root . '/bypass.txt');
            $wpc_kill196 = ['critical_desktop.css', 'critical_mobile.css', 'critical_combined.css',
                'land_uuid.txt', 'land_rev.txt', 'land_ts.txt', 'uuid.txt', 'stale.txt',
                'bypass.txt', 'gen_fails.json'];
            $dirs = (array) @glob($root . '/*', GLOB_ONLYDIR);
            $cap = (int) apply_filters('wpc_crit_delete_dir_cap', 3000);
            foreach ($dirs as $wpc_i196 => $d) {
                if ($wpc_i196 >= $cap) {
                    break;
                }
                foreach ($wpc_kill196 as $f) {
            ...
            if (function_exists('wpc_cache_first_log')) {
                wpc_cache_first_log('crit-files-deleted', '', '', ['files' => $n, 'dirs' => count($dirs)]);
            }
        } catch (\Throwable $e) {
        }
        return $n;
...
if (!function_exists('wpc_repull_kick_now')) {
    ...
    function wpc_kick_global_bucket_ok()
    {
        try {
            ...
            if (!function_exists('wpc_fs_update') || !is_dir($dir) || !is_writable($dir)) { return false; }
            list($locked, $ok) = wpc_fs_update($dir . 'gbucket', function ($cur) use ($cap, $win) {
                $st    = json_decode((string) $cur, true);
                $now   = time();
                $start = (is_array($st) && isset($st['s'])) ? (int) $st['s'] : 0;
                $cnt   = (is_array($st) && isset($st['c'])) ? (int) $st['c'] : 0;
                if (($now - $start) >= $win) { $start = $now; $cnt = 0; }
                $ok = ($cnt < $cap);
                return [$ok ? json_encode(['s' => $start, 'c' => $cnt + 1]) : null, $ok];
            }, true);
    ...
    {
        try {
            if (!$inline && function_exists('wpc_render_guard39_active') && wpc_render_guard39_active() && function_exists('wpc_net_defer39')) {
                wpc_net_defer39('kick:' . md5((string) $urlKey), function () use ($urlKey) { wpc_repull_kick_now($urlKey); });
                return true;
            }
    ...
    function wpc_repull_kick_receiver()
    {
        try {
            ...
            if (function_exists('wpc_under_pressure') && wpc_under_pressure()
                && apply_filters('wpc_kick_pressure_gate', true)) {
                $wpc_pk360 = isset($_GET['k']) ? sanitize_text_field((string) $_GET['k']) : '';
                
                
                if (function_exists('set_transient') && $wpc_pk360 !== '') {
                    set_transient('wpc_kick_pressure360_' . md5($wpc_pk360), 1,
                        (int) apply_filters('wpc_kick_pressure_backoff', 90));
                }
                if (function_exists('wpc_cache_first_log')) {
                    wpc_cache_first_log('kick-pressure-deferred', $wpc_pk360, '', []);
                }
                wp_die('', '', ['response' => 200]);
            }
            ...
            if ($k === '' || get_transient('wpc_repull_kick_' . md5($k)) || wpc_kick_recent($k, 120)
                || get_transient('wpc_kick_pressure360_' . md5($k))) {
                wp_die('', '', ['response' => 200]);
            }
            ...
            if ($have && function_exists('wpc_lcp_repull_handler')) {
                ...
                && ($wpc_unserv685 !== false || $wpc_stale73
...
if (!function_exists('wpc_crit_self_arm_watchdog')) {
    function wpc_crit_self_arm_watchdog()
    {
        try {
            ...
            if (wpc_admin_held69('wpc_crit_watchdog')) {
                return;
            }
            if (!is_admin() && !(function_exists('wp_doing_ajax') && wp_doing_ajax())
            ...
            wpc_admin_hold69('wpc_crit_watchdog', 120);
            $set = get_option(WPS_IC_SETTINGS);
    ...
    add_action('wpc_crit_redispatch', function ($urlKey, $sync = 0) {
        try {
            ...
            if (empty($urlKey) || !class_exists('wps_criticalCss') || !class_exists('wps_ic_url_key')) {
                if (function_exists('wpc_cache_first_log')) {
                    wpc_cache_first_log('redispatch-bail', (string) $urlKey, '', ['cls' => class_exists('wps_criticalCss') ? 1 : 0]);
                }
                return;
            }
...
if (!function_exists('wpc_crit_webhook_rx')) {
    ...
    function wpc_crit_webhook_rx()
    {
        try {
            ...
            if (function_exists('ignore_user_abort')) { @ignore_user_abort(true); }
            $wpc_landed173 = false;
            $wpc_rev156 = (int) (isset($j['wire_rev']) ? $j['wire_rev'] : (isset($j['rev']) ? $j['rev'] : 0));
            if (class_exists('wps_criticalCss')) {
                $cc = new wps_criticalCss();
                $wpc_landed173 = (bool) $cc->pullDerivedArtifacts($k, $uuid, true, $wpc_rev156);
                if ($wpc_landed173 && function_exists('wpc_cache_first_log')) {
                    wpc_cache_first_log('whk-landed', $k, '', ['uuid' => substr($uuid, 0, 8), 'rev' => $wpc_rev156]);
                }
            } else {
            ...
            echo wp_json_encode(['landed' => $wpc_landed173, 'uuid' => (string) $uuid, 'rev' => $wpc_rev156,
                'reason' => $wpc_landed173 ? '' : (class_exists('wps_criticalCss') ? 'pull-refused' : 'no-class-cron-nudged')]);
            exit;
        } catch (\Throwable $e) {
...
if (!function_exists('wpc_pending_pull174')) {
    function wpc_pending_pull174()
    {
        try {
            if ((function_exists('is_admin') && is_admin()) || !defined('WPS_IC_CRITICAL') || !class_exists('wps_ic_url_key')) {
                return;
            }
            if (!apply_filters('wpc_pending_pull', true)) {
                return;
            }
            $k = ltrim((string) (new wps_ic_url_key())->setup(), '/');
            if ($k === '') {
                return;
            }
            $d = rtrim(WPS_IC_CRITICAL, '/') . '/' . $k . '/';
            $wpc_dt174 = (int) @file_get_contents($d . 'dispatch_ts.txt');
            if ($wpc_dt174 < 1 || (time() - $wpc_dt174) > (int) apply_filters('wpc_pending_pull_window', 900)) {
                return;
            }
            if ((int) @filemtime($d . 'land_uuid.txt') >= $wpc_dt174) {
                return;
            }
            if (get_transient('wpc_ppull_' . md5($k))) {
                return;
            }
            set_transient('wpc_ppull_' . md5($k), 1, (int) apply_filters('wpc_pending_pull_every', 30));
            if (!wpc_shutdown_detach_or_cron66($k)) {
                return;
            }
            if (function_exists('ignore_user_abort')) { @ignore_user_abort(true); }
            if (function_exists('set_time_limit')) { @set_time_limit(30); }
            if (function_exists('wpc_crit_kick_collect') && wpc_crit_kick_collect($k)
                && (int) @filemtime($d . 'land_uuid.txt') >= $wpc_dt174
                && function_exists('wpc_cache_first_log')) {
                wpc_cache_first_log('pending-pull-landed', $k, '', ['age' => time() - $wpc_dt174]);
            }
        } catch (\Throwable $e) {
        }
    }
    add_action('shutdown', 'wpc_pending_pull174', 99);
}
if (!function_exists('wpc_shutdown_detach_or_cron66')) {
    




    function wpc_shutdown_detach_or_cron66($k)
    {
        $wpc_fin66 = function_exists('wpc_finish_request39') ? wpc_finish_request39() : false;
        if ($wpc_fin66) {
            return true;
        }
        if ($k !== '' && function_exists('wpc_pl_sched') && function_exists('wp_next_scheduled')
            && !wp_next_scheduled('wpc_lcp_repull', [$k, 1])) {
            wpc_pl_sched(time() + 60, 'wpc_lcp_repull', [$k, 1]);
        }
        return false;
    }
    ...
    function wpc_pointer_tick188()
    {
        try {
            if ((function_exists('is_admin') && is_admin()) || !defined('WPS_IC_CRITICAL')
                || !class_exists('wps_ic_url_key')) {
                return;
            }
            if (!apply_filters('wpc_pointer_tick', true)) {
                return;
            }
            if (!class_exists('wps_criticalCss') && defined('WPS_IC_DIR')) {
                @include_once WPS_IC_DIR . 'addons/criticalCss/criticalCss-v2.php';
            ...
            $d = rtrim(WPS_IC_CRITICAL, '/') . '/' . $k . '/';
            if (get_transient('wpc_ptick_hold196')) {
                return;
            }
            $lu = trim((string) @file_get_contents($d . 'land_uuid.txt'));
            if ($lu === '') {
                return;
            }
            if (get_transient('wpc_ptick_' . md5($k))) {
                return;
            }
            set_transient('wpc_ptick_' . md5($k), 1, (int) apply_filters('wpc_pointer_tick_every', 60));
            if (!wpc_shutdown_detach_or_cron66($k)) {
                return;
            }
            $cc = new wps_criticalCss();
            if (!method_exists($cc, 'fetchLatestPointerUuid') || !method_exists($cc, 'pullDerivedArtifacts')) {
                return;
            }
            $p = preg_replace('/[^a-f0-9-]/i', '', (string) $cc->fetchLatestPointerUuid($k));
            if (strlen($p) < 8 || $p === $lu) {
                return;
            }
            ...
            if ($cc->pullDerivedArtifacts($k, $p, true)
                && trim((string) @file_get_contents($d . 'land_uuid.txt')) === $p
                && function_exists('wpc_cache_first_log')) {
                wpc_cache_first_log('pointer-tick-landed', $k, '', ['uuid' => substr($p, 0, 8), 'was' => substr($lu, 0, 8)]);
            }
        } catch (\Throwable $e) {
        }
    }
    add_action('shutdown', 'wpc_pointer_tick188', 98);
}
...
if (!function_exists('wpc_cflog_viewer')) {
    ...
    if (apply_filters('wpc_cflog_nopriv', true)) {
        add_action('wp_ajax_nopriv_wpc_cflog', function () {
            $wpc_k66 = isset($_REQUEST['key']) ? (string) $_REQUEST['key'] : '';
            $wpc_o66 = defined('WPS_IC_OPTIONS') ? get_option(WPS_IC_OPTIONS) : false;
            $wpc_a66 = (is_array($wpc_o66) && !empty($wpc_o66['api_key'])) ? (string) $wpc_o66['api_key'] : '';
            if ($wpc_a66 === '' || $wpc_k66 === '' || !hash_equals($wpc_a66, $wpc_k66)) {
                wp_send_json_error('forbidden', 403);
            }
            $log = function_exists('wpc_cflog_read') ? wpc_cflog_read(40) : (array) get_option('wpc_cache_first_log', []);
            foreach ($log as &$e) {
...
if (!function_exists('wpc_font_face_fallback_norm148')) {
    ...
    function wpc_font_face_fallback_norm148($fe)
    {
        try {
            if (!is_array($fe)) {
                return $fe;
            }
            $fb = (!empty($fe['fallback']) && is_array($fe['fallback'])) ? $fe['fallback'] : null;
            if ($fb === null) {
                return $fe;
            }
            if (empty($fe['validated']) && !empty($fb['validated'])) {
                $fe['validated'] = true;
            }
            $row = (!empty($fe['metrics']) && is_array($fe['metrics'])) ? $fe['metrics'] : [];
            foreach (['size_adjust' => 'size-adjust', 'size-adjust' => 'size-adjust',
                      'ascent_override' => 'ascent-override', 'ascent-override' => 'ascent-override',
                      'descent_override' => 'descent-override', 'descent-override' => 'descent-override',
                      'line_gap_override' => 'line-gap-override', 'line-gap-override' => 'line-gap-override'] as $k => $h) {
                if (empty($row[$h]) && !empty($fb[$k]) && preg_match('/^[0-9.]+%$/', (string) $fb[$k])) {
                    $row[$h] = (string) $fb[$k];
                }
            }
            if (empty($row['fallback_stack']) && !empty($fb['fallback_stack']) && is_array($fb['fallback_stack'])) {
                $stack = [];
                foreach ($fb['fallback_stack'] as $n) {
                ...
                if ($stack) {
                    $row['fallback_stack'] = $stack;
                    if (empty($row['local'])) {
                        $row['local'] = $stack[0];
                    }
                }
            }
            if (empty($row['local']) && !empty($fb['local']) && is_string($fb['local'])) {
                $row['local'] = $fb['local'];
            }
            if (!empty($row)) {
                $fe['metrics'] = $row;
        ...
        try {
            ...
            $write = function ($p, $v) {
                return function_exists('wpc_crit_meta_write')
                    ? wpc_crit_meta_write($p, $v) : wpc_fs_put($p, $v);
            };
            $rev = (int) $wireRev;
...
if (!function_exists('wpc_manifest_consume')) {
    ...
    function wpc_manifest_consume($urlKey, $manifestUrl, $genId, $pageUrl = '')
    {
        try {
            ...
            if (!is_dir($dir)) { @wp_mkdir_p($dir); }
            $write = function ($p, $v) {
                return function_exists('wpc_crit_meta_write') ? wpc_crit_meta_write($p, $v) : wpc_fs_put($p, $v);
            };
    ...
    function wpc_consume_fonts_artifact($fonts, $urlKey)
    {
        ...
        try {
            ...
            foreach ($fonts as $fe) {
                ...
                if (function_exists('wpc_font_face_fallback_norm148')) {
                    $fe = wpc_font_face_fallback_norm148($fe);
                }
                
                
                
                if (!isset($wpc_own151)) { $wpc_own151 = []; }
                $wpc_ob151 = null;
                if (!empty($fe['fonts_owner']) && is_array($fe['fonts_owner'])) {
                    $wpc_ob151 = $fe['fonts_owner'];
                } elseif (!empty($fe['owner']) && is_string($fe['owner'])) {
                    $wpc_ob151 = ['owner' => (string) $fe['owner'], 'complete' => !empty($fe['complete']),
                        'reason' => isset($fe['reason']) ? (string) $fe['reason'] : ''];
                }
                if (is_array($wpc_ob151)) {
                    if (isset($wpc_ob151['mobile']) || isset($wpc_ob151['desktop'])) {
                        $wpc_own151 = $wpc_ob151 + $wpc_own151;
                    } elseif (!empty($fe['device']) && in_array($fe['device'], ['mobile', 'desktop'], true)) {
                        $wpc_own151[$fe['device']] = $wpc_ob151;
                    } elseif (empty($wpc_own151)) {
                        $wpc_own151 = $wpc_ob151;
                    }
                }
                ...
                if (empty($fe['url'])) { continue; }
                if (!isset($wpc_dlq205)) { $wpc_dlq205 = []; }
                $wpc_dlq205[] = [(int) ($fe['bytes'] ?? 999999), $fe];
            }
            ...
            if (!empty($wpc_dlq205)) {
                usort($wpc_dlq205, function ($a, $b) { return $a[0] - $b[0]; });
                $wpc_cap205 = max(1, (int) apply_filters('wpc_font_download_cap', 12));
                foreach ($wpc_dlq205 as $wpc_d205) {
                    if ($n >= $wpc_cap205) { break; }
                    if ((time() - $wpc_fonts_t0) > (int) apply_filters('wpc_fonts_download_budget', 10)) { break; }
                    $fe = $wpc_d205[1];
                    $fu = (string) $fe['url'];
            ...
            if (defined('WPS_IC_CRITICAL')) {
                ...
                if (!empty($wpc_own151) && is_array($wpc_own151)) {
                    wpc_crit_meta_write($cd . 'fonts-owner.json', wp_json_encode($wpc_own151));
                    if (function_exists('wpc_cache_first_log')) {
                        wpc_cache_first_log('fonts-owner-mirror', (string) $urlKey, '', ['o' => wp_json_encode($wpc_own151)]);
                    }
                }
                ...
                if (!empty($wpc_fdval219)) {
                    $wpc_fdold72 = get_option('wpc_font_metrics_validated');
                    $wpc_fdnew72 = ['t' => time(), 'fams' => array_values(array_unique($wpc_fdval219))];
                    update_option('wpc_font_metrics_validated', $wpc_fdnew72, false);
                    ...
                    $wpc_fdoldf72 = (is_array($wpc_fdold72) && !empty($wpc_fdold72['fams']) && is_array($wpc_fdold72['fams']))
                        ? $wpc_fdold72['fams'] : [];
                    if (array_diff($wpc_fdnew72['fams'], $wpc_fdoldf72) !== []
                        && apply_filters('wpc_fd_validate_purge', true)
                        && class_exists('wps_ic_cache') && method_exists('wps_ic_cache', 'removeHtmlCacheFiles')) {
                        wps_ic_cache::removeHtmlCacheFiles('all', '', '', 'soft');
                        if (function_exists('wpc_cache_first_log')) {
                            wpc_cache_first_log('fd-validate-purge', '', '', ['fams' => count($wpc_fdnew72['fams'])]);
                        }
                    }
                }
                if (apply_filters('wpc_atf_subset_inline', true)) {
                    $wpc_cand191 = [];
                    foreach ($fonts as $fe) {
                        if (!is_array($fe) || empty($fe['url']) || empty($fe['family'])) { continue; }
                        ...
                        $wpc_cand191[] = [strlen($w), $fe, $w];
                    }
                    usort($wpc_cand191, function ($a, $b) { return $a[0] - $b[0]; });
                    ...
                    $wpc_fr193 = [];
                    foreach ($fonts as $wpc_fe193) {
                        if (count($wpc_fr193) >= 64 || !is_array($wpc_fe193)
                            || empty($wpc_fe193['url']) || empty($wpc_fe193['unicode_range'])) { continue; }
                        $wpc_fb193 = basename((string) parse_url((string) $wpc_fe193['url'], PHP_URL_PATH));
                        $wpc_ur193 = preg_replace('/[^0-9A-Fa-fUu+,\- ]/', '', (string) $wpc_fe193['unicode_range']);
                        if ($wpc_fb193 === '' || $wpc_ur193 === ''
                            || !preg_match('/^[A-Za-z0-9._-]+\.woff2$/', $wpc_fb193)) { continue; }
                        $wpc_fr193[$wpc_fb193] = $wpc_ur193;
                    }
                    if (!empty($wpc_fr193) && get_option('wpc_font_file_ranges193') !== $wpc_fr193) {
                        update_option('wpc_font_file_ranges193', $wpc_fr193, false);
                        if (function_exists('wpc_cache_first_log')) {
                            wpc_cache_first_log('font-file-ranges', (string) $urlKey, '', ['n' => count($wpc_fr193)]);
                        }
                    }
                    $sub = ''; $sn = 0; $wpc_sb191 = 0;
                    $wpc_bud191 = (int) apply_filters('wpc_subset_inline_budget', 49152);
                    $wpc_max191 = (int) apply_filters('wpc_subset_inline_max', 12);
                    foreach ($wpc_cand191 as $wpc_ci191) {
                        if ($sn >= $wpc_max191) { break; }
                        if ($wpc_sb191 + $wpc_ci191[0] > $wpc_bud191) { continue; }
                        $fe = $wpc_ci191[1];
                        $w = $wpc_ci191[2];
                        $fam = str_replace(["'", "\\", "\r", "\n", '<', '>'], '', (string) $fe['family']);
...
if (!function_exists('wpc_rum_census_receiver')) {
    ...
    function wpc_rum_census_receiver()
    {
        ...
        if ((isset($_GET['enc']) && $_GET['enc'] === 'gz') || substr($wpc_raw775, 0, 2) === "\x1f\x8b") {
            if (!function_exists('gzdecode')) { exit; }
            $wpc_raw775 = (string) @gzdecode($wpc_raw775);
            if ($wpc_raw775 === '' || strlen($wpc_raw775) > 524288) { exit; }
        }
        $wpc_j775 = json_decode($wpc_raw775, true);
        $wpc_v46 = (int) (is_array($wpc_j775) && isset($wpc_j775['v']) ? $wpc_j775['v'] : 0);
        if (!is_array($wpc_j775) || ($wpc_v46 !== 1 && $wpc_v46 !== 2)
        ...
        $wpc_out775 = wpc_rum_census_shape46($wpc_j775, (string) $wpc_opt775['api_key']);
        ...
        $wpc_body46 = (string) wp_json_encode($wpc_out775);
        $wpc_hdr46 = ['Content-Type' => 'application/json'];
        if ($wpc_out775['v'] === 2 && function_exists('gzencode')) {
            $wpc_gz46 = gzencode($wpc_body46, 6);
            while (is_string($wpc_gz46) && strlen($wpc_gz46) > 32768 && !empty($wpc_out775['atf_selectors'])) {
                $wpc_out775['atf_selectors'] = array_slice($wpc_out775['atf_selectors'], 0, (int) floor(count($wpc_out775['atf_selectors']) / 2));
                $wpc_out775['truncated'] = true;
                $wpc_body46 = (string) wp_json_encode($wpc_out775);
                $wpc_gz46 = gzencode($wpc_body46, 6);
            }
            if (is_string($wpc_gz46) && $wpc_gz46 !== '') {
                $wpc_body46 = $wpc_gz46;
                $wpc_hdr46['Content-Encoding'] = 'gzip';
            }
        }
    ...
    function wpc_rum_census_shape46($j, $apikey)
    {
        $v = (int) (isset($j['v']) ? $j['v'] : 1) === 2 ? 2 : 1;
        $vp = (isset($j['viewport']) && is_array($j['viewport'])) ? $j['viewport'] : [];
        $out = [
            'v' => $v,
            'apikey' => (string) $apikey,
            'url' => substr((string) $j['url'], 0, 500),
            'viewport' => [
                'w' => (int) (isset($vp['w']) ? $vp['w'] : 0),
                'h' => (int) (isset($vp['h']) ? $vp['h'] : 0),
                'dpr' => (float) (isset($vp['dpr']) ? $vp['dpr'] : 1),
            ],
            'lcp' => null,
            'atf_images' => [],
        ];
        if (isset($j['lcp']) && is_array($j['lcp'])) {
            $lr = (isset($j['lcp']['rect']) && is_array($j['lcp']['rect'])) ? $j['lcp']['rect'] : [];
            $out['lcp'] = [
                'selector' => substr((string) (isset($j['lcp']['selector']) ? $j['lcp']['selector'] : ''), 0, 120),
                'url' => (isset($j['lcp']['url']) && is_string($j['lcp']['url'])) ? substr($j['lcp']['url'], 0, 300) : null,
                'rect' => [
                    'w' => (int) (isset($lr['w']) ? $lr['w'] : 0),
                    'h' => (int) (isset($lr['h']) ? $lr['h'] : 0),
                    'x' => (int) (isset($lr['x']) ? $lr['x'] : 0),
                    'y' => (int) (isset($lr['y']) ? $lr['y'] : 0),
                ],
                't_ms' => (int) (isset($j['lcp']['t_ms']) ? $j['lcp']['t_ms'] : 0),
            ];
        }
        if (isset($j['atf_images']) && is_array($j['atf_images'])) {
            foreach (array_slice($j['atf_images'], 0, 20) as $im) {
                if (!is_array($im)) { continue; }
                $cl = [];
                if (isset($im['classes']) && is_array($im['classes'])) {
                    foreach (array_slice($im['classes'], 0, 2) as $c) {
                        if (is_string($c) && $c !== '') { $cl[] = substr($c, 0, 80); }
                    }
                }
                $out['atf_images'][] = [
                    'classes' => $cl,
                    'slot_w' => (int) (isset($im['slot_w']) ? $im['slot_w'] : 0),
                    'slot_h' => (int) (isset($im['slot_h']) ? $im['slot_h'] : 0),
                    'intrinsic_w' => (int) (isset($im['intrinsic_w']) ? $im['intrinsic_w'] : 0),
                    'current_src' => (isset($im['current_src']) && is_string($im['current_src'])) ? substr($im['current_src'], 0, 300) : '',
                    'loading' => substr((string) (isset($im['loading']) ? $im['loading'] : ''), 0, 10),
                ];
            }
        }
        $strs = function ($list, $max, $len) {
            $o = [];
            if (!is_array($list)) { return $o; }
            foreach ($list as $x) {
                if (!is_string($x) || $x === '' || preg_match('/[\x00-\x08\x0b\x0c\x0e-\x1f]/', $x)) { continue; }
                $x = substr($x, 0, $len);
                if (!in_array($x, $o, true)) { $o[] = $x; }
                if (count($o) >= $max) { break; }
            }
            return $o;
        };
        $out['fonts_used'] = $strs(isset($j['fonts_used']) ? $j['fonts_used'] : [], 64, 120);
        if ($v === 2) {
            $out['device'] = (isset($j['device']) && $j['device'] === 'mobile') ? 'mobile' : 'desktop';
            $out['atf_selectors'] = $strs(isset($j['atf_selectors']) ? $j['atf_selectors'] : [], 2000, 300);
            $out['atf_ids'] = $strs(isset($j['atf_ids']) ? $j['atf_ids'] : [], 200, 80);
            $out['truncated'] = !empty($j['truncated']);
        }
        return $out;
    }
    ...
    function wpc_font_inline_self_arm()
    {
        try {
            if (!is_admin() || wp_doing_ajax() || (defined('DOING_CRON') && DOING_CRON)
                ...
                || wpc_admin_held69('wpc_font_selfarm_lock')) {
                return;
            }
            if (!class_exists('wps_ic_fonts') && defined('WPS_IC_DIR')) {
                @include_once WPS_IC_DIR . 'addons/fonts/fonts.class.php';
            ...
            wpc_admin_hold69('wpc_font_selfarm_lock', 30 * MINUTE_IN_SECONDS); 
...
if (!function_exists('wpc_convergence_selfheal')) {
    function wpc_convergence_selfheal()
    {
        try {
            if (!is_admin() || wp_doing_ajax() || (defined('DOING_CRON') && DOING_CRON)
                || !apply_filters('wpc_convergence_selfheal', true) || wpc_admin_held69('wpc_selfheal_lock')) {
                return;
            }
            wpc_admin_hold69('wpc_selfheal_lock', HOUR_IN_SECONDS);
...
if (!function_exists('wpc_device_blind_cache_conflict')) {
    ...
    add_action('admin_init', function () {
        ...
        if (!wpc_device_blind_cache_conflict()) { wpc_state_clear81('breeze_devices'); return; }
        $fix1 = wp_nonce_url(admin_url('admin-post.php?action=wpc_devblind_fix&mode=breeze_off'), 'wpc_devblind');
        wpc_state81('breeze_devices', 'warning', __('Breeze is caching one page copy for all devices, which conflicts with per-device optimization: mobile visitors can receive desktop-optimized pages.', 'wp-compress-image-optimizer'), $fix1, __('Disable Breeze page cache', 'wp-compress-image-optimizer'));
    }, 30);
...
if (!function_exists('wpc_auto_mode_on')) {
        ...
        function wpc_slowreq_arm()
        {
            ...
            add_action('shutdown', function () {
                ...
                if (function_exists('wpc_auto_journal')) {
                    ...
                    if (($ms - $wpc_waived751) >= (int) apply_filters('wpc_render_breaker_trip_ms', 20000)
                        && class_exists('wps_cdn_rewrite') && method_exists('wps_cdn_rewrite', 'wpc_render_breaker83_trip')) {
                        wps_cdn_rewrite::wpc_render_breaker83_trip((int) ($ms - $wpc_waived751), 'slow-render');
                    }
                    wpc_auto_journal('slow-render', [
                        ...
                        'http_n'  => $h['n'],
                        'net_blocked' => isset($h['blocked']) ? (int) $h['blocked'] : 0,
                        'http_ms' => $h['ms'],
    ...
    function wpc_auto_lever_set(&$set, $lv, $val, $alsoVals = null)
    {
        ...
        if (!empty($lv['key']) && $lv['key'] === 'used-css' && (string) $val === '1'
            && function_exists('wpc_ucss_divi5_guard') && wpc_ucss_divi5_guard()) { return; }
        if (!empty($lv['nested']) && is_array($lv['nested'])) {
            $n = $lv['nested'];
    ...
    function wpc_auto_chain_maybe($src)
    {
        try {
            ...
            if ($what === '') { return; }
            if (wpc_admin_held69('wpc_auto_chain_throttle')) { return; }
            wpc_admin_hold69('wpc_auto_chain_throttle', 25);
            if (function_exists('wpc_cache_first_log')) {
    ...
    function wpc_auto_kick($what)
    {
        if (function_exists('wpc_render_guard39_active') && wpc_render_guard39_active() && function_exists('wpc_net_defer39')) {
            wpc_net_defer39('autokick:' . ($what === 'poll' ? 'poll' : 'tick'), function () use ($what) { wpc_auto_kick($what); });
            return;
        }
        ...
        try {
            ...
            if (empty($fonts) && $fu !== '') {
                ...
                if ($wpc_fc624 === 200) {
                    ...
                    wpc_fs_put($wpc_4f624, $wpc_4n624 . ':' . substr(md5($fu), 0, 8));
                    if ($wpc_4n624 >= 4) {
...
if (!function_exists('wpc_fonts_pointer_heal')) {
    ...
    function wpc_fonts_pointer_heal()
    {
        try {
            ...
            if (!get_option('wpc_font_metrics_present') && !wpc_admin_held69('wpc_fonts_scan66')) {
                wpc_admin_hold69('wpc_fonts_scan66', 10 * MINUTE_IN_SECONDS);
            ...
            if (wpc_admin_held69('wpc_fonts_heal_tick')) { return; }
            wpc_admin_hold69('wpc_fonts_heal_tick', 600);
            $wpc_fh397 = 0;
...
if (!function_exists('wpc_update_window_active')) {
    ...
    function wpc_update_window_end_handler()
    {
        ...
        try {
            ...
            if (class_exists('wps_ic_cache_integrations')) {
                wps_ic_cache_integrations::purgeAll(false, true, false, false, true, false, 'soft');
            }
    ...
    function wpc_render_armed_for_cache($buffer)
    {
        ...
        try {
            ...
            if ($critOn && !preg_match('/<style\b[^>]*id=(["\'])wpc-critical-css\1[^>]*>\s*(?!<\/style)\S/i', $buffer)) {
...
if (!function_exists('wpc_r2_probation_state')) {
    ...
    function wpc_r2_kick()
    {
        try {
            if (function_exists('wpc_render_guard39_active') && wpc_render_guard39_active() && function_exists('wpc_net_defer39')) {
                wpc_net_defer39('r2kick', function () { wpc_r2_kick(); });
                return;
            }
