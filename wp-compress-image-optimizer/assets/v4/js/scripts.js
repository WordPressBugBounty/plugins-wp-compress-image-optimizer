jQuery(document).ready(function ($) {
    ...
    // now solely owns Stop. Backend makes StopBulk actually halt the v2
    ...
    $('#optimizationTable').on('click', '.exclude_dropdown_anchor', function (e) {
        ...
        if ($(this).data('setting_name') === 'critical_css') {
            if ($(this).hasClass('page_generating')) {
                ...
                // v7.10.507 — a crit already exists, so this is a REGENERATE, not a delete. The
                // handler drops the artifact, forces a real gen past the 300s debounce, and purges
                // the page cache so the new crit actually reaches the wire.
...
jQuery(document).ready(function ($) {
    ...
    // Purge->land watchdog: the admin is present, so their polls carry collection
    // (each poll runs due cron + re-nudges dispatch) — no traffic dependency.
...
// Ported from the shipped minified build (source drift): API connectivity test button.
jQuery(document).ready(function ($) {
