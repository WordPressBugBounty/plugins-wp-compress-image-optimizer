...
function preload() {
    ...
    var styles = [].slice.call(document.querySelectorAll('[rel="wpc-stylesheet"]:not([data-wpc-dormant]),[type="wpc-stylesheet"]:not([data-wpc-dormant])'));
    ...
    styles.forEach(function (element, index) {
        ...
        if (element.id && element.id.indexOf('wpc-used-css') === 0 && element.media
            ...
            try {
                var mqI = window.matchMedia(element.media);
                var actI = function () {
                    if (mqI.matches && element.getAttribute('rel') !== 'stylesheet') {
    ...
    Promise.all(customPromiseFlag).then(function () {
        ...
        styles = [].slice.call(document.querySelectorAll('[rel="wpc-stylesheet"]:not([data-wpc-dormant]),[type="wpc-stylesheet"]:not([data-wpc-dormant])'));
        ...
        if (criticalCss) {
            try {
                if (!document.getElementById("wpc-crit-faces")) {
                    var wpcFf = String(criticalCss.textContent || "").match(/@font-face\s*\{[^}]*\}/gi);
                    if (wpcFf && wpcFf.length) {
                        var wpcFst = document.createElement("style");
                        wpcFst.id = "wpc-crit-faces";
                        wpcFst.textContent = wpcFf.join("");
                        (criticalCss.parentNode || document.head).insertBefore(wpcFst, criticalCss);
                    }
                }
            } catch (e) {}
            criticalCss.remove();
