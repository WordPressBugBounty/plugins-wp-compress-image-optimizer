jQuery(document).ready(function ($) {
    ...
    // Counter-flow debug. Enable with ?wpc_counter_debug=1 on the media page (or
    ...
    function wpcSwapCardWithAnimation(imageID, payload, hbStatus) {
        ...
        if (stateChanged) {
            ...
            // height-locked + the delta chip is now an inline flex sibling ,
            ...
            success: function (response) {
                ...
                if (response && response.success === true && response.data && response.data.html) {
                    $.each(response.data.html, function (index, payload) {
                        ...
                        // Passive Smart-Delivery tracking. If the heartbeat surfaces an image
    ...
    // Smooth count-up tween for the variant-count chip — rAF-based, monotonic, cancels any
    ...
    function wpcHeartbeatApplyChipData(imageID, payload) {
        ...
        //
        // Runs for OPTIMIZING cards too — NOT gated on compressed. The is-compressing loading
        ...
        if (payload.chip) {
            ...
            if ($chip.length) {
                ...
                if (parts.length >= 5) {
                    ...
                    // Transient chip-reveal REMOVED — the variant chip is now hidden on BOTH the
    ...
    // Proactive Smart-Delivery watch — register the first ~12 not-yet-compressed cards so the
    ...
    $('body').on('click', '.wps-ic-compress-live', function (e) {
        ...
        card.find('.wpc-ml-body').html(
            ...
            '<div class="wpc-variant-count-chip-row" style="display:none;margin-top:6px;line-height:1;">' + // hidden on the JS-built on-click "Optimizing…" card too — matches the PHP page-load card (.65); the count still updates the hidden chip for the heartbeat/?wpc_counter_debug
