<?php
/**
 * WP Compress — Instant Performance & Speed Optimization.
 * File: integrations/hosting/pressidium.php
 *
 * @package wp-compress-image-optimizer
 * @version 7.22.38
 */

if (!defined('ABSPATH')) {
    exit; 
}

class wps_ic_pressidium extends wps_ic_integrations {

    public function is_active() {
        return class_exists('Ninukis_Plugin');
    }

    public function do_checks() {
        
    }

    public function fix_setting($setting) {
        
    }

    public function add_admin_hooks() {
        return [
            'wps_ic_purge_all_cache' => [
                'callback' => 'purge_cache',
                'priority' => 10,
                'args' => 1
            ]
        ];
    }

    public function purge_cache($url_key = false) {
        if (class_exists('Ninukis_Plugin')) {
            $plugin = Ninukis_Plugin::get_instance();
            if (method_exists($plugin, 'purgeAllCaches')) {
                $plugin->purgeAllCaches();
            }
        }
    }

}
