...
class wps_ic_meta_box {
    ...
    /*
		if (get_transient('cdn' . 'Enabled') === false){
			$call = wp_remote_get(WPS_IC_KEYSURL . '?action=get_account_status_v6&apikey=' . get_option(WPS_IC_OPTIONS)['api_key'] . '&range=month&hash=' . md5(mt_rand(999, 9999)), [
				'timeout'    => 30,
				'sslverify'  => false,
		...
		$locked['advanced_cache'] = !$this->isFeatureEnabled('caching');
		$locked['adaptive'] = !$this->isFeatureEnabled('adaptive');
		$locked['critical_css'] = !$this->isFeatureEnabled('css');
		$locked['delay_js'] = !$this->isFeatureEnabled('delay-js');
