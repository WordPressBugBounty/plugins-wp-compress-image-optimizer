...
class wps_ic_ajax extends wps_ic
{
    ...
    public function wps_ic_optimize_start()
    {
        ...
        if ($apikey === '') {
            wp_send_json_error(['msg' => 'Connect the site first — no API key found.']);
        }
        ...
        if (is_wp_error($response)) {
            wp_send_json_error(['msg' => "Couldn't reach the optimizer service — " . $response->get_error_message()]);
        }
        $code = (int) wp_remote_retrieve_response_code($response);
        $data = json_decode((string) wp_remote_retrieve_body($response), true);
        if ($code === 200 && is_array($data) && !empty($data['run_id']) && (($data['status'] ?? '') === 'accepted')) {
            wp_send_json_success(['run_id' => (string) $data['run_id'], 'mode' => (string) ($data['mode'] ?? 'advisory_dry_run')]);
        }
        ...
        wp_send_json_error(['msg' => self::wpc_optimize_error_text($raw), 'raw' => $raw]);
    }
    ...
    public function wps_ic_optimize_status()
    {
        ...
        if ($run_id === '' || !preg_match('/^[A-Za-z0-9][A-Za-z0-9._:-]{2,120}$/', $run_id)) {
            wp_send_json_error(['msg' => 'Invalid run id.']);
        }
        ...
        if (is_wp_error($response)) {
            wp_send_json_error(['msg' => "Couldn't reach the optimizer — " . $response->get_error_message(), 'transient' => true]);
        }
        $code = (int) wp_remote_retrieve_response_code($response);
        $data = json_decode((string) wp_remote_retrieve_body($response), true);
        ...
        if ($code === 404 || (is_array($data) && (($data['status'] ?? '') === 'not_found'))) {
            wp_send_json_success(['state' => 'pending']);
        }
        if ($code !== 200 || !is_array($data)) {
            wp_send_json_error(['msg' => 'Optimizer returned an unexpected response (HTTP ' . $code . ').', 'transient' => true]);
        }
        wp_send_json_success($data); 
    }
