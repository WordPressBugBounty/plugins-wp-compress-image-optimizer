            ...
            if (!$hooked && function_exists('register_shutdown_function')) {
                ...
                register_shutdown_function(function () use (&$buf) {
                    try {
                        ...
                        if (function_exists('is_admin') && !is_admin() && function_exists('get_transient') && get_transient('wpc_diag_flush_lock')) {
                            return;
                        }
...
function wpc_get_local_optimized_ids()
{
    ...
    global $wpdb;
    $ids = $wpdb->get_col("SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key = 'ic_status' AND meta_value = 'compressed'");
...
function wpc_url_to_attachment_id($url)
{
    ...
    $id = (class_exists('wps_rewriteLogic') && method_exists('wps_rewriteLogic', 'wpc_att_id')) ? (int)wps_rewriteLogic::wpc_att_id($base_url) : attachment_url_to_postid($base_url);
...
function wpc_purge_cdn_urls($attachment_id)
{
    ...
    foreach ($urls_to_purge as $url) {
        wp_remote_get("https://cdn-mc.zapwp.net/health/cache-purge?apikey=" . urlencode($options['api_key']) . "&url=" . urlencode($url), ['timeout' => 5, 'blocking' => false, 'sslverify' => false]);
    }


    $site_url = site_url();
    $ladder_widths = [150, 221, 300, 400, 442, 480, 640, 720, 755, 768, 800, 960, 1100, 1132, 1200, 1280, 1366, 1440, 1510, 1536, 1600, 1800, 1887, 2048, 2560];
    $cdn_zone = '';
            ...
            foreach ($u_hosts as $u_host_for_purge) {
                ...
                foreach ([0, 1, 2] as $wp_fmt) {
                    foreach ($ladder_widths as $w) {
                        ...
                        wp_remote_get("https://cdn-mc.zapwp.net/health/cache-purge?apikey=" . urlencode($options['api_key']) . "&url=" . urlencode($transform_path), ['timeout' => 5, 'blocking' => false, 'sslverify' => false]);
                    }
                }
            }
        }
...
function wpc_purge_cdn_urls_single($attachment_id, $abs_path)
{
    ...
    error_log(sprintf('[WPC PurgeSingle] imageID=%d url=%s', (int)$attachment_id, $url));
}
...
function wpc_viewport_lazy_seed267()
{
    ...
    try {
        ...
        if (is_array($s) && !empty($s['nativeLazy']) && $s['nativeLazy'] == '1' && (empty($s['lazy']) || $s['lazy'] != '1') && apply_filters('wpc_viewport_lazy_default', true)) {
            $s['lazy'] = '1';
...
function wpc_v2_rewrite_img_to_natural_urls($img_tag, $cdn_zone, $upload_basedir, $upload_baseurl, $site_url)
{
    ...
    $attrs = [['name' => 'src', 'is_srcset' => false], ['name' => 'srcset', 'is_srcset' => true], ['name' => 'data-src', 'is_srcset' => false], ['name' => 'data-srcset', 'is_srcset' => true],];
    ...
    foreach ($attrs as $a) {
        ...
        $new_value = $a['is_srcset'] ? wpc_v2_rewrite_srcset_value($original_value, $cdn_zone, $upload_basedir, $upload_baseurl, $site_url) : wpc_v2_rewrite_single_url_to_natural($original_value, $cdn_zone, $upload_basedir, $upload_baseurl, $site_url);

        if ($new_value !== $original_value) {

            $img_tag = preg_replace('/(\b' . preg_quote($name, '/') . '\s*=\s*")' . preg_quote($original_value, '/') . '(")/i', '$1' . str_replace(['\\', '$'], ['\\\\', '\\$'], $new_value) . '$2', $img_tag, 1);
        }
    }
...
if (!function_exists('wpc_picture_apply_sizes_to_img')) {
    ...
    function wpc_picture_apply_sizes_to_img($img_tag, $sizes_value)
    {
        if (preg_match('/\ssizes\s*=\s*["\'][^"\']*["\']/i', $img_tag)) {
            return preg_replace('/\ssizes\s*=\s*["\'][^"\']*["\']/i', ' sizes="' . $sizes_value . '"', $img_tag, 1);
        }
...
function wpc_inject_picture_tags($content)
{
    ...
    $wpc_avif_ok = !class_exists('WPC_Delivery_Resolver') || WPC_Delivery_Resolver::effective_ceiling($settings) === 'avif';


    $wpc_webp_ok = !class_exists('WPC_Delivery_Resolver') || WPC_Delivery_Resolver::effective_ceiling($settings) !== 'off';


    $wpc_cdn_imgs_on = !class_exists('WPC_Negotiated_Delivery') || WPC_Negotiated_Delivery::cdn_images_enabled($settings);
    ...
    $content = preg_replace_callback('/<img\b[^>]*class="[^"]*wp-image-(\d+)[^"]*"[^>]*>/i', function ($matches) use ($optimized, $cdn_zone, $upload_basedir_for_rewrite, $upload_baseurl_for_rewrite, $site_url_for_rewrite, $settings, $wpc_avif_ok, $wpc_webp_ok) {
        $img_tag = $matches[0];
        ...
        if ($cdn_zone) {
            $img_tag = wpc_v2_rewrite_img_to_natural_urls($img_tag, $cdn_zone, $upload_basedir_for_rewrite, $upload_baseurl_for_rewrite, $site_url_for_rewrite);
        }
        ...
        foreach ($variants as $label => $data) {
            ...
            if (!defined('WPC_SKIP_PICTURE_VARIANT_VALIDATION') || !WPC_SKIP_PICTURE_VARIANT_VALIDATION) {
                ...
                if (preg_match('/-(\d+)x(\d+)\.\w+$/', $filename, $dm) && ((int)$dm[1] <= 2 || (int)$dm[2] <= 2)) {
                    continue;
                }
        ...
        if (function_exists('wpc_v2_sized_trigger_queue') && function_exists('wpc_get_theme_content_width') && !preg_match('/\b(alignfull|alignwide|wp-block-cover|elementor|brz-|brxe-|et_pb)\b/i', $img_tag)) {
            ...
            if ($pa_cap > 0) {
                ...
                $pa_targets = function_exists('wpc_v2_ideal_targets_from_sizes') ? wpc_v2_ideal_targets_from_sizes($pa_sizes, $pa_cap) : array_unique([(int)round(206 * 1.75), 412, (int)round(206 * 3), $pa_cap, (int)round($pa_cap * 1.75), $pa_cap * 2]);
                foreach ($pa_targets as $pa_t) {
                    ...
                    foreach ($pa_existing as $pa_e) {
                        if ($pa_e >= $pa_t && ($pa_e - $pa_t) / $pa_t < 0.08) {
                            $pa_near = true;
                            break;
                        }
                    }
        ...
        $lazy_enabled_for_optimistic = function_exists('wpc_v2_get_lazy_enabled') && wpc_v2_get_lazy_enabled();
        ...
        $lazy_enabled_for_uni_ladder = function_exists('wpc_v2_get_lazy_enabled') && wpc_v2_get_lazy_enabled();
        if ($lazy_enabled_for_uni_ladder && $cdn_zone && is_array($meta_for_lazy)) {
            ...
            $u_base_uni = $orig_u_url_uni !== '' ? $orig_u_url_uni : ($upload_dir['baseurl'] . '/' . $upload_subdir . '/' . basename((string)$meta_for_lazy['file']));
        ...
        if (wpc_picture_should_inject_lazy($img_tag, $settings)) {
            ...
            if (function_exists('wpc_diagnostic_log')) {
                wpc_diagnostic_log('PICTURE_LAZY', 'injected loading=lazy on non-LCP IMG id=' . (int)$matches[1]);
            }
        }
        ...
        if ($smart_lcp_sizes !== '') {
            ...
            if (function_exists('wpc_diagnostic_log')) {
                wpc_diagnostic_log('PICTURE_LCP_SIZES', 'override id=' . (int)$matches[1] . ' sizes="' . $smart_lcp_sizes . '"');
            }
        }
        ...
        $is_mobile_for_cap = class_exists('wps_ic_rewriteLogic') ? (bool)wps_ic_rewriteLogic::$isMobile : (function_exists('wp_is_mobile') && wp_is_mobile());
        $is_adaptive_for_cap = !empty($settings['generate_adaptive']) && (string)$settings['generate_adaptive'] === '1';
        if ($is_mobile_for_cap && $is_adaptive_for_cap) {
            $mob_cap_final = (int)apply_filters('wpc_mobile_srcset_cap', (int)get_option('wpc-min-mobile-width', 400), isset($img_tag) ? (string)$img_tag : '');
            if ($mob_cap_final > 0) {
...
function wpc_early_404_for_missing_upload_images()
{
    ...
    if (preg_match('/-\d+x\d+\.(?:avif|webp|jpe?g|png)$/i', $rel) && !(defined('WPC_RUNG_INTERCEPT_OFF') && WPC_RUNG_INTERCEPT_OFF)) {
        return;
    }
...
if (!function_exists('wpc_caps_store')) {
    ...
    {
        $rec = ['v' => 1, 't' => time(), 'un' => $unrestricted ? 1 : 0, 'caps' => [],];
...
class wps_ic
{
            ...
            if (version_compare($installed_version, $wpc_realv189, '<') || !empty($_GET['simulateVersionChange'])) {
                ...
                if (function_exists('register_shutdown_function')) {
                    register_shutdown_function(function () use (&$wpc_upgrade_done) {
                        ...
                        if (is_array($wpc_le) && isset($wpc_le['type']) && in_array($wpc_le['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR], true)) {
                            error_log('[WPC Upgrade] HARD-FATAL during upgrade pass — wpc_core_version left UNBUMPED, next admin load will retry: ' . $wpc_le['message'] . ' @ ' . $wpc_le['file'] . ':' . $wpc_le['line']);
                        }
                    });
                }
                ...
                try {
                    ...
                    } elseif (function_exists('wp_schedule_single_event') && function_exists('wp_next_scheduled') && !wp_next_scheduled('wpc_v2_deferred_config_sync')) {
                    ...
                    $migrateDirty = false;
                    if (is_array($migrateSettings) && !empty($migrateSettings['generate_webp']) && $migrateSettings['generate_webp'] == '1' && !isset($migrateSettings['picture_webp'])) {
                    ...
                    if (is_array($wpc_opts214) && !empty($wpc_opts214['api_key']) && function_exists('wpc_apply_link_preset') && apply_filters('wpc_upgrade_apply_preset', true)) {
                    ...
                    if (apply_filters('wpc_purge_html_on_update', true) && class_exists('wps_ic_cache') && method_exists('wps_ic_cache', 'removeHtmlCacheFiles')) {
                        try {
                    ...
                    error_log('[WPC Upgrade] CAUGHT upgrade-pass error — wpc_core_version left UNBUMPED, next admin load will retry: ' . $wpc_upgrade_err->getMessage() . ' @ ' . $wpc_upgrade_err->getFile() . ':' . $wpc_upgrade_err->getLine());
                }
            }
    ...
    public static function wpc_upgrade_remote_lane()
    {
        ...
        try {
            if (function_exists('wpc_v2_asset_mime_probe_run') && (string)get_option('wpc_v2_cf_asset_mime_ok', '') !== '1') {
                $wpc_us819 = get_option(WPS_IC_SETTINGS);
                if (is_array($wpc_us819) && !empty($wpc_us819['live-cdn']) && (string)$wpc_us819['live-cdn'] === '1' && !(function_exists('wpc_v2_zone_cdn_suppressed') && wpc_v2_zone_cdn_suppressed()) && wpc_v2_asset_mime_probe_run() && class_exists('wps_ic_cache') && method_exists('wps_ic_cache', 'removeHtmlCacheFiles')) {
                    wps_ic_cache::removeHtmlCacheFiles('all');
                    if (function_exists('wpc_cache_first_log')) {
            ...
            if (!empty($cfReassert['token']) && !empty($cfReassert['zone'])) {
                ...
                if (class_exists('WPC_CloudflareAPI')) {
                    ...
                    if (method_exists($cfReassertSdk, 'configureCF')) {
                        $cfReassertSdk->configureCF(isset($wpc_cfs217['edge-cache']) ? (string)$wpc_cfs217['edge-cache'] : 'home', !empty($wpc_cfs217['assets']) && (string)$wpc_cfs217['assets'] === '1');
                    }
                    ...
                    if ($wpc_prev217 !== '' && version_compare($wpc_prev217, apply_filters('wpc_edge_reset_below', '7.10.210'), '<') && method_exists($cfReassertSdk, 'purgeCacheAsync')) {
                    ...
                    } elseif (apply_filters('wpc_purge_cf_on_update', true) && method_exists($cfReassertSdk, 'purgeByTags')) {
                    ...
                    if (method_exists($cfReassertSdk, 'addRendererAllowRule')) {
                        try {
                            $cfReassertSdk->addRendererAllowRule($cfReassert['zone']);
                        } catch (\Throwable $e) {
                        }
                    }
                    if (method_exists($cfReassertSdk, 'addBrowserTtlRules')) {
                        try {
                            $cfReassertSdk->addBrowserTtlRules($cfReassert['zone']);
                        } catch (\Throwable $e) {
                        }
                    }
        ...
        try {
            ...
            if (is_array($wpc_set830) && !empty($wpc_set830['critical']['css']) && (string)$wpc_set830['critical']['css'] === '1' && apply_filters('wpc_upgrade_resync', true) && (string)get_option('wpc_upgrade_resync_v', '') !== $wpc_ver830) {
        ...
        error_log(sprintf('[WPC Upgrade] remote lane done [t=%dms peak=%.0fM]', (int)round((microtime(true) - (isset($_SERVER['REQUEST_TIME_FLOAT']) ? (float)$_SERVER['REQUEST_TIME_FLOAT'] : microtime(true))) * 1000), memory_get_peak_usage(true) / 1048576));
    }
    ...
    public static function check_account_status($ignore_transient = false)
    {
        ...
        if (!empty($transient_data) && $transient_data !== 'no-site-found' && !$ignore_transient && empty($_GET['refresh']) && !(defined('DOING_CRON') && DOING_CRON) && apply_filters('wpc_account_status_async', true)) {
            if (function_exists('wp_schedule_single_event') && function_exists('wp_next_scheduled') && !wp_next_scheduled('wpc_account_status_refresh')) {
        ...
        if (wp_remote_retrieve_response_code($call) == 200) {
            ...
            if (!empty($body) && $body !== 'no-site-found') {
                ...
                if (!empty($site_url) && !empty($api_url) && $api_url !== $site_url) {
                    ...
                    $logs[] = ['ts' => current_time('mysql'), 'site_url' => $site_url, 'api_url' => $api_url, 'request_uri' => isset($_SERVER['REQUEST_URI']) ? (string)$_SERVER['REQUEST_URI'] : '', 'host' => isset($_SERVER['HTTP_HOST']) ? (string)$_SERVER['HTTP_HOST'] : '',];
                ...
                if (empty($body->packageConfiguration)) {
                    ...
                    if (function_exists('wpc_caps_store')) {
                        wpc_caps_store($packageConfig, false);
                    }
    ...
    public static function wpc_account_gate_live_cdn($account_status, &$settings)
    {
        ...
        $kill = (array)apply_filters('wpc_account_status_disables_cdn', ['suspended', 'cancelled', 'canceled', 'expired', 'inactive', 'deleted', 'terminated']);
    ...
    public static function activation()
    {
        ...
        if (get_option('wpc_install_fresh') === false) {
            ...
            $wpc_af_fresh = get_option('wpc_settings_initialized') !== '1' && (!is_array($wpc_af_settings) || count($wpc_af_settings) <= 3) && get_option('wpc_link_preset_applied') === false;
        ...
        if (is_array($wpc_act_set126) && (isset($wpc_act_set126['replace-fonts']) ? $wpc_act_set126['replace-fonts'] : '') === 'local' && function_exists('wp_schedule_single_event') && function_exists('wp_next_scheduled') && !wp_next_scheduled('wpc_font_rescan')) {
    ...
    public static function checkQuotaStatus()
    {
        ...
        if (function_exists('wp_schedule_single_event') && function_exists('wp_next_scheduled') && !wp_next_scheduled('wpc_quota_status_refresh') && apply_filters('wpc_quota_status_async', true)) {
    ...
    public function init()
    {
        ...
        if (get_option('wps_ic_url_changed')) {
            add_action('admin_notices', function () {
                ...
                $message = sprintf('<strong>Error!</strong> Seems like your URL changed, please reconnect with a new apikey. <a href="%s">Reconnect</a>', esc_url($reconnect_url));
        ...
        if (!empty($_GET['fetchTest']) && sanitize_text_field($_GET['apikey']) == self::$options['api_key']) {
            ...
            if (!is_wp_error($download)) {
                ...
                if (json_last_error() === JSON_ERROR_NONE) {
                    ...
                    if (!empty($body['insights']) && (int)($body['insights']['schema_version'] ?? 0) === 1 && empty($body['insights']['error'])) {
                        update_option('wpc_psi_insights', ['data' => $body['insights'], 'schema_version' => 1, 'lastRun' => time(),], false);
                    }
        ...
        if (get_option('hide_wpcompress_plugin') && !class_exists('whtlbl_whitelabel_plugin') && !defined('WHITE_LABEL_DIR')) {
    ...
    public function fetchCritical()
    {
        if (!empty($_GET['criticalDone'])) {
            ...
            if (!is_array($wpc_cb_body)) {
                $wpc_cb_body = [];
            }
            foreach (['uuid', 'apikey', 'pageUrl', 'lcp_url', 'delay_url', 'used_css_url', 'tpl_key', 'ready'] as $wpc_ck) {
                if ((!isset($_GET[$wpc_ck]) || $_GET[$wpc_ck] === '') && isset($wpc_cb_body[$wpc_ck]) && is_scalar($wpc_cb_body[$wpc_ck]) && $wpc_cb_body[$wpc_ck] !== '') {
                    $_GET[$wpc_ck] = (string)$wpc_cb_body[$wpc_ck];
                }
            }


            if ((string)($wpc_cb_body['event'] ?? '') === 'generation_complete' && !empty($wpc_cb_body['manifest_url']) && !empty($wpc_cb_body['gen_id']) && apply_filters('wpc_manifest_webhook', true)) {
                ...
                $wpc_mexp697 = $wpc_mk697key === '' ? '' : hash_hmac('sha256', (string)$wpc_cb_body['gen_id'] . '|' . (string)($wpc_cb_body['url_key'] ?? '') . '|' . (string)($wpc_cb_body['ts'] ?? ''), $wpc_mk697key);
                $wpc_mok697 = ($wpc_mexp697 !== '' && $wpc_msig697 !== '' && hash_equals($wpc_mexp697, $wpc_msig697)) || ($wpc_mk697key !== '' && !empty($_GET['apikey']) && $wpc_mk697key === sanitize_text_field((string)$_GET['apikey']));
                if (!$wpc_mok697) {
            ...
            if (!empty($uuid) && !empty($apikey)) {
                ...
                if ($dbApiKey == $apikey) {
                    ...
                    if (defined('WPS_IC_CRITICAL') && function_exists('wpc_crit_meta_write')) {
                        ...
                        if (!is_dir($wpc_cbdir) && function_exists('wp_mkdir_p')) {
                            @wp_mkdir_p($wpc_cbdir);
                        }
                        if (!empty($wpc_cb_body['delay_inline']) && !empty($wpc_cb_body['delay']) && is_array($wpc_cb_body['delay']) && function_exists('wpc_delay_inline_fresher') && wpc_delay_inline_fresher($wpc_cbdir . 'delay.json', $wpc_cb_body['delay']) && class_exists('wps_ic_js_delay_v3') && wps_ic_js_delay_v3::wpc_delay_measured_shape($wpc_cb_body['delay']) && apply_filters('wpc_delay_inline_consume', true)) {
                            ...
                            if (is_string($wpc_dib382) && $wpc_dib382 !== '' && strlen($wpc_dib382) <= 524288) {
                                ...
                                if (function_exists('wpc_cache_first_log')) {
                                    wpc_cache_first_log('delay-inline-landed', (string)$urlKey, '', ['via' => 'criticalDone', 'bytes' => strlen($wpc_dib382)]);
                                }
                            }
                        }
                        $wpc_cblcp382 = (isset($wpc_cb_body['lcp_json']) && is_array($wpc_cb_body['lcp_json'])) ? $wpc_cb_body['lcp_json'] : ((isset($wpc_cb_body['lcp']) && is_array($wpc_cb_body['lcp'])) ? $wpc_cb_body['lcp'] : null);
                        if (!empty($wpc_cb_body['lcp_inline']) && is_array($wpc_cblcp382) && (isset($wpc_cblcp382['lcp_element']) || isset($wpc_cblcp382['hints'])) && apply_filters('wpc_lcp_inline_consume', true)) {
                            $wpc_lib382 = wp_json_encode($wpc_cblcp382);
                            if (is_string($wpc_lib382) && $wpc_lib382 !== '' && strlen($wpc_lib382) <= 524288) {
                                $wpc_oldauth383c = function_exists('wpc_lcp_first_auth') ? wpc_lcp_first_auth(json_decode((string)@file_get_contents($wpc_cbdir . 'lcp.json'), true)) : null;
                                wpc_crit_meta_write($wpc_cbdir . 'lcp.json', $wpc_lib382);
                                @unlink($wpc_cbdir . 'lcp_none.txt');

                                if ($wpc_oldauth383c !== false && function_exists('wpc_lcp_first_auth') && wpc_lcp_first_auth($wpc_cblcp382) === false && function_exists('wpc_lcp_edge_flip_purge')) {
                                ...
                                if (function_exists('wpc_cache_first_log')) {
                                    wpc_cache_first_log('lcp-inline-landed', (string)$urlKey, '', ['via' => 'criticalDone', 'bytes' => strlen($wpc_lib382)]);
                                }
                            }
                        }
                    }
                    ...
                    try {
                        ...
                        $wpc_fonts67 = (!empty($wpc_cb_body['fonts']) && is_array($wpc_cb_body['fonts'])) ? $wpc_cb_body['fonts'] : ((!empty($_GET['fonts']) && is_array(json_decode(urldecode((string)$_GET['fonts']), true))) ? json_decode(urldecode((string)$_GET['fonts']), true) : []);
                        if (!empty($wpc_fonts67) && defined('WPS_IC_FONTS_DIR') && apply_filters('wpc_fonts_artifact_consume', true)) {
                            ...
                            foreach ($wpc_fonts67 as $wpc_fe) {
                                ...
                                if (stripos($wpc_fh, 'critical-css-mc.b-cdn.net') === false || !preg_match('/^[A-Za-z0-9._-]+\.woff2$/', $wpc_fb)) {
                                    continue;
                                }
                                ...
                                if (!file_exists($wpc_dst) || (int)@filesize($wpc_dst) !== (int)($wpc_fe['bytes'] ?? -1)) {
                                    ...
                                    $wpc_fbody = (!is_wp_error($wpc_fr) && wp_remote_retrieve_response_code($wpc_fr) === 200) ? wp_remote_retrieve_body($wpc_fr) : '';
                            ...
                            if (apply_filters('wpc_atf_subset_inline', true) && defined('WPS_IC_CRITICAL') && !empty($urlKey)) {
                                ...
                                foreach ($wpc_fonts67 as $wpc_sfe) {
                                    ...
                                    if (!isset($wpc_rr_diag)) {
                                        $wpc_rr_diag = [];
                                    }
                                    $wpc_rr_diag[] = strtolower($wpc_sfam) . '|' . $wpc_swt . '|' . $wpc_sst . ' keys=' . implode(',', array_keys($wpc_sfe)) . ' ur=' . ($wpc_sur !== '' ? $wpc_sur : '(none)') . ' rr=' . ($wpc_srr !== '' ? $wpc_srr : '(NONE)');
                                    if ($wpc_srr !== '' && $wpc_sur !== '') {
                                    ...
                                    $wpc_sub_css .= "@font-face{font-family:'" . $wpc_sfam . "';font-weight:" . $wpc_swt . ';font-style:' . $wpc_sst . ';src:url(data:font/woff2;base64,' . base64_encode($wpc_sfw) . ") format('woff2');" . ($wpc_sur !== '' ? 'unicode-range:' . $wpc_sur . ';' : '') . 'font-display:block}';
                                    $wpc_sub_n++;
                                }
                                ...
                                if ($wpc_sub_css !== '') {
                                    if (!is_dir(dirname($wpc_sub_path))) {
                                        @wp_mkdir_p(dirname($wpc_sub_path));
                                    }
                                ...
                                if (!empty($wpc_rr_map) && $wpc_sub_css !== '') {
                                    if (get_option('wpc_font_remote_ranges') !== $wpc_rr_map) {
                                        ...
                                        if (function_exists('wpc_cache_first_log')) {
                                            wpc_cache_first_log('font-remote-ranges', (string)$urlKey, '', ['n' => count($wpc_rr_map), 'src' => 'core']);
                                        }
                                    }
                                }
                            }
                    ...
                    try {
                        $wpc_wire_url67 = !empty($wpc_cb_body['wire_url']) ? sanitize_url((string)$wpc_cb_body['wire_url']) : (!empty($_GET['wire_url']) ? sanitize_url(urldecode((string)$_GET['wire_url'])) : '');
                        $wpc_wire_rev67 = isset($wpc_cb_body['wire_rev']) ? (int)$wpc_cb_body['wire_rev'] : (isset($_GET['wire_rev']) ? (int)$_GET['wire_rev'] : 0);
                        $wpc_wire_sig67 = !empty($wpc_cb_body['wire_sig']) ? (string)$wpc_cb_body['wire_sig'] : (!empty($_GET['wire_sig']) ? sanitize_text_field(urldecode((string)$_GET['wire_sig'])) : '');
                    ...
                    if (apply_filters('wpc_lcp_async', true) && defined('WPS_IC_CRITICAL') && !@is_readable(rtrim(WPS_IC_CRITICAL, '/') . '/' . $urlKey . '/lcp.json') && @is_readable(rtrim(WPS_IC_CRITICAL, '/') . '/' . $urlKey . '/lcp_url.txt') && function_exists('wp_schedule_single_event') && function_exists('wp_next_scheduled') && !wp_next_scheduled('wpc_lcp_repull', [$urlKey, 1])) {
    ...
    {
        $page_builders = ['run_compress', 'run_restore', 'bwc', 'elementor-preview', 'fl_builder', 'et_fb', 'preview', 'builder', 'brizy', 'fb-edit', 'bricks', 'ct_template', 'ct_builder', 'cs-render', 'tatsu', 'trp-edit-translation', 'brizy-edit-iframe', 'ct_builder', 'livecomposer_editor', 'tatsu', 'tatsu-header', 'tatsu-footer', 'tve', 'is-editor-iframe', 'pagelayer-live'];
    ...
    public function inAdmin()
    {
        ...
        if (!empty($this::$options['api_key']) && empty($this::$zone_name) && get_option('wps_ic_allow_live') !== false && !(function_exists('wp_doing_ajax') && wp_doing_ajax()) && (time() - (int)get_option('wpc_zone_backfill_at')) > HOUR_IN_SECONDS) {
    ...
    public static function getActiveFeatures()
    {
        ...
        return ['critical_css' => !empty($settings['critical']) && !empty($settings['critical']['css']), 'delay_js' => !empty($settings['delay-js-v2']) || !empty($settings['delay-js']), 'cdn' => !empty($settings['live-cdn']) || !empty($settings['cdn']), 'lazy_load' => !empty($settings['lazy']), 'native_lazy' => !empty($settings['nativeLazy']), 'webp' => !empty($settings['generate_webp']) || !empty($settings['picture_webp']), 'avif' => !empty($settings['picture_avif']), 'minify_css' => !empty($settings['css_minify']), 'combine_css' => !empty($settings['css_combine']), 'minify_js' => !empty($settings['js_minify']), 'combine_js' => !empty($settings['js_combine']), 'defer_js' => !empty($settings['js_defer']), 'cloudflare' => !empty($cf['zone']), 'cache' => !empty($settings['cache']) && !empty($settings['cache']['advanced']), 'local_compress' => !empty($settings['local']) && !empty($settings['local']['media-library']), 'plugin_version' => self::$version,];
    }
    ...
    public function inFrontEnd()
    {
        ...
        if (!empty($_GET['apiGenerateCritical'])) {
            ...
            if (empty($wpc_agc_opts['api_key']) || $wpc_agc_key === '' || !hash_equals((string)$wpc_agc_opts['api_key'], $wpc_agc_key) || ((time() - (int)get_option('wpc_apigen_at')) < 60 && !update_option('wpc_apigen_at', time(), false))) {
...
add_action('admin_init', function () {
    ...
    if (is_array($s) && !empty($s['serve']) && is_array($s['serve'])) {
        ...
        foreach (['jpg', 'png', 'gif', 'svg'] as $k) {
            if (!empty($s['serve'][$k]) && (string)$s['serve'][$k] === '1') {
                $any = true;
                break;
            }
        }
        $v = $any ? '1' : '0';
        if ((string)($s['serve']['jpg'] ?? '') !== $v || (string)($s['serve']['png'] ?? '') !== $v || (string)($s['serve']['gif'] ?? '') !== $v || (string)($s['serve']['svg'] ?? '') !== $v) {
...
function wpc_do_cleanup_backups()
{
    ...
    $iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($backupDir, RecursiveDirectoryIterator::SKIP_DOTS), RecursiveIteratorIterator::CHILD_FIRST);
    ...
    $dirs = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($backupDir, RecursiveDirectoryIterator::SKIP_DOTS), RecursiveIteratorIterator::CHILD_FIRST);
    foreach ($dirs as $dir) {
        if ($dir->isDir()) {
...
{
    load_plugin_textdomain(WPS_IC_TEXTDOMAIN, false, dirname(plugin_basename(__FILE__)) . '/languages');
}
...
add_action('update_option_' . WPS_IC_SETTINGS, function ($old, $new) {
    ...
    foreach (['generate_webp', 'picture_avif', 'wpc_nextgen'] as $k) {
        if ((string)($old[$k] ?? '') !== (string)($new[$k] ?? '')) {
            if (function_exists('wp_schedule_single_event') && function_exists('wp_next_scheduled') && !wp_next_scheduled('wpc_sitechange_trailing')) {
    ...
    try {
        ...
        if ($isOn) {
            ...
            if ($wpc_was_ss357 && class_exists('wps_ic_cache') && method_exists('wps_ic_cache', 'cfPurgeAllHtml')) {
                try {
                    wps_ic_cache::cfPurgeAllHtml(false, true);
                } catch (\Throwable $e) {
                }
            }
        }
...
if (!function_exists('wpc_delay_v3_report_handler')) {
    function wpc_delay_v3_report_handler()
    {
        ...
        $raw = isset($_POST['payload']) ? (string)wp_unslash($_POST['payload']) : (isset($_GET['payload']) ? (string)wp_unslash($_GET['payload']) : '');
        ...
        if (!is_array($data) || ((empty($data['e']) || !is_array($data['e'])) && !$wpc_bootfail360 && !$wpc_bootretr360 && !$wpc_lcpmx440 && !$wpc_lcpok447 && !$wpc_lcptr452)) {
            wp_send_json_error('bad-payload');
        }
        ...
        if ($wpc_bootfail360 || $wpc_bootretr360) {
            ...
            if ($wpc_bootfail360 && count((array)$wpc_bf360['u']) >= 3 && !get_option('wpc_delay_aggr_off')) {
                ...
                foreach ((array)$wpc_bf360['p'] as $wpc_pp360) {
                    try {
                        if (class_exists('wps_ic_url_key') && class_exists('wps_ic_cache_integrations') && method_exists('wps_ic_cache_integrations', 'purgeUrlHtml')) {
        ...
        if ($wpc_lcptr452) {
            ...
            if (!is_array($wpc_trs452)) {
                $wpc_trs452 = [];
            }
            $wpc_num452 = function ($v) {
                return is_numeric($v) ? (int)$v : 0;
            };
            $wpc_trs452[] = ['t' => time(), 'u' => sanitize_text_field(substr((string)($data['u'] ?? ''), 0, 60)), 'fcp' => $wpc_num452($data['fcp'] ?? 0), 'lcp' => $wpc_num452($data['lcp'] ?? 0), 'gap' => $wpc_num452($data['gap'] ?? 0), 'own' => $wpc_num452($data['own'] ?? 0), 'pct' => $wpc_num452($data['pct'] ?? 0), 'v' => sanitize_text_field(substr((string)($data['v'] ?? ''), 0, 20)), 'ch' => in_array(($data['ch'] ?? ''), ['b', 'i'], true) ? (string)$data['ch'] : '?', 'hum' => isset($data['hum']) ? (int)$data['hum'] : -1, 'ltn' => $wpc_num452($data['ltn'] ?? 0), 'ltms' => $wpc_num452($data['ltms'] ?? 0), 'top' => isset($data['top']) ? (int)$data['top'] : -1, 'vh' => isset($data['vh']) ? (int)$data['vh'] : -1, 'inv' => isset($data['inv']) ? (int)$data['inv'] : -1, 'el' => sanitize_text_field(substr((string)($data['el'] ?? ''), 0, 32)), 'url' => sanitize_text_field(substr((string)($data['url'] ?? ''), 0, 48)), 'r' => is_array($data['r'] ?? null) ? array_map($wpc_num452, array_slice($data['r'], 0, 8)) : [], 'net' => is_array($data['net'] ?? null) ? array_slice($data['net'], 0, 6) : [], 'lt' => is_array($data['lt'] ?? null) ? array_slice($data['lt'], 0, 5) : [],];


            update_option('wpc_lcp_trace_reports', array_slice($wpc_trs452, -(int)apply_filters('wpc_lcp_trace_keep', 8)), false);
            if (empty($data['e'])) {
        ...
        if ($wpc_lcpok447) {
            ...
            if (!is_array($wpc_ok447)) {
                $wpc_ok447 = [];
            }
            $wpc_ok447 = ['t' => time(), 'n' => isset($wpc_ok447['n']) ? min((int)$wpc_ok447['n'] + 1, 1000000) : 1,];
        ...
        if ($wpc_lcpmx440) {
            ...
            if ($wpc_mxw440 !== '') {
                ...
                $wpc_mx440[$wpc_mxk440] = ['t' => time(), 'u' => $wpc_mxu440, 'got' => $wpc_mxg440, 'want' => $wpc_mxw440, 'n' => isset($wpc_mx440[$wpc_mxk440]['n']) ? (int)$wpc_mx440[$wpc_mxk440]['n'] + 1 : 1,];
        ...
        if (!empty($wpc_promoted) && !get_option('wpc_delay_v3_manifest_off') && apply_filters('wpc_delay_v3_autotune', true)) {
            foreach ($log as $entry) {
                if (empty($entry['f']) || (int)$entry['n'] < 2) {
                    continue;
                }
                $wpc_m = (string)$entry['m'];
                if (stripos($wpc_m, 'is not defined') === false && stripos($wpc_m, "can't find variable") === false) {
                    continue;
                }
                $wpc_fh = strtolower((string)parse_url((string)$entry['f'], PHP_URL_HOST));
                if ($wpc_fh !== '' && $wpc_strip($wpc_fh) !== $wpc_strip($wpc_sh) && strpos($wpc_fh, 'zapwp') === false && strpos($wpc_fh, 'b-cdn') === false) {
                    continue;
                }
        ...
        if (apply_filters('wpc_delay_v3_autotune', true)) {
            ...
            if (count($tuned) < 5) {
                foreach ($log as $entry) {
                    ...
                    if (preg_match('/\.\s*(?:on|each|extend|hasclass|ready|ajax|fn)\b[^a-z]{0,4}is not a function|pseudos|jquery is not|\$ is not/i', (string)$entry['m']) && !apply_filters('wpc_autotune_jqenv_ok', false, (string)$entry['f'])) {
                        continue;
                    }
                    ...
                    if ((strpos($base, 'jquery') !== false || preg_match('/-js-(after|before)$/', $base)) && !apply_filters('wpc_autotune_jquery_ok', false, $base)) {
                        continue;
                    }
    ...
    add_action('admin_notices', function () {
        ...
        echo '<div class="notice notice-info is-dismissible"><p><strong>WP Compress — JavaScript delay self-tuned:</strong> visitors repeatedly hit errors from <code>' . esc_html($base) . '</code> while it was delayed, so it was automatically added to your "Scripts to Exclude" list (Optimize JavaScript → Excludes) and the page cache was refreshed. You can remove it there any time.</p></div>';
    ...
    add_action('admin_notices', function () {
        ...
        echo '<div class="notice notice-info is-dismissible"><p><strong>WP Compress — JavaScript delay self-healed:</strong> visitors hit errors from <code>' . esc_html($wpc_pb) . '</code> after the render analysis moved it earlier in the load, so this site was automatically reverted to the standard (safe) delay behavior and the page cache was refreshed. It re-evaluates automatically after the next page analysis; nothing needs your attention.</p></div>';
...
if (!function_exists('wpc_apiv3_recover_surviving_key')) {
    function wpc_apiv3_recover_surviving_key()
    {
        ...
        foreach (['wps_ic_options', 'wps_ic_settings'] as $src) {
            $o = get_option($src);
            if (is_array($o) && !empty($o['api_key'])) {
                $key = (string)$o['api_key'];
                break;
            }
        }
        return $key;
...
add_action('admin_init', function () {
    if (get_option('wpc_apiv3_reconnect_done')) {
        return;
    }
    if (!wpc_apiv3_recover_needed()) {
        update_option('wpc_apiv3_reconnect_done', 1, false);
        return;
    }
    if (wpc_apiv3_recover_surviving_key() === '') {
        return;
    }
    if (get_transient('wpc_apiv3_reconnect_backoff')) {
        return;
    }
    if (function_exists('wp_schedule_single_event') && function_exists('wp_next_scheduled') && !wp_next_scheduled('wpc_apiv3_reconnect')) {
...
if (!function_exists('wpc_first_run_psi_now')) {
    function wpc_first_run_psi_now()
    {
        ...
        try {
            $requests = new wps_ic_requests();
            $args = ['url' => home_url(), 'uuid' => $uuid, 'hash' => $uuid, 'apikey' => $opts['api_key'], 'version' => defined('WPC_PLUGIN_VERSION') ? WPC_PLUGIN_VERSION : '', 'plugin_version' => defined('WPC_PLUGIN_VERSION') ? WPC_PLUGIN_VERSION : '',];
