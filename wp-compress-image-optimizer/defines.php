...
if (!function_exists('wpc_crit_meta_files')) {
    ...
    {
        return ['url.txt', 'uuid.txt', 'land_uuid.txt', 'dispatch_ts.txt', 'land_ts.txt', 'stale.txt', 'tpl.txt', 'used_tpl.txt', 'lcp_url.txt', 'lcp_src.txt', 'delay_url.txt', 'fonts_url.txt', 'used_css_url.txt', 'used_css_mobile_url.txt', 'used_css_desktop_url.txt', 'used_css_sheets_url.txt', 'crit_combined_url.txt', 'crit_combined_src.txt', 'font-preload.txt'];
    }
}
...
if (!function_exists('wpc_opcache_refresh')) {
    function wpc_opcache_refresh($ctx = '')
    {
        ...
        $wpc_n514 = 0;
        $wpc_cap514 = function_exists('apply_filters') ? (int)apply_filters('wpc_opcache_invalidate_cap', 1200) : 1200;
        try {
            $wpc_it514 = new RecursiveIteratorIterator(new RecursiveDirectoryIterator(WPS_IC_DIR, FilesystemIterator::SKIP_DOTS), RecursiveIteratorIterator::LEAVES_ONLY);
        ...
        if (function_exists('wpc_v2_fast404_file')) {
            $wpc_mu277 = (string)wpc_v2_fast404_file();
            if ($wpc_mu277 !== '' && @is_file($wpc_mu277)) {
                @opcache_invalidate($wpc_mu277, true);
                $wpc_n514++;
            }
        }
        return $wpc_n514;
...
if (!function_exists('wpc_diag_sleep')) {
    function wpc_diag_sleep($seconds, $ctx = '')
    {
        ...
        $wpc_cap514 = function_exists('apply_filters') ? (float)apply_filters('wpc_diag_sleep_cap_s', 3.0, $ctx) : 3.0;
        $wpc_bud514 = function_exists('apply_filters') ? (float)apply_filters('wpc_diag_sleep_budget_s', 4.0, $ctx) : 4.0;
    ...
    function wpc_worker_lock($logical, $budget_ms = null)
    {
        ...
        for (; ;) {
            ...
            if (microtime(true) >= $deadline) {
                ...
                if (function_exists('wpc_prof_mark')) {
                    wpc_prof_mark('LOCKFAIL:' . $logical, $t0);
                }
                return false;
            }
            usleep(30000);
...
if (!function_exists('wpc_css_is_icon_font')) {
    ...
    function wpc_css_is_icon_font($family)
    {
        ...
        return (bool)preg_match('/icon|awesome|fa[- 0-9]|material|dashicon|glyphicon|icomoon|ionicon|line.?awesome|themify|elegant|feather|simple\.?line|etmodules|et-?modules|divi/i', $f);
    }
}
...
if (!function_exists('wpc_ua_is_mobile')) {
    ...
    function wpc_ua_is_mobile()
    {
        ...
        if (strpos($agent, 'ipad') !== false || strpos($agent, 'tablet') !== false || strpos($agent, 'windows phone') !== false || strpos($agent, 'mobile') !== false) {
            return true;
        }
...
function wpc_variant_miss_302_belt258()
{
    try {
        if (is_admin() || !function_exists('is_404') || !is_404() || !apply_filters('wpc_variant_miss_302', true)) {
            return;
        }
        $uri = (string)($_SERVER['REQUEST_URI'] ?? '');
        $path = (string)parse_url($uri, PHP_URL_PATH);
        if (!preg_match('~^(.*/wp-content/uploads/.+)\.(avif|webp)$~i', $path, $pm) || strpos($path, '..') !== false) {
            return;
        }
        $stem = (string)$pm[1];
        $exts = [];
        $hint = strtolower((string)($_GET['src'] ?? ''));
        if (preg_match('/^(?:jpe?g|png|gif)$/', $hint)) {
            $exts[] = $hint;
        }
        foreach (['jpg', 'jpeg', 'png'] as $e258) {
            if (!in_array($e258, $exts, true)) {
                $exts[] = $e258;
            }
        }
        $wc = strpos($stem, '/wp-content/');
        if ($wc === false) {
            return;
        }
        $rel = substr($stem, $wc);
        foreach ($exts as $e258) {
            if (@is_readable(rtrim(ABSPATH, '/') . $rel . '.' . $e258)) {
                if (function_exists('wpc_cache_first_log')) {
                    wpc_cache_first_log('variant-miss-302', '', $rel . '.' . $e258, ['from' => substr($path, -60)]);
                }
                if (function_exists('nocache_headers')) {
                    nocache_headers();
                }
                wp_redirect(home_url($rel . '.' . $e258), 302);
                exit;
            }
        }
    } catch (\Throwable $e) {
...
add_action('template_redirect', 'wpc_variant_miss_302_belt258', 0);
...
function wpc_is_low_value_page()
{
    ...
    if (function_exists('is_404') && function_exists('did_action') && did_action('template_redirect') && is_404() && (!function_exists('apply_filters') || apply_filters('wpc_low_value_404', true))) {
        return true;
    }
...
function wpc_bound_socket_timeout()
{
    ...
    if (function_exists('stream_context_set_default')) {
        @stream_context_set_default(['http' => ['timeout' => $max, 'follow_location' => 0, 'max_redirects' => 0, 'ignore_errors' => true], 'https' => ['timeout' => $max, 'follow_location' => 0, 'max_redirects' => 0, 'ignore_errors' => true],]);
    }
    if (function_exists('register_shutdown_function')) {
    ...
    function wpc_purge_ledger_why()
    {
        ...
        try {
            ...
            foreach ((array)$wpc_bt as $wpc_f) {
                ...
                if (stripos($wpc_fn, 'ledger') !== false || $wpc_fn === 'wpc_purge_record' || $wpc_fn === 'wpc_purge_ledger_add') {
                    continue;
                }
...
if (!function_exists('wpc_settings_ledger_record')) {
    function wpc_settings_ledger_record($new, $old, $option = '')
    {
        try {
            ...
            $wpc_why = function_exists('wpc_purge_ledger_why') ? wpc_purge_ledger_why() : ['at' => '', 'by' => '', 'stack' => '', 'hook' => '', 'src' => '', 'ref' => ''];
            $wpc_row = ['t' => time(), 'opt' => (string)$option, 'at' => $wpc_why['at'], 'by' => $wpc_why['by'], 'hook' => $wpc_why['hook'], 'src' => $wpc_why['src'], 'ref' => $wpc_why['ref'], 'stack' => $wpc_why['stack'], 'n_old' => is_array($old) ? count($old) : -1, 'n_new' => is_array($new) ? count($new) : -1,

                'gone' => array_slice($wpc_d['gone'], 0, 40), 'changed' => array_slice($wpc_d['changed'], 0, 25), 'added' => array_slice($wpc_d['added'], 0, 25), 'verb' => $wpc_d['gone'] ? 'REPLACE' : 'set',];

            $wpc_watch = (array)apply_filters('wpc_settings_ledger_watch', ['emoji-remove', 'force-delay-jquery', 'delay_js', 'critical', 'cache', 'preset', 'mode']);
            foreach ($wpc_watch as $wpc_wk) {
                if (in_array($wpc_wk, $wpc_d['changed'], true) || in_array($wpc_wk, $wpc_d['gone'], true)) {
                    ...
                    $wpc_row['w'][$wpc_wk] = substr((string)(is_scalar($wpc_ov) ? $wpc_ov : @json_encode($wpc_ov)), 0, 24) . '=>' . substr((string)(is_scalar($wpc_nv) ? $wpc_nv : @json_encode($wpc_nv)), 0, 24);
                }
            }
            ...
            if ($wpc_d['gone'] && function_exists('wpc_cache_first_log')) {
                wpc_cache_first_log('settings-wholesale-replace', (string)$wpc_row['at'], '', ['gone' => count($wpc_d['gone']), 'by' => $wpc_row['by'], 'hook' => $wpc_row['hook'], 'src' => $wpc_row['src'],]);
            }
        } catch (\Throwable $e) {
        }
        return $new;
...
if (function_exists('add_filter') && defined('WPS_IC_SETTINGS')) {
    ...
    if (function_exists('add_action')) {
        ...
        add_action('delete_option', function ($wpc_opt596) {
            ...
            wpc_settings_ledger_record([], is_array($wpc_cur596) ? $wpc_cur596 : [], (string)$wpc_opt596 . ':delete');
        }, 10, 1);
    }
}
...
if (!function_exists('wpc_purge_tag_resolve')) {
    function wpc_purge_tag_resolve($sample)
    {
        ...
        foreach (explode(' ', $wpc_s) as $wpc_one) {
            $wpc_out[] = isset($GLOBALS['wpc_purge_tagmap'][$wpc_one]) ? $GLOBALS['wpc_purge_tagmap'][$wpc_one] . ' (' . $wpc_one . ')' : $wpc_one;
        }
...
if (!function_exists('wpc_purge_record')) {
    function wpc_purge_record($surface, $method, $scope = '', $count = 1, $ok = true, $sample = '')
    {
        try {
            ...
            $wpc_row = ['t' => time(), 'sf' => (string)$surface, 'm' => (string)$method, 'sc' => (string)$scope, 'n' => (int)$count, 'ok' => $ok ? 1 : 0, 'src' => $wpc_w['src'], 'at' => $wpc_w['at'], 'by' => substr($wpc_w['by'], 0, 48), 'hk' => $wpc_w['hook'], 'st' => $wpc_w['stack'], 'rf' => $wpc_w['ref'], 'u' => substr(wpc_purge_tag_resolve($sample), 0, 200), 'pid' => (isset($_REQUEST['post']) && is_scalar($_REQUEST['post'])) ? (int)$_REQUEST['post'] : 0,];
...
if (!function_exists('wpc_purge_ledger_report')) {
    function wpc_purge_ledger_report()
    {
        ...
        foreach ($wpc_led as $wpc_r) {
            ...
            if ($wpc_now - $wpc_t <= 3600) {
                $wpc_1h++;
            }
            if ($wpc_now - $wpc_t <= 86400) {
                $wpc_24h++;
            }
            if (empty($wpc_r['ok'])) {
                $wpc_fail++;
        ...
        arsort($wpc_sf);
        sort($wpc_gaps);
        return ['rows' => count($wpc_led), 'last_1h' => $wpc_1h, 'last_24h' => $wpc_24h, 'failed' => $wpc_fail, 'same_second' => $wpc_dupe, 'median_gap_s' => $wpc_gaps ? $wpc_gaps[intdiv(count($wpc_gaps), 2)] : 0, 'by_surface' => $wpc_sf, 'by_method' => array_slice($wpc_meth, 0, 10, true), 'by_line' => array_slice($wpc_at, 0, 10, true), 'by_caller' => array_slice($wpc_by, 0, 10, true), 'by_hook' => array_slice($wpc_hk, 0, 10, true), 'by_trigger' => $wpc_src, 'oldest' => isset($wpc_led[0]['t']) ? (int)$wpc_led[0]['t'] : 0, 'file' => wpc_purge_ledger_file(),];
    }
}
...
if (!function_exists('wpc_cdn_host600')) {
    function wpc_cdn_host600()
    {
        ...
        try {
            ...
            if (is_array($wpc_cf600) && !empty($wpc_cf600['settings']['cdn']) && $wpc_cfc600 !== '' && $wpc_ok600) {
...
if (!function_exists('wpc_url_is_low_value')) {
    function wpc_url_is_low_value($wpc_url)
    {
        ...
        if (!$wpc_low && $wpc_path !== '' && preg_match('/\.(?:pdf|zip|rar|7z|tar|gz|tgz' . '|jpe?g|png|gif|webp|avif|svg|ico|bmp|tiff?' . '|mp4|webm|mov|avi|mkv|mp3|wav|ogg|m4a' . '|css|js|mjs|map|woff2?|ttf|otf|eot' . '|docx?|xlsx?|pptx?|csv|rtf|psd|ai|eps)$/i', $wpc_path)) {
            $wpc_low = true;
        }
        ...
        if (!$wpc_low && $wpc_path !== '' && preg_match('#(?:^|/)wp-(?:admin|json|content|includes)/' . '|(?:^|/)(?:wp-login|wp-cron|wp-signup|wp-activate|wp-trackback|wp-comments-post|xmlrpc)\.php$' . '|\.(?:xml|txt|json)$#i', $wpc_path)) {
            $wpc_low = true;
        }
        if (!$wpc_low && $wpc_qs !== '') {
        ...
        try {
            if (function_exists('wp_upload_dir')) {
                ...
                if (empty($wpc_ud618['error']) && !empty($wpc_ud618['basedir'])) {
                    ...
                    if (!@is_file($wpc_new618) && defined('WPS_IC_CACHE') && @is_file(WPS_IC_CACHE . 'font-carrier.css')) {
...
if (!function_exists('wpc_font_carrier_key602')) {
    ...
    {
        $fam = preg_match('/font-family\s*:\s*["\']?([^"\';}]+)/i', $block, $m) ? strtolower(trim($m[1], " \t\"'")) : '';
        if ($fam === '') {
            return '';
        }
...
if (!function_exists('wpc_font_carrier_record602')) {
    function wpc_font_carrier_record602($css)
    {
        try {
            ...
            if (!apply_filters('wpc_font_carrier_record', true)) {
                return false;
            }
            ...
            if (class_exists('wps_cdn_rewrite') && method_exists('wps_cdn_rewrite', 'wpc_host_twin_css42')) {
                $out = (string)wps_cdn_rewrite::wpc_host_twin_css42($out);
                if ($out === '') {
                    return false;
                }
            }
...
if (!function_exists('wpc_font_carrier_needed602')) {
    ...
    {
        if (is_admin() || (function_exists('wp_doing_ajax') && wp_doing_ajax()) || (defined('REST_REQUEST') && REST_REQUEST) || (function_exists('is_feed') && is_feed())) {
            return false;
        }
...
if (!function_exists('wpc_font_preload_postpaint_tag')) {
    ...
    function wpc_font_preload_postpaint_tag($entries, $marker = '')
    {
        ...
        $wpc_late716 = false;
        try {
            if (class_exists('wps_ic_js_delay_v3') && function_exists('get_option') && defined('WPS_IC_SETTINGS') && is_callable(['wps_ic_js_delay_v3', 'wpc_delay_master_on'])) {
                $wpc_late716 = (bool)wps_ic_js_delay_v3::wpc_delay_master_on(get_option(WPS_IC_SETTINGS));
            }
        } catch (\Throwable $e) {
            $wpc_late716 = false;
        }
        $wpc_g716 = 'var g=function(){window.__wpcFPB=window.__wpcFPB||{};' . 'for(var i=0;i<u.length;i++){if(window.__wpcFPB[u[i][0]])continue;window.__wpcFPB[u[i][0]]=1;' . 'var l=document.createElement("link");l.rel="preload";l.as="font";l.type=u[i][1];' . 'l.crossOrigin="anonymous";l.href=u[i][0];document.head.appendChild(l)}};';
        if ($wpc_late716) {


            return '<script data-nodefer="1" data-wpc-fpb="1"' . $mk . '>(function(){var u=' . $json . ';' . $wpc_g716 . 'var hu=0,hf=function(){if(!hu){hu=1;setTimeout(g,50)}};' . '["pointerdown","keydown","touchstart","wheel","scroll","mousemove"].forEach(function(e){window.addEventListener(e,hf,{once:true,passive:true,capture:true})});' . '})();</script>';
        }


        return '<script data-nodefer="1" data-wpc-fpb="1"' . $mk . '>(function(){var u=' . $json . ';' . $wpc_g716 . 'requestAnimationFrame(function(){setTimeout(function(){g()},0)})})();</script>';
    }
}
...
if (!function_exists('wpc_font_carrier_emit602')) {
    function wpc_font_carrier_emit602()
    {
        static $done = false;
        try {
            if ($done || !wpc_font_carrier_needed602()) {
                return;
            }
            $done = true;
            ...
            if (class_exists('wps_cdn_rewrite') && method_exists('wps_cdn_rewrite', 'wpc_host_twin_css42')) {
                $css = (string)wps_cdn_rewrite::wpc_host_twin_css42($css);
                if ($css === '') {
                    return;
                }
            }
            ...
            $wpc_fo602 = (function_exists('get_option') && defined('WPS_IC_SETTINGS')) ? get_option(WPS_IC_SETTINGS) : [];
            $wpc_off602 = is_array($wpc_fo602) && !empty($wpc_fo602['font-display']) && strtolower((string)$wpc_fo602['font-display']) === 'off';
            $css = (string)preg_replace_callback('/@font-face\s*\{[^{}]*\}/is', function ($m) use ($wpc_off602) {
                if (stripos($m[0], 'font-display') !== false) {
                    return $m[0];
                }
                $icon = preg_match('/font-family\s*:\s*["\']?([^"\';}]+)/i', $m[0], $f) && function_exists('wpc_css_is_icon_font') && wpc_css_is_icon_font($f[1]);
                if (!$icon && $wpc_off602) {
                    return $m[0];
                }
...
if (!function_exists('wpc_host_heal32')) {
    function wpc_host_heal32()
    {
        try {
            if (!function_exists('home_url') || !function_exists('get_option') || !function_exists('update_option') || !defined('WPS_IC_FONTS_DIR')) {
                return;
            }
            if (!apply_filters('wpc_host_heal', true)) {
                return;
            }
            $wpc_h32 = strtolower((string)parse_url((string)home_url('/'), PHP_URL_HOST));
            if ($wpc_h32 === '') {
                return;
            }
            $wpc_st32 = (string)get_option('wpc_home_host32', '');
            if ($wpc_st32 === $wpc_h32) {
                return;
            }
            $wpc_hit32 = false;
            $wpc_why32 = '';
            $wpc_seen32 = get_option('wpc_home_hosts32', []);
            if (!is_array($wpc_seen32)) {
                $wpc_seen32 = [];
            }
            if ($wpc_st32 !== '') {
                if (!in_array($wpc_h32, $wpc_seen32, true) && time() - (int)get_option('wpc_host_heal_ts32', 0) > 21600) {
                    $wpc_hit32 = true;
                    $wpc_why32 = 'stamp:' . $wpc_st32;
                }
            } elseif (!(function_exists('is_multisite') && is_multisite())) {
                $wpc_scan32 = [];
                $wpc_sheets32 = @glob(rtrim(WPS_IC_FONTS_DIR, '/') . '/*/*.css');
                if (is_array($wpc_sheets32)) {
                    $wpc_scan32 = array_slice($wpc_sheets32, 0, 40);
                }
                $wpc_cf32 = function_exists('wpc_font_carrier_file602') ? (string)wpc_font_carrier_file602() : '';
                if ($wpc_cf32 !== '' && @is_file($wpc_cf32)) {
                    $wpc_scan32[] = $wpc_cf32;
                }
                foreach ($wpc_scan32 as $wpc_f32) {
                    $wpc_b32 = @file_get_contents($wpc_f32, false, null, 0, 262144);
                    if (!is_string($wpc_b32) || $wpc_b32 === '') {
                        continue;
                    }
                    if (preg_match_all('~https?://([a-z0-9.\-]+)(?::\d+)?/[^"\')\s]*cache/wp-cio-fonts/~i', $wpc_b32, $wpc_m32)) {
                        foreach ($wpc_m32[1] as $wpc_fh32) {
                            if (strtolower($wpc_fh32) !== $wpc_h32) {
                                $wpc_hit32 = true;
                                $wpc_why32 = 'bytes:' . strtolower($wpc_fh32);
                                break 2;
                            }
                        }
                    }
            ...
            if ($wpc_hit32) {
                $wpc_n32 = 0;
                $wpc_root32 = rtrim(WPS_IC_FONTS_DIR, '/');
                $wpc_dirs32 = @glob($wpc_root32 . '/*', GLOB_ONLYDIR);
                foreach (is_array($wpc_dirs32) ? $wpc_dirs32 : [] as $wpc_d32) {
                    $wpc_in32 = @glob($wpc_d32 . '/*');
                    foreach (is_array($wpc_in32) ? $wpc_in32 : [] as $wpc_i32) {
                        if (is_file($wpc_i32) && @unlink($wpc_i32)) {
                            $wpc_n32++;
                        }
                        if ($wpc_n32 > 4000) {
                            break 2;
                ...
                $wpc_loose32 = @glob($wpc_root32 . '/*');
                foreach (is_array($wpc_loose32) ? $wpc_loose32 : [] as $wpc_l32) {
                    if (is_file($wpc_l32) && @unlink($wpc_l32)) {
                        $wpc_n32++;
                    }
                }
                ...
                if (function_exists('wpc_font_carrier_file602')) {
                    $wpc_c32 = (string)wpc_font_carrier_file602();
                    if ($wpc_c32 !== '' && @is_file($wpc_c32)) {
                        @unlink($wpc_c32);
                    }
                }
                if (defined('WPS_IC_CRITICAL')) {
                    $wpc_v132 = @glob(rtrim(WPS_IC_CRITICAL, '/') . '/critical_*.css');
                    foreach (is_array($wpc_v132) ? $wpc_v132 : [] as $wpc_vf32) {
                        @unlink($wpc_vf32);
                    }
                    $wpc_uc32 = @glob(rtrim(WPS_IC_CRITICAL, '/') . '/used-css/*');
                    foreach (is_array($wpc_uc32) ? $wpc_uc32 : [] as $wpc_uf32) {
                        if (is_file($wpc_uf32)) {
                            @unlink($wpc_uf32);
                        }
                    }
                ...
                if (function_exists('wpc_cache_first_log')) {
                    wpc_cache_first_log('host-heal', $wpc_h32, '', ['why' => $wpc_why32, 'files' => $wpc_n32]);
                }
            }
            if (!in_array($wpc_h32, $wpc_seen32, true)) {
                $wpc_seen32[] = $wpc_h32;
                update_option('wpc_home_hosts32', array_slice($wpc_seen32, -8), false);
            }
            update_option('wpc_home_host32', $wpc_h32, true);
        } catch (\Throwable $e) {
        }
    }

    if (function_exists('add_action')) {
        add_action('init', 'wpc_host_heal32', 99);
    }
}
...
if (!function_exists('wpc_purge_request_allowed')) {
    function wpc_purge_request_allowed($what = '')
    {
        try {
            ...
            if ($wpc_act604 !== '' && strpos($wpc_act604, 'wpc') !== 0 && strpos($wpc_act604, 'wps_ic') !== 0 && strpos($wpc_act604, 'ic_') !== 0) {
            ...
            if (function_exists('did_action') && did_action('template_redirect')) {
                if ((function_exists('is_404') && is_404()) || (function_exists('wpc_is_low_value_page') && wpc_is_low_value_page())) {
    ...
    function wpc_used_css_provenance_ok($wpc_dir605, $wpc_cur605)
    {
        try {
            ...
            if (function_exists('wpc_cache_first_log') && !get_transient('wpc_ucssprov_log')) {
                ...
                wpc_cache_first_log('used-css-provenance-mismatch', '', '', ['used_tpl' => substr($wpc_was605, 0, 24), 'cur_tpl' => substr($wpc_cur605, 0, 24),]);
            }
            return false;
        } catch (\Throwable $e) {
            return true;
...
if (!function_exists('wpc_body_scope_guard606')) {
    function wpc_body_scope_guard606($wpc_crit606 = '', $wpc_ucss606 = true)
    {
        try {
            ...
            $wpc_css606 = '';
            $wpc_pos606 = '';
            if (is_string($wpc_crit606) && $wpc_crit606 !== '' && preg_match_all('/(?<![\w.#\[-])body\s*\{([^{}]{0,600})\}/i', $wpc_crit606, $wpc_pm606)) {
            ...
            $wpc_bg606 = '';
            if (is_string($wpc_crit606) && $wpc_crit606 !== '' && preg_match_all('/(?<![\w.#\[-])body\s*\{([^{}]{0,600})\}/i', $wpc_crit606, $wpc_bm606)) {
...
if (!function_exists('wpc_canon_same_host609')) {
    function wpc_canon_same_host609($wpc_a609, $wpc_b609)
    {
        ...
        if ($wpc_ha609 === '' || $wpc_hb609 === '') {
            return false;
        }
        $wpc_st609 = function ($wpc_h) {
            return strpos($wpc_h, 'www.') === 0 ? substr($wpc_h, 4) : $wpc_h;
        };
...
if (!function_exists('wpc_canon_url609')) {
    function wpc_canon_url609($wpc_u609)
    {
        try {
            ...
            if ($wpc_u609 === '' || !function_exists('get_option')) {
                return $wpc_u609;
            }
            if (!apply_filters('wpc_canonical_loopback', true)) {
                return $wpc_u609;
            }
            $wpc_map609 = get_option('wpc_canon_map', []);
            if (is_array($wpc_map609)) {
                $wpc_k609 = md5($wpc_u609);
                if (!empty($wpc_map609[$wpc_k609]['u']) && wpc_canon_same_host609($wpc_u609, $wpc_map609[$wpc_k609]['u'])) {
            ...
            if (!is_string($wpc_n609) || $wpc_n609 === '' || $wpc_n609 === $wpc_p609) {
                return $wpc_u609;
            }
...
if (!function_exists('wpc_canon_learn609')) {
    function wpc_canon_learn609($wpc_from609, $wpc_to609)
    {
        try {
            ...
            if ($wpc_from609 === '' || $wpc_to609 === '' || $wpc_from609 === $wpc_to609) {
                return false;
            }
            if (!function_exists('get_option') || !wpc_canon_same_host609($wpc_from609, $wpc_to609)) {
                return false;
            }
            $wpc_map609 = get_option('wpc_canon_map', []);
            if (!is_array($wpc_map609)) {
            ...
            if (isset($wpc_map609[$wpc_k609]['u']) && $wpc_map609[$wpc_k609]['u'] === $wpc_to609) {
                return false;
            }
            $wpc_now609 = time();
            ...
            if (count($wpc_map609) >= $wpc_cap609) {
                $wpc_map609 = array_slice($wpc_map609, -($wpc_cap609 - 1), null, true);
            }
...
if (!function_exists('wpc_canon_final609')) {
    ...
    {
        try {
            if (!is_array($wpc_r609) || empty($wpc_r609['http_response']) || !is_object($wpc_r609['http_response']) || !method_exists($wpc_r609['http_response'], 'get_response_object')) {
                return '';
            }
...
if (!function_exists('wpc_foreign_device_blind_cache')) {
    function wpc_foreign_device_blind_cache()
    {
        ...
        $signals = ['breeze' => defined('BREEZE_VERSION'), 'wp-rocket' => defined('WP_ROCKET_VERSION'), 'litespeed' => defined('LSCWP_V'), 'w3tc' => defined('W3TC'), 'wp-super-cache' => defined('WPCACHEHOME'), 'wp-fastest-cache' => defined('WPFC_MAIN_PATH'), 'cache-enabler' => defined('CACHE_ENABLER_VERSION'), 'comet-cache' => defined('COMET_CACHE_PLUGIN_FILE'),];
...
if (!function_exists('wpc_cf_fronted_html')) {
    ...
    function wpc_cf_fronted_html()
    {
        ...
        if ($wpc_seen801) {
            if (function_exists('get_option') && function_exists('update_option') && !get_option('wpc_cf_fronted_seen', 0)) {
...
if (!function_exists('wpc_devblind_edge')) {
    ...
    function wpc_devblind_edge()
    {
        ...
        $wpc_cfi243 = function_exists('get_option') ? get_option(defined('WPS_IC_CF') ? WPS_IC_CF : 'wps-ic-cf') : [];
        if (!(is_array($wpc_cfi243) && !empty($wpc_cfi243['token']) && !empty($wpc_cfi243['zone']))) {
            return true;
        }
        ...
        return !is_array($wpc_dk801) || empty($wpc_dk801['devkey']) || (isset($wpc_dk801['src']) ? (string)$wpc_dk801['src'] : '') !== 'readback';
    }
}
...
if (!function_exists('wpc_foreign_purge610')) {
    function wpc_foreign_purge610($wpc_arg610 = false, $wpc_ctx610 = '')
    {
        try {
            ...
            if (!apply_filters('wpc_foreign_purge_enabled', true, $wpc_ctx610)) {
                return false;
            }


            $wpc_scope610 = ($wpc_arg610 === false || $wpc_arg610 === null || $wpc_arg610 === '') ? 'site' : 'url:' . md5((string)$wpc_arg610);
            ...
            if ($wpc_win610 > 0 && function_exists('get_transient')) {
                ...
                if (get_transient($wpc_k610)) {
                    if (function_exists('wpc_cache_first_log')) {
                        wpc_cache_first_log('foreign-purge-throttled', '', '', ['scope' => $wpc_scope610, 'ctx' => (string)$wpc_ctx610, 'win' => $wpc_win610,]);
                    }
                    return false;
                }
...
if (!function_exists('wpc_dispatch_day611')) {
    function wpc_dispatch_day611()
    {
        return 'wpc_disp611_' . gmdate('Ymd');
    }
}
...
if (!function_exists('wpc_dispatch_allowed611')) {
    function wpc_dispatch_allowed611($wpc_url611, $wpc_burst611 = false)
    {
        try {
            ...
            if (!apply_filters('wpc_dispatch_budget', true, $wpc_url611)) {
                return true;
            }
    ...
    function wpc_dispatch_note611($wpc_url611, $wpc_code611, $wpc_body611 = '', $wpc_hdr611 = [])
    {
        ...
        try {
            ...
            if ($wpc_code611 === 400 || strpos($wpc_b611, 'unrenderable_url') !== false) {
                ...
                $wpc_out611['drop'] = true;
                $wpc_out611['retry'] = false;
                $wpc_out611['class'] = 'unrenderable';
                if (function_exists('get_option')) {
                    $wpc_d611 = get_option('wpc_disp_dead611', []);
                    if (!is_array($wpc_d611)) {
                        $wpc_d611 = [];
                    }
                    if (count($wpc_d611) > 500) {
                        $wpc_d611 = array_slice($wpc_d611, -400, null, true);
                    }
            ...
            } elseif (strpos($wpc_b611, 'fetch_blocked') !== false || strpos($wpc_b611, 'css_stub') !== false || strpos($wpc_b611, 'css_empty') !== false) {
                $wpc_out611['site_fault'] = true;
                $wpc_out611['cool_s'] = $wpc_ra611 > 0 ? $wpc_ra611 : 300;
                $wpc_out611['class'] = 'origin';
            } elseif (strpos($wpc_b611, 'server_error') !== false || strpos($wpc_b611, 'generation_failed') !== false || $wpc_code611 >= 500) {
            ...
            wpc_dispatch_log611('dispatch-note', (string)$wpc_url611, ['code' => $wpc_code611, 'class' => $wpc_out611['class'], 'drop' => $wpc_out611['drop'] ? 1 : 0, 'cool' => $wpc_out611['cool_s'], 'site_fault' => $wpc_out611['site_fault'] ? 1 : 0,]);
            return $wpc_out611;
        } catch (\Throwable $e) {
            return $wpc_out611;
        }
...
if (!function_exists('wpc_lane_lib613')) {
    ...
    function wpc_lane_lib613($wpc_h613)
    {
        ...
        $wpc_libs613 = (array)apply_filters('wpc_lane_libraries', ['jquery', 'jquery-core', 'jquery-migrate', 'jquery-ui-core', 'underscore', 'backbone', 'wp-polyfill', 'wp-hooks', 'wp-i18n', 'wp-util', 'lodash', 'moment', 'react', 'react-dom',]);
        foreach ($wpc_libs613 as $wpc_l613) {
            if ($wpc_h613 === strtolower((string)$wpc_l613)) {
                return true;
            }
        }
...
if (!function_exists('wpc_lane_split_detect613')) {
    function wpc_lane_split_detect613($wpc_html613)
    {
        try {
            ...
            $wpc_warm613 = !empty($_SERVER['HTTP_X_WPC_CACHE_WARM']) || (defined('DOING_CRON') && DOING_CRON);
            if (!$wpc_warm613) {
                return;
            }
            if (!apply_filters('wpc_lane_split_detect', true)) {
                return;
            }
            if (!is_string($wpc_html613) || strlen($wpc_html613) < 1024) {
                return;
            }
            if (!function_exists('get_transient') || get_transient('wpc_lane_split613')) {
                return;
            }
            ...
            if (!is_object($wpc_reg613) || empty($wpc_reg613->registered)) {
                return;
            }
            ...
            if (preg_match_all('/"id"\s*:\s*"([A-Za-z0-9_\-]+)"/', $wpc_html613, $wpc_dm613)) {
                foreach ($wpc_dm613[1] as $wpc_di613) {
                    $wpc_del613[wpc_lane_handle613($wpc_di613)] = 1;
                }
            }
            ...
            if (preg_match_all('/<script\b[^>]*>/i', $wpc_html613, $wpc_tg615)) {
                foreach ($wpc_tg615[0] as $wpc_t615) {
                    if (stripos($wpc_t615, ' src=') === false) {
                        continue;
                    }
                    if (stripos($wpc_t615, 'data-wpc-defer') !== false) {
                        continue;
                    }
                    if (stripos($wpc_t615, 'wpc-delay') !== false) {
                        continue;
                    }
                    if (!preg_match('/\bid=["\']([A-Za-z0-9_\-]+)["\']/i', $wpc_t615, $wpc_im615)) {
                        continue;
                    }
            ...
            foreach (array_keys($wpc_del613) as $wpc_dh613) {
                unset($wpc_syn613[$wpc_dh613]);
            }
            if (empty($wpc_del613) || empty($wpc_syn613)) {
                return;
            }
            ...
            foreach ($wpc_reg613->registered as $wpc_h613 => $wpc_o613) {
                $wpc_deps613 = (is_object($wpc_o613) && !empty($wpc_o613->deps) && is_array($wpc_o613->deps)) ? $wpc_o613->deps : [];
                if (empty($wpc_deps613)) {
                    continue;
                }
                foreach ($wpc_deps613 as $wpc_d613) {

                    if (isset($wpc_del613[$wpc_h613]) && isset($wpc_syn613[$wpc_d613]) && !wpc_lane_lib613($wpc_d613)) {
            ...
            if (empty($wpc_quiet613) && empty($wpc_hard613)) {
                return;
            }
            if (function_exists('wpc_cache_first_log')) {
                wpc_cache_first_log('lane-split', '', (string)($_SERVER['REQUEST_URI'] ?? ''), ['quiet' => count($wpc_quiet613), 'hard' => count($wpc_hard613), 'q' => substr(implode(',', array_slice(array_unique($wpc_quiet613), 0, 6)), 0, 200), 'h' => substr(implode(',', array_slice(array_unique($wpc_hard613), 0, 6)), 0, 200),]);
            }
        } catch (\Throwable $e) {
        }
    }
...
if (!function_exists('wpc_loop_grid_tokens16')) {
    function wpc_loop_grid_tokens16($html)
    {
        ...
        return (preg_match_all('/<(?:article|li|div)\b[^>]{0,300}class="[^"]*(?:loop-entry|entry-list-item|post-grid-item|kb-query-item)[^"]*"/i', $wpc_scan16, $wpc_lm16) && count($wpc_lm16[0]) >= 2) ? $wpc_found16 : [];
    }
}
...
if (!function_exists('wpc_crit_sanity_mark617')) {
    function wpc_crit_sanity_mark617($dir, $critBytes, $ids = [])
    {
        ...
        @file_put_contents(rtrim((string)$dir, '/') . '/sanity_bad.txt', md5((string)$critBytes) . "\n" . 'ids=' . implode(',', array_map('strval', (array)$ids)) . ';ts=' . time());
    }
}
...
if (!function_exists('wpc_ucss_sanity_mark617')) {
    function wpc_ucss_sanity_mark617($dir, $path)
    {
        ...
        @file_put_contents(rtrim((string)$dir, '/') . '/ucss_sanity_bad.txt', (int)@filesize($path) . ':' . (int)@filemtime($path) . ':' . basename((string)$path));
    }
}
if (!function_exists('wpc_ucss_sanity_bad617')) {
    function wpc_ucss_sanity_bad617($dir, $path)
    {
        ...
        return $wpc_m617 !== '' && $wpc_m617 === (int)@filesize($path) . ':' . (int)@filemtime($path) . ':' . basename((string)$path);
    }
}
...
if (!function_exists('wpc_font_carrier_seed619')) {
    function wpc_font_carrier_seed619()
    {
        try {
            ...
            if ($wpc_store619 === '' || (@is_file($wpc_store619) && (int)@filemtime($wpc_store619) >= $wpc_new619)) {
                return;
            }
            $wpc_blocks619 = '';
            foreach ($wpc_srcs619 as $wpc_s619) {
...
if (!function_exists('wpc_sanity_escalate622')) {
    function wpc_sanity_escalate622($args, $url)
    {
        try {
            ...
            $wpc_ucbad625 = @is_file($wpc_dir622 . 'ucss_ok.txt') && strpos((string)@file_get_contents($wpc_dir622 . 'ucss_ok.txt'), 'bad:') === 0;
...
if (!function_exists('wpc_pic_scan_css631')) {
        ...
        try {
            ...
            while ($off < $n && $blocks < 6000) {
                ...
                foreach (explode(',', $pre) as $sel) {
                    $sel = trim($sel);
                    if ($sel === '' || strlen($sel) > 240 || stripos($sel, 'picture') !== false || stripos($sel, ':has(') !== false) {
                        continue;
                    }
    ...
    function wpc_pic_scan_page631($html)
    {
        ...
        try {
            ...
            $wpc_sb632 = preg_match_all('#<style\b([^>]*)>([^<]*(?:<(?!/style)[^<]*)*)</style>#i', (string)$html, $wpc_sm632, PREG_SET_ORDER);
...
if (!function_exists('wpc_store_touch642')) {
    ...
    {
        try {
            if (is_string($wpc_p642) && $wpc_p642 !== '' && @is_file($wpc_p642) && (int)@filemtime($wpc_p642) < time() - 86400) {
                @touch($wpc_p642);
            }
        } catch (\Throwable $e) {
        }
...
if (!function_exists('wpc_perf_debug_token742')) {
    ...
    function wpc_perf_debug_token742($wpc_mint742 = true)
    {
        ...
        if (function_exists('random_bytes')) {
            try {
                $wpc_t742 = bin2hex(random_bytes(16));
            } catch (\Throwable $e) {
                $wpc_t742 = '';
            }
        }
...
if (!function_exists('wpc_uploads_harden649')) {
    function wpc_uploads_harden649()
    {
        try {
            ...
            $lines = ['<Files ~ "\.(php|php3|php4|php5|php7|php8|phtml|phar)$">', '  <IfModule mod_authz_core.c>', '    Require all denied', '  </IfModule>', '  <IfModule !mod_authz_core.c>', '    Order allow,deny', '    Deny from all', '  </IfModule>', '</Files>',];
            if (!@file_exists($file)) {
...
if (!function_exists('wpc_svg_inline_data718')) {
    ...
    function wpc_svg_inline_data718($url)
    {
        ...
        try {
            ...
            if (substr(strtolower($p), -4) !== '.svg' || (strpos($p, '/wp-content/') !== 0 && strpos($p, '/wp-includes/') !== 0)) {
                return '';
            }
            if (!defined('ABSPATH')) {
                return '';
            ...
            if ($b === '' || stripos($b, '<svg') === false || stripos($b, '<script') !== false || stripos($b, 'onload=') !== false || stripos($b, 'javascript:') !== false) {
                return '';
            }
...
if (!function_exists('wpc_legacy_levers')) {
    ...
    {
        return ['remove-srcset' => 'Remove srcset', 'add-image-sizes' => 'Add Image Sizes', 'force-natural' => 'Force Natural URLs', 'fold-split' => 'Fold Split', 'emit-src-hints-always' => 'Always Emit Source Hints', 'disable-trigger-dom-event' => 'Disable onLoad Event', 'force-delay-captcha' => 'Force Delay reCAPTCHA (test)', 'force-delay-jquery' => 'Force Delay jQuery (test)', 'minimal-mobile-css' => 'Minimal Mobile CSS (Beta)', 'maximum-mobile' => 'Maximum Optimization — Interaction-Only (Beta)',];
    }
}
...
if (!function_exists('wpc_delay_v3_autoheal54')) {
    function wpc_delay_v3_autoheal54()
    {
        if (!function_exists('get_option') || !defined('WPS_IC_SETTINGS')) {
            return;
        }
        $wpc_s54 = get_option(WPS_IC_SETTINGS);
        if (!is_array($wpc_s54) || !isset($wpc_s54['delay-js-v2']) || (string)$wpc_s54['delay-js-v2'] !== '1') {
            return;
        }
        if (!isset($wpc_s54['delay-js-v3']) || (string)$wpc_s54['delay-js-v3'] !== '0') {
            return;
        }
        if (function_exists('apply_filters') && !apply_filters('wpc_delay_v3_auto', true)) {
            return;
        }
        $wpc_s54['delay-js-v3'] = '1';
        update_option(WPS_IC_SETTINGS, $wpc_s54);
        if (function_exists('wpc_cache_first_log')) {
    ...
    if (function_exists('add_action')) {
        add_action('init', 'wpc_delay_v3_autoheal54', 4);
    }
}


if (!function_exists('wpc_delay_v3_excl_seed55')) {
    function wpc_delay_v3_excl_seed55()
    {
        if (!function_exists('get_option')) {
            return;
        }
        $wpc_x55 = get_option('wpc-excludes');
        if (!is_array($wpc_x55) || array_key_exists('delay_js_v3', $wpc_x55)) {
            return;
        }
        if (function_exists('apply_filters') && !apply_filters('wpc_delay_v3_excl_seed', true)) {
            return;
        }
        $wpc_x55['delay_js_v3'] = (isset($wpc_x55['delay_js_v2']) && is_array($wpc_x55['delay_js_v2'])) ? array_values($wpc_x55['delay_js_v2']) : array();
        update_option('wpc-excludes', $wpc_x55);
        if (function_exists('wpc_cache_first_log')) {
            wpc_cache_first_log('delay-v3-excl-seed', '', '', array('n' => count($wpc_x55['delay_js_v3'])));
        }
    }

    if (function_exists('add_action')) {
        add_action('init', 'wpc_delay_v3_excl_seed55', 4);
    }
}
...
if (!function_exists('wpc_strip_livefaces_fams329')) {
    ...
    function wpc_strip_livefaces_fams329($facesCss, $doc)
    {
        try {
            if (!is_string($facesCss) || $facesCss === '' || !is_string($doc) || stripos($facesCss, '@font-face') === false || stripos($doc, 'wpc-live-faces') === false || !apply_filters('wpc_livefaces_strip', true)) {
                return $facesCss;
            }
            $wpc_fams329 = [];
            if (preg_match_all('/<style\b[^>]*wpc-live-faces[^>]*>(.*?)<\/style>/is', $doc, $wpc_bm329)) {
                foreach ($wpc_bm329[1] as $wpc_blk329) {
                    if (preg_match_all('/@font-face\s*\{[^{}]*font-family\s*:\s*["\']?([^"\';}]+)/is', $wpc_blk329, $wpc_fm329)) {
                        foreach ($wpc_fm329[1] as $wpc_f329) {
                            $wpc_fams329[strtolower(trim($wpc_f329))] = 1;
                        }
                    }
                }
            }
            if (empty($wpc_fams329)) {
                return $facesCss;
            }
            $wpc_out329 = preg_replace_callback('/@font-face\s*\{[^{}]*\}/is', function ($m) use ($wpc_fams329) {
                if (preg_match('/font-family\s*:\s*["\']?([^"\';}]+)/i', $m[0], $f) && !empty($wpc_fams329[strtolower(trim($f[1]))])) {
                    return '';
                }


                return (string)preg_replace('/font-display\s*:\s*optional/i', 'font-display:swap', $m[0]);
            }, $facesCss);
            return is_string($wpc_out329) ? $wpc_out329 : $facesCss;
        } catch (\Throwable $e) {
            return $facesCss;
        }
    }
...
if (!function_exists('wpc_ucss_divi5_guard')) {
    ...
    {
        static $wpc_g321 = null;
        if ($wpc_g321 === null) {
            $wpc_g321 = false;
            try {
                if (function_exists('wp_get_theme') && function_exists('get_template')) {
                    $wpc_t321 = wp_get_theme(get_template());
                    if ($wpc_t321 && stripos((string)$wpc_t321->get('Name'), 'divi') !== false && (int)$wpc_t321->get('Version') >= 5) {
                        $wpc_g321 = true;
                    }
                }
            } catch (\Throwable $e) {
        ...
        return (bool)apply_filters('wpc_ucss_divi5_guard', $wpc_g321);
    }

    if (function_exists('add_action')) {
        add_action('after_switch_theme', function () {
            try {
                if (!wpc_ucss_divi5_guard()) {
                    return;
                }
                $wpc_s321 = get_option(WPS_IC_SETTINGS);
                if (is_array($wpc_s321) && !empty($wpc_s321['used-css']) && $wpc_s321['used-css'] == '1' && function_exists('wpc_r2_rollback')) {
...
if (!function_exists('wpc_doctrine_reconcile66')) {
    function wpc_ucss_invalidate313()
    {
        if (!defined('WPS_IC_CRITICAL')) {
            return 0;
        }
        $wpc_n313 = 0;
        $wpc_root313 = rtrim(WPS_IC_CRITICAL, '/');
        foreach ((array)@glob($wpc_root313 . '/used-css/*') as $wpc_f313) {
            if (is_file($wpc_f313) && @unlink($wpc_f313)) {
                $wpc_n313++;
            }
        }
        foreach ((array)@glob($wpc_root313 . '/*/used_css*_url.txt') as $wpc_f313) {
            if (@unlink($wpc_f313)) {
                $wpc_n313++;
            }
        }
        if (function_exists('wpc_cache_first_log')) {
            wpc_cache_first_log('ucss-invalidate', (string)$wpc_n313, '', []);
        }
        return $wpc_n313;
    }

    function wpc_doctrine_reconcile66($wpc_force66 = false)
    {
        $wpc_did66 = [];
        ...
        if (function_exists('wpc_delay_v3_autoheal54')) {
            wpc_delay_v3_autoheal54();
            $wpc_did66[] = 'v3-pair';
        }


        if (function_exists('wpc_pillar_rider_heal215')) {
            $wpc_prh215 = wpc_pillar_rider_heal215();
            if ($wpc_prh215 > 0) {
                $wpc_did66[] = 'pillar-riders:' . $wpc_prh215;


                if (function_exists('wpc_css_host_twin_heal231') && defined('WPS_IC_CRITICAL') && get_option('wpc_twin_sweep_v') !== WPC_PLUGIN_VERSION) {
                    $wpc_th231 = 0;
                    $wpc_tf231 = array_merge((array)glob(rtrim(WPS_IC_CRITICAL, '/') . '/used-css/*.css'), (array)glob(rtrim(WPS_IC_CRITICAL, '/') . '/*/critical_*.css'));
                    foreach (array_slice($wpc_tf231, 0, 400) as $wpc_tp231) {
                        $wpc_tb231 = (string)@file_get_contents($wpc_tp231);
                        if ($wpc_tb231 === '') {
                            continue;
                        }
                        $wpc_tn231 = wpc_css_host_twin_heal231($wpc_tb231);
                        if (is_string($wpc_tn231) && $wpc_tn231 !== $wpc_tb231 && @file_put_contents($wpc_tp231 . '.tmp', $wpc_tn231, LOCK_EX) !== false && @rename($wpc_tp231 . '.tmp', $wpc_tp231)) {
                            $wpc_th231++;
                        }
                    }
                    if ($wpc_th231 > 0) {
                        $wpc_did66[] = 'host-twin-sweep:' . $wpc_th231;
                    }
                    update_option('wpc_twin_sweep_v', WPC_PLUGIN_VERSION, false);
                }
            }
        }
        if (function_exists('wpc_delay_v3_excl_seed55')) {
            wpc_delay_v3_excl_seed55();
            $wpc_did66[] = 'excl-seed';
        }
        if (function_exists('wpc_delay_aggr_rearm')) {
            wpc_delay_aggr_rearm();
            $wpc_did66[] = 'aggr-rearm';
        }


        $wpc_as143 = get_option('wpc_autosafe102');
        if (get_option('wpc_css_passthrough') === '1' && is_array($wpc_as143) && !empty($wpc_as143['armed_v']) && get_option('wpc_ff_sentinel', '0') === '1' && defined('WPC_PLUGIN_VERSION') && ($wpc_as143['probe_v'] ?? '') !== WPC_PLUGIN_VERSION && apply_filters('wpc_autosafe_probation', true)) {
            $wpc_as143['probe_v'] = WPC_PLUGIN_VERSION;
            update_option('wpc_autosafe102', $wpc_as143, false);
            update_option('wpc_css_passthrough', '0', false);
            ...
            $wpc_did66[] = 'autosafe-probation';
        }


        if (function_exists('wpc_ucss_divi5_guard') && wpc_ucss_divi5_guard()) {
            $wpc_s321g = get_option(WPS_IC_SETTINGS);
            if (is_array($wpc_s321g) && !empty($wpc_s321g['used-css']) && $wpc_s321g['used-css'] == '1') {
                if (function_exists('wpc_r2_rollback')) {
                    wpc_r2_rollback('divi5-store-guard');
                } else {
                    $wpc_s321g['used-css'] = '0';
                    update_option(WPS_IC_SETTINGS, $wpc_s321g);
                    if (class_exists('wps_ic_cache_integrations') && method_exists('wps_ic_cache_integrations', 'purgeAll')) {
                        wps_ic_cache_integrations::purgeAll(false, true, false, false, true);
                    }
                }
                $wpc_did66[] = 'divi5-ucss-guard';
            }
        }
        if ($wpc_force66) {
            ...
            $wpc_did66[] = 'kill-switches-cleared';
            if (function_exists('wpc_ucss_invalidate313')) {
                $wpc_did66[] = 'ucss-invalidated:' . wpc_ucss_invalidate313();
            }


            if (function_exists('wpc_crit_soft_purge_all') && function_exists('wpc_repull_kick_now')) {
                $wpc_sn160 = (int)wpc_crit_soft_purge_all();
                wpc_repull_kick_now('');
                $wpc_did66[] = 'crit-stale-marked:' . $wpc_sn160;


                if (function_exists('do_action') && function_exists('home_url') && class_exists('wps_ic_url_key')) {
                    $wpc_hk166 = ltrim((string)(new wps_ic_url_key())->setup(home_url('/')), '/');
                    if ($wpc_hk166 !== '') {
                        do_action('wpc_crit_redispatch', $wpc_hk166, 0);
                        $wpc_did66[] = 'home-gen-inline';
                    }
                }
            }
            if (class_exists('wps_ic_cache_integrations') && method_exists('wps_ic_cache_integrations', 'purgeAll')) {
                wps_ic_cache_integrations::purgeAll(false, true, false, true, true);
                $wpc_did66[] = 'purge-forced';
            }


            if (defined('WPS_IC_CF') && apply_filters('wpc_cf_skip_reassert', true)) {
                $wpc_cf189 = get_option(WPS_IC_CF);
                if (is_array($wpc_cf189) && !empty($wpc_cf189['token']) && !empty($wpc_cf189['zone'])) {
                    ...
                    if (class_exists('WPC_CloudflareAPI')) {
                        try {
                            $wpc_sdk189 = new WPC_CloudflareAPI($wpc_cf189['token']);
                            if (method_exists($wpc_sdk189, 'addCdnBypassRule')) {
                                $wpc_sdk189->addCdnBypassRule($wpc_cf189['zone']);
                                $wpc_did66[] = 'cf-skip-reasserted';
                            }
                        } catch (\Throwable $e) {
                        }
                    }
        ...
        if (function_exists('wpc_cache_first_log')) {
            wpc_cache_first_log('doctrine-reconcile', $wpc_force66 ? 'force' : 'auto', '', ['did' => implode(',', $wpc_did66)]);
        }
        return $wpc_did66;
    }

    function wpc_doctrine_stamp66()
    {
        if (!defined('WPC_PLUGIN_VERSION') || !function_exists('get_option')) {
            return;
        }
        if (get_option('wpc_doctrine_ver') === WPC_PLUGIN_VERSION) {
            return;
        }
        wpc_doctrine_reconcile66(false);


        if (get_option('wpc_doctrine_ver') !== false && apply_filters('wpc_update_purges_html', true) && class_exists('wps_ic_cache_integrations') && method_exists('wps_ic_cache_integrations', 'purgeAll')) {
            wps_ic_cache_integrations::purgeAll(false, true, false, false, true);
            if (function_exists('wpc_cache_first_log')) {
                wpc_cache_first_log('update-html-purge', (string)get_option('wpc_doctrine_ver'), '', ['to' => WPC_PLUGIN_VERSION]);
            }
        }
        update_option('wpc_doctrine_ver', WPC_PLUGIN_VERSION, false);
    }

    if (function_exists('add_action')) {
        add_action('init', 'wpc_doctrine_stamp66', 6);
    }


    function wpc_disable_emoji233()
    {
        if (is_admin() || !apply_filters('wpc_disable_emoji', true)) {
            return;
        }
        $s233 = function_exists('get_option') && defined('WPS_IC_SETTINGS') ? get_option(WPS_IC_SETTINGS) : [];
        $on233 = class_exists('wps_ic_js_delay_v3') && method_exists('wps_ic_js_delay_v3', 'wpc_delay_master_on') ? (bool)wps_ic_js_delay_v3::wpc_delay_master_on(is_array($s233) ? $s233 : []) : (is_array($s233) && !empty($s233['delay-js']) && $s233['delay-js'] == '1');
        if (!$on233) {
            return;
        }
        ...
        add_filter('wp_resource_hints', 'wpc_emoji_hints233', 10, 2);
    }

    function wpc_emoji_hints233($urls, $relation)
    {
        if ($relation === 'dns-prefetch' && is_array($urls)) {
            foreach ($urls as $k233 => $u233) {
                if (is_string($u233) && strpos($u233, 's.w.org') !== false) {
                    unset($urls[$k233]);
                }
            }
        }
    ...
    if (function_exists('add_action')) {
        add_action('init', 'wpc_disable_emoji233', 5);
    }
    function wpc_refresh_auto_mode_handler66()
    {
        ...
        wp_send_json_success(['did' => wpc_doctrine_reconcile66(true)]);
    }

    if (function_exists('add_action')) {
        add_action('wp_ajax_wpc_refresh_auto_mode', 'wpc_refresh_auto_mode_handler66');
    }


    function wpc_update_auto_refresh187()
    {
        if (!defined('WPC_PLUGIN_VERSION') || !apply_filters('wpc_update_auto_refresh', !defined('WPC_DISABLE_AUTO_REFRESH'))) {
            return;
        }
        if (get_option('wpc_autorefresh_ver') === WPC_PLUGIN_VERSION) {
            return;
        }
        if (!current_user_can('manage_options') && !current_user_can('manage_wpc_settings')) {
            return;
        }
        if (get_transient('wpc_autorefresh_lock187')) {
            return;
        }
        set_transient('wpc_autorefresh_lock187', 1, 60);
        update_option('wpc_autorefresh_ver', WPC_PLUGIN_VERSION, false);
        $wpc_did187 = wpc_doctrine_reconcile66(true);
        if (function_exists('wpc_cache_first_log')) {
            wpc_cache_first_log('update-auto-refresh', WPC_PLUGIN_VERSION, '', ['did' => implode(',', (array)$wpc_did187)]);
        }
    }

    if (function_exists('add_action')) {
        add_action('admin_init', 'wpc_update_auto_refresh187', 20);
    }
}

if (!function_exists('wpc_li_debug_receipt669')) {


    function wpc_li_debug_receipt669()
    {
        if (empty($_GET['wpc_li_debug']) || !function_exists('is_user_logged_in') || !is_user_logged_in() || !function_exists('current_user_can') || (!current_user_can('manage_options') && !current_user_can('manage_wpc_settings'))) {
            return;
        }
        $s = function_exists('get_option') && defined('WPS_IC_SETTINGS') ? get_option(WPS_IC_SETTINGS) : [];
        $s = is_array($s) ? $s : [];
        $gate = isset($GLOBALS['wpc_li_gate669']) ? (string)$GLOBALS['wpc_li_gate669'] : 'none';
        $bits = ['ver=' . (defined('WPC_PLUGIN_VERSION') ? WPC_PLUGIN_VERSION : '?'), 'gate=' . $gate, 'bypass_filter=' . (function_exists('apply_filters') && (bool)apply_filters('wpc_logged_in_bypass', true) ? '1' : '0'), 'cache_serve=' . (isset($GLOBALS['wpc_cc_skip']) ? (string)$GLOBALS['wpc_cc_skip'] : 'n/a'), 'live-cdn=' . (isset($s['live-cdn']) ? (string)$s['live-cdn'] : 'unset'), 'used-css=' . (isset($s['used-css']) ? (string)$s['used-css'] : 'unset'), 'crit=' . (isset($s['critical']['css']) ? (string)$s['critical']['css'] : 'unset'), 'delay-v3=' . (isset($s['delay-js-v3']) ? (string)$s['delay-js-v3'] : 'unset'), 'cache.advanced=' . (isset($s['cache']['advanced']) ? (string)$s['cache']['advanced'] : 'unset'), 'replace-fonts=' . (isset($s['replace-fonts']) ? (string)$s['replace-fonts'] : 'unset'), 'lane=' . (string)get_option('wpc_delivery_lane', 'cdn'), 'css-passthrough=' . (string)get_option('wpc_css_passthrough', '0') . (is_array($wpc_as669 = get_option('wpc_autosafe102')) && !empty($wpc_as669['armed_ts']) ? ' auto-safe-armed=' . gmdate('Y-m-d H:i', (int)$wpc_as669['armed_ts']) . ' hits=' . count((array)($wpc_as669['hits'] ?? [])) : ' auto-safe=watching'),];
        if (function_exists('admin_url') && function_exists('wp_create_nonce')) {
            $wpc_pt88 = get_option('wpc_css_passthrough', '0') === '1' ? '0' : '1';
            $bits[] = 'toggle-passthrough: ' . admin_url('admin-post.php?action=wpc_css_passthrough88&on=' . $wpc_pt88 . '&_wpnonce=' . wp_create_nonce('wpc_css_passthrough88'));
            $wpc_sn91 = get_option('wpc_ff_sentinel', '0') === '1' ? '0' : '1';
            $bits[] = 'ff-sentinel=' . get_option('wpc_ff_sentinel', '0') . ' toggle: ' . admin_url('admin-post.php?action=wpc_ff_sentinel91&on=' . $wpc_sn91 . '&_wpnonce=' . wp_create_nonce('wpc_ff_sentinel91'));
        }
        ...
        $safe77 = function ($v) {
            return str_replace('--', '- -', esc_html((string)$v));
        };
        $st77 = [];
        foreach (['wpc_update_window_until', 'wpc_delay_aggr_off', 'wpc_delay_aggr_fails', 'wpc_artifact_refresh_v', 'wpc_delay_v3_auto', 'delay_js_v3', 'delay_js_v2'] as $o77) {
            $st77[] = $o77 . '=' . var_export(get_option($o77, 'unset'), true);
        }
        if (function_exists('get_transient')) {
            $st77[] = 'mc_down=' . var_export(get_transient('wpc_mc_down'), true);
        }
        if (function_exists('wpc_v2_zone_cdn_suppressed')) {
            $st77[] = 'zone_suppressed=' . (wpc_v2_zone_cdn_suppressed() ? '1' : '0');
        }
        if (class_exists('wps_rewriteLogic') && method_exists('wps_rewriteLogic', 'wpc_nat_why808')) {
            $st77[] = 'nat=' . wps_rewriteLogic::wpc_nat_why808();
        }
        $key77 = '';
        if (class_exists('wps_ic_url_key')) {
            $wpc_uri77 = function_exists('home_url') ? home_url(strtok((string)($_SERVER['REQUEST_URI'] ?? '/'), '?')) : '';
            $key77 = ltrim((string)(new wps_ic_url_key())->setup($wpc_uri77), '/');
        }
        $st77[] = 'urlKey=' . $key77;
        if ($key77 !== '' && defined('WPS_IC_CRITICAL')) {
            $d77 = rtrim(WPS_IC_CRITICAL, '/') . '/' . $key77 . '/';
            $ls77 = [];
            foreach (array_slice((array)@glob($d77 . '*'), 0, 20) as $f77) {
                $ls77[] = basename($f77) . ':' . (int)@filesize($f77);
            }
            $st77[] = 'critDir=' . (is_dir($d77) ? (empty($ls77) ? 'EMPTY' : implode(',', $ls77)) : 'MISSING');
        }
        $tail77 = '';
        if (defined('WPS_IC_CACHE')) {
            $lf77 = rtrim(WPS_IC_CACHE, '/') . '/wpc-cflog.jsonl';
            $sz77 = (int)@filesize($lf77);
            if ($sz77 > 0) {
                $tail77 = (string)@file_get_contents($lf77, false, null, max(0, $sz77 - 8192));
                $ln77 = explode("\n", trim($tail77));
                $tail77 = implode("\n", array_slice($ln77, -30));
            }
        }
        ...
        $probe81 = function ($url) use ($safe77) {
            $r = wp_remote_get($url, ['timeout' => 10, 'sslverify' => false, 'headers' => ['user-agent' => 'Mozilla/5.0 (wpc-li-debug3)']]);
            if (is_wp_error($r)) {
                return 'FETCH-ERR: ' . $r->get_error_message();
            }
            $h = (string)wp_remote_retrieve_body($r);
            $o = [];
            $o[] = 'http=' . wp_remote_retrieve_response_code($r) . ' bytes=' . strlen($h) . ' st=' . implode(';', (array)wp_remote_retrieve_header($r, 'server-timing')) . ' cc=' . (string)wp_remote_retrieve_header($r, 'x-wpc-cc');
            $o[] = 'loader=' . (preg_match('/delay-v3-loader\.min\.js\?v=([0-9.]+)/', $h, $lm) ? $lm[1] : 'ABSENT');
            $o[] = 'crit=' . (strpos($h, 'wpc-critical-css') !== false ? 'yes' : 'NO') . ' stamp=' . (preg_match('/wpc-budget-final:[^*]{0,60}/', $h, $bm) ? $bm[0] : '-') . ' max-crit=' . (strpos($h, 'wpc-max-crit-served:') !== false ? 'served' : '-');
            $o[] = 'carriers: late-faces=' . (int)(strpos($h, 'wpc-late-faces') !== false) . ' crit-faces=' . (int)(strpos($h, 'wpc-crit-faces') !== false) . ' faces76=' . (int)(strpos($h, 'wpc-live-faces76') !== false) . ' missing75=' . (int)(strpos($h, 'wpc-missing-faces75') !== false) . ' vars-guard=' . (int)(strpos($h, 'wpc-vars-guard') !== false);
            $parked = [];
            if (preg_match_all('/<link\b[^>]*rel=["\x27]wpc-[a-z-]*stylesheet["\x27][^>]*href=["\x27]([^"\x27]+)["\x27]/i', $h, $pm)) {
                foreach (array_slice((array)$pm[1], 0, 10) as $pu) {
                    ...
                    $flag = '';
                    if (strpos($pu, '/et-cache/') !== false) {
                        $flag .= '[ET-CACHE!]';
                    }
                    if (substr_count($bn, '.vars2.css') > 0) {
                        $flag .= '[VARS2-CHAIN x' . substr_count($bn, '.vars2.css') . '!]';
                    }
                    $parked[] = $bn . $flag;
                }
            }
            ...
            if (preg_match_all('/@font-face\s*\{[^}]*?font-family\s*:\s*["\x27]?([^;"\x27}]+)/is', $h, $dm)) {
                foreach ((array)$dm[1] as $d) {
                    $dec[strtolower(trim($d))] = 1;
                }
            }
            $fb = [];
            foreach (array_keys($dec) as $d) {
                if (substr($d, -9) === ' fallback') {
                    $fb[] = substr($d, 0, -9);
                    unset($dec[$d]);
                }
            }
            ...
            if (preg_match_all('/--[a-z0-9_-]*font[a-z0-9_-]*\s*:\s*["\x27]([^;"\x27}]{3,40})["\x27]/i', $h, $um)) {
                foreach ((array)$um[1] as $u) {
                    $used[strtolower(trim($u))] = 1;
                }
            }
            ...
            $o[] = 'faces: declared=' . implode(',', array_slice(array_keys($dec), 0, 8)) . ' | metric-fallback=' . implode(',', array_slice($fb, 0, 8)) . ' | UNDECLARED-USED=' . ($miss ? implode(',', $miss) : 'none') . ' | NO-FALLBACK=' . ($nofb ? implode(',', array_slice($nofb, 0, 6)) : 'none');
            return implode("\n", array_map($safe77, $o));
        };
        echo "<!-- wpc-li-debug3 PAGE PROBE\nCACHED COPY (" . $safe77(home_url('/')) . "):\n" . $probe81(home_url('/')) . "\nFRESH MINT (cache-busted):\n" . $probe81(home_url('/?wpcdbg3=' . time())) . "\n-->\n";
    }

    if (function_exists('add_action')) {
        add_action('wp_footer', 'wpc_li_debug_receipt669', PHP_INT_MAX);


        if (!function_exists('wpc_css_passthrough_toggle88')) {
            function wpc_css_passthrough_toggle88()
            {
                if (!function_exists('current_user_can') || !current_user_can('manage_options') || empty($_GET['_wpnonce']) || !wp_verify_nonce((string)$_GET['_wpnonce'], 'wpc_css_passthrough88')) {
                    wp_die('denied');
                }
                $wpc_to88 = (isset($_GET['on']) && $_GET['on'] === '1') ? '1' : '0';
                update_option('wpc_css_passthrough', $wpc_to88, false);
                if ($wpc_to88 === '0') {
                    $wpc_as88 = get_option('wpc_autosafe102');
                    $wpc_as88 = is_array($wpc_as88) ? $wpc_as88 : [];
                    $wpc_as88['manual_off_ts'] = time();
                    update_option('wpc_autosafe102', $wpc_as88, false);
                }
                if (class_exists('wps_ic_cache_integrations')) {
                    wps_ic_cache_integrations::purgeAll(false, true, false, false, true);
                }
                wp_die('wpc_css_passthrough=' . $wpc_to88 . ' — caches purged. Reload the site.');
            }

            if (function_exists('add_action')) {
                add_action('admin_post_wpc_css_passthrough88', 'wpc_css_passthrough_toggle88');


                if (!function_exists('wpc_ff_sentinel_head91')) {
                    function wpc_ff_sentinel_head91()
                    {
                        if (get_option('wpc_ff_sentinel', '0') !== '1' || (function_exists('is_admin') && is_admin())) {
                            return;
                        }
                        $wpc_aj91 = function_exists('admin_url') ? admin_url('admin-ajax.php') : '/wp-admin/admin-ajax.php';
                        echo '<script id="wpc-ff-sentinel">(function(){if(window.top!==window)return;var W="wpc-";' . 'function chk(tag){try{var h=document.querySelector("h1");if(!h)return;var cs=getComputedStyle(h);' . 'var fs=parseFloat(cs.fontSize)||0;var fam=(cs.fontFamily||"").toLowerCase();' . 'var marked=!!document.querySelector("[id^="+W+"],[class*="+W+"]");' . 'var fam1=(cs.fontFamily||"").split(",")[0].replace(/["\x27]/g,"").trim();' . 'var notRendering=false;try{var cx=document.createElement("canvas").getContext("2d");' . 'cx.font="32px \'"+fam1+"\', Times New Roman";var wA=cx.measureText("Falknerei GgQq").width;' . 'cx.font="32px Times New Roman";var wB=cx.measureText("Falknerei GgQq").width;' . 'notRendering=(fam1.toLowerCase().indexOf("times")!==0&&Math.abs(wA-wB)<0.5);}catch(e){}' . 'var remBad=false;try{var rs=getComputedStyle(document.documentElement);' . 'remBad=(parseFloat(rs.fontSize)===16&&(rs.getPropertyValue("--fs-h1")||rs.getPropertyValue("--fluid"))!=="");}catch(e){}' . 'var bad=(fs>0&&fs<24)||(marked&&(notRendering||remBad||fam.indexOf("times")===0||fam.indexOf("serif")===0));' . 'if(!bad)return;var links=[];var ls=document.querySelectorAll("link[rel]");' . 'for(var i=0;i<ls.length&&links.length<15;i++){var rl=ls[i].getAttribute("rel");' . 'if(rl&&(rl.indexOf("stylesheet")>-1)){links.push(rl+"|"+String(ls[i].href).slice(-60)+"|"+(ls[i].media||""));}}' . 'var st="";try{var nv=performance.getEntriesByType("navigation")[0];' . '(nv.serverTiming||[]).forEach(function(x){st+=x.name+"="+x.description+";";});}catch(e){}' . 'var fd=new FormData();fd.append("action","wpc_ff_beacon91");fd.append("d",JSON.stringify(' . '{tag:tag,fs:fs,fam:fam.slice(0,40),nr:notRendering?1:0,rb:remBad?1:0,href:location.pathname,st:st,ua:navigator.userAgent.slice(0,70),links:links}).slice(0,6000));' . 'try{navigator.sendBeacon("' . esc_js($wpc_aj91) . '",fd);}catch(e){}}' . 'setTimeout(function(){chk("1500")},1500);setTimeout(function(){chk("3500")},3500);' . '})();</script>' . "\n";
                    }

                    function wpc_ff_beacon_rx91()
                    {
                        $wpc_n91 = (int)get_transient('wpc_ffb91_n');
                        if ($wpc_n91 >= 30) {
                            wp_die('', '', 200);
                        }
                        set_transient('wpc_ffb91_n', $wpc_n91 + 1, 3600);
                        $wpc_d91 = isset($_POST['d']) ? (string)wp_unslash($_POST['d']) : '';
                        if ($wpc_d91 !== '' && strlen($wpc_d91) <= 8192 && function_exists('wpc_cache_first_log')) {
                            $wpc_j91 = json_decode($wpc_d91, true);
                            if (is_array($wpc_j91)) {
                                wpc_cache_first_log('ff-sentinel', substr((string)($wpc_j91['href'] ?? ''), 0, 80), '', $wpc_j91);
                                wpc_auto_safe102($wpc_j91);
                            }
                        }
                        wp_die('', '', 200);
                    }
                    ...
                    function wpc_auto_safe102($wpc_j102)
                    {
                        try {
                            if (!function_exists('apply_filters') || !apply_filters('wpc_auto_safe', true) || get_option('wpc_css_passthrough') === '1' || empty($wpc_j102['tag'])) {
                                return;
                            }
                            $wpc_as102 = get_option('wpc_autosafe102');
                            $wpc_as102 = is_array($wpc_as102) ? $wpc_as102 : [];
                            $wpc_ver102 = defined('WPC_PLUGIN_VERSION') ? WPC_PLUGIN_VERSION : '0';
                            if (($wpc_as102['armed_v'] ?? '') === $wpc_ver102 || (time() - (int)($wpc_as102['manual_off_ts'] ?? 0)) < 7 * 86400) {
                                return;
                            }
                            $wpc_hits102 = array_values(array_filter((array)($wpc_as102['hits'] ?? []), function ($wpc_h102) {
                                return is_array($wpc_h102) && (time() - (int)($wpc_h102[0] ?? 0)) < 21600;
                            }));
                            $wpc_hits102[] = [time(), substr(md5((string)($wpc_j102['href'] ?? '')), 0, 8), substr(md5((string)($wpc_j102['ua'] ?? '')), 0, 8)];
                            $wpc_as102['hits'] = array_slice($wpc_hits102, -20);
                            $wpc_hrefs102 = array_unique(array_column($wpc_as102['hits'], 1));
                            $wpc_uas102 = array_unique(array_column($wpc_as102['hits'], 2));
                            if (count($wpc_as102['hits']) >= 3 && (count($wpc_hrefs102) >= 2 || count($wpc_uas102) >= 2)) {
                                update_option('wpc_css_passthrough', '1', false);
                                $wpc_as102['armed_v'] = $wpc_ver102;
                                $wpc_as102['armed_ts'] = time();
                                if (class_exists('wps_ic_cache_integrations')) {
                                    wps_ic_cache_integrations::purgeAll(false, true, false, false, true);
                                }
                                if (function_exists('wpc_cache_first_log')) {
                                    wpc_cache_first_log('auto-safe-armed', '', '', ['hits' => count($wpc_as102['hits']), 'pages' => count($wpc_hrefs102), 'uas' => count($wpc_uas102)]);
                                }
                            }
                            update_option('wpc_autosafe102', $wpc_as102, false);
                        } catch (\Throwable $wpc_e102) {
                        }
                    }

                    function wpc_ff_sentinel_toggle91()
                    {
                        if (!function_exists('current_user_can') || !current_user_can('manage_options') || empty($_GET['_wpnonce']) || !wp_verify_nonce((string)$_GET['_wpnonce'], 'wpc_ff_sentinel91')) {
                            wp_die('denied');
                        }
                        update_option('wpc_ff_sentinel', (isset($_GET['on']) && $_GET['on'] === '1') ? '1' : '0', false);
                        if (class_exists('wps_ic_cache_integrations')) {
                            wps_ic_cache_integrations::purgeAll(false, true, false, false, true);
                        }
                        wp_die('wpc_ff_sentinel=' . get_option('wpc_ff_sentinel') . ' — caches purged.');
                    }

                    if (function_exists('add_action')) {
                        add_action('wp_head', 'wpc_ff_sentinel_head91', -2147483646);
                        add_action('wp_ajax_wpc_ff_beacon91', 'wpc_ff_beacon_rx91');
                        add_action('wp_ajax_nopriv_wpc_ff_beacon91', 'wpc_ff_beacon_rx91');
                        add_action('admin_post_wpc_ff_sentinel91', 'wpc_ff_sentinel_toggle91');
                    }
                }
        ...
        if (!function_exists('wpc_ff_debug_head86')) {
            function wpc_ff_debug_head86()
            {
                if (empty($_GET['wpc_ff_debug']) || (function_exists('is_admin') && is_admin())) {
                    return;
                }
                $wpc_v86 = defined('WPC_PLUGIN_VERSION') ? WPC_PLUGIN_VERSION : '?';
                echo '<script id="wpc-ff-debug">(function(){if(window.top!==window)return;var T0=performance.now(),S=[],EV=[],DONE=0;' . 'function probe(sel){var el=document.querySelector(sel);if(!el)return null;var cs=getComputedStyle(el);var w=0;' . 'try{var tw=document.createTreeWalker(el,NodeFilter.SHOW_TEXT),n;while((n=tw.nextNode())){if(n.textContent.trim().length>3){var r=document.createRange();r.selectNodeContents(n);w=Math.round(r.getBoundingClientRect().width*10)/10;break;}}}catch(e){}' . 'return[cs.fontSize,cs.fontFamily.split(",")[0].replace(/"/g,""),cs.fontWeight,w,el.offsetWidth];}' . 'var SELS=["h1","h2",".et_pb_text p, p",".et_pb_button, a.rs-btn","#top-menu a, nav a"];' . 'function carriers(){var W="wpc-",ids=[W+"critical-css",W+"type-guard",W+"vars-guard",W+"late-faces",W+"crit-faces",W+"missing-faces75"],o={};' . 'for(var i=0;i<ids.length;i++){o[ids[i]]=!!document.getElementById(ids[i]);}' . 'o["faces76"]=!!document.querySelector("."+W+"live-faces76");o["used-css"]=!!document.querySelector("#"+W+"used-css-rest,[data-"+W+"ucss],[id^="+W+"used-css]");' . 'var pk=0,lv=0,ls=document.querySelectorAll("link[rel]");for(var j=0;j<ls.length;j++){var rl=ls[j].getAttribute("rel")||"";if(rl.indexOf("wpc-")===0)pk++;else if(rl==="stylesheet")lv++;}' . 'o.parked=pk;o.live=lv;return o;}' . 'var iv=setInterval(function(){var t=Math.round(performance.now());var row={t:t,p:SELS.map(probe)};' . 'var prev=S[S.length-1];if(!prev||JSON.stringify(prev.p)!==JSON.stringify(row.p))S.push(row);' . 'if(t-T0>10000){clearInterval(iv);DONE=1;render();}},50);' . 'try{new PerformanceObserver(function(l){l.getEntries().forEach(function(e){if(!e.hadRecentInput&&e.value>0.001){var src=[];try{(e.sources||[]).forEach(function(sc){var n=sc.node;if(!n)return;var d=(n.tagName||"?")+(n.id?"#"+n.id:"")+(n.className&&n.className.baseVal===undefined?"."+String(n.className).trim().split(/\\s+/).slice(0,2).join("."):"");var pr=sc.previousRect,cr=sc.currentRect;src.push(d+"["+pr.top+","+pr.height+"->"+cr.top+","+cr.height+"]");});}catch(x){}EV.push({t:Math.round(e.startTime),ls:Math.round(e.value*1000)/1000,src:src.slice(0,4)});}});}).observe({type:"layout-shift",buffered:true});}catch(e){}' . 'try{document.fonts.addEventListener("loadingdone",function(e){EV.push({t:Math.round(performance.now()),fonts:e.fontfaces.map(function(f){return f.family+"/"+f.weight+"/"+f.status;}).slice(0,8)});});}catch(e){}' . 'function render(){var rep={ver:"' . esc_js($wpc_v86) . '",url:location.pathname+location.search,ua:navigator.userAgent.slice(0,60),carriers:carriers(),frames:S,events:EV};' . 'var d=document.createElement("div");d.style.cssText="position:fixed;bottom:8px;right:8px;z-index:2147483647;background:#111;color:#0f0;font:11px monospace;padding:10px;max-width:420px;max-height:45vh;overflow:auto;border:1px solid #0f0;border-radius:6px";' . 'var btn=document.createElement("button");btn.textContent="COPY WPC FIRST-FRAME REPORT";btn.style.cssText="display:block;margin-bottom:6px;padding:6px;cursor:pointer";' . 'var pre=document.createElement("pre");pre.style.whiteSpace="pre-wrap";pre.textContent=JSON.stringify(rep);' . 'btn.onclick=function(){try{navigator.clipboard.writeText(pre.textContent);btn.textContent="COPIED — send to support";}catch(e){}};' . 'd.appendChild(btn);d.appendChild(pre);document.body.appendChild(d);window.__wpcFF=rep;}' . '})();</script>' . "\n";
            }

            if (function_exists('add_action')) {
                add_action('wp_head', 'wpc_ff_debug_head86', -2147483647);
            }
        }

        add_action('admin_footer', 'wpc_li_debug_receipt669', PHP_INT_MAX);
    }
}


if (!function_exists('wpc_delivery_config_sig146')) {
    function wpc_delivery_config_sig146()
    {
        $wpc_parts146 = [function_exists('get_option') ? get_option(defined('WPS_IC_SETTINGS') ? WPS_IC_SETTINGS : 'wps_ic_settings') : null, function_exists('get_option') ? get_option('wpc-excludes') : null, function_exists('get_option') ? get_option('wpc_css_passthrough') : null, defined('WPC_PLUGIN_VERSION') ? WPC_PLUGIN_VERSION : '',];
        return substr(md5(serialize($wpc_parts146)), 0, 12);
    }
}
if (!function_exists('wpc_dcv_flip146')) {
    function wpc_dcv_flip146()
    {
        try {
            $wpc_sig146 = wpc_delivery_config_sig146();
            if ((string)get_option('wpc_dcv') === $wpc_sig146) {
                return;
            }
            update_option('wpc_dcv', $wpc_sig146, false);
            if (class_exists('wps_ic_cache_integrations')) {
                wps_ic_cache_integrations::purgeAll(false, true, false, true, true);
            }
            if (defined('WPS_IC_CACHE')) {
                if (!@is_dir(WPS_IC_CACHE)) {
                    @mkdir(rtrim(WPS_IC_CACHE, '/'), 0755, true);
                }
                @file_put_contents(rtrim(WPS_IC_CACHE, '/') . '/dcv.txt', $wpc_sig146);
            }
            ...
            if (function_exists('wpc_cache_first_log')) {
                wpc_cache_first_log('dcv-flip', $wpc_sig146, '', []);
            }
        } catch (\Throwable $e) {
        }
    }
    ...
    if (function_exists('add_action')) {
        add_action('update_option_' . (defined('WPS_IC_SETTINGS') ? WPS_IC_SETTINGS : 'wps_ic_settings'), 'wpc_dcv_flip146', 20, 0);
        add_action('add_option_' . (defined('WPS_IC_SETTINGS') ? WPS_IC_SETTINGS : 'wps_ic_settings'), 'wpc_dcv_flip146', 20, 0);
        add_action('update_option_wpc-excludes', 'wpc_dcv_flip146', 20, 0);
        add_action('add_option_wpc-excludes', 'wpc_dcv_flip146', 20, 0);
        add_action('update_option_wpc_css_passthrough', 'wpc_dcv_flip146', 20, 0);
        add_action('add_option_wpc_css_passthrough', 'wpc_dcv_flip146', 20, 0);
        add_action('delete_option_wpc_css_passthrough', 'wpc_dcv_flip146', 20, 0);
    }
}


if (!function_exists('wpc_lite_pillar_riders148')) {
    function wpc_lite_pillar_riders148($settings, $key, $value)
    {
...
if (!function_exists('wpc_pillar_rider_heal215')) {
    function wpc_pillar_rider_heal215()
    {
        if (!function_exists('get_option') || !defined('WPS_IC_SETTINGS')) {
            return 0;
        }
        ...
        $healed = 0;
        $map = [];
        $wpc_g321r = function_exists('wpc_ucss_divi5_guard') && wpc_ucss_divi5_guard();
        if (!empty($set['critical']['css']) && $set['critical']['css'] == '1' && !$wpc_g321r) {
            $map['used-css'] = '1';
        }
        if (!empty($set['imagesPreset']) && $set['imagesPreset'] == '1') {
            $map += ['retina' => '1', 'generate_adaptive' => '1', 'generate_webp' => '1', 'picture_webp' => '1', 'picture_avif' => '1'];
        ...
        if (!empty($set['critical']['css']) && $set['critical']['css'] == '1' && !$wpc_g321r && isset($set['used-css']) && (string)$set['used-css'] === '0' && get_option('wpc_r2_reentry_v') !== (defined('WPC_PLUGIN_VERSION') ? WPC_PLUGIN_VERSION : '?')) {
            $wpc_frozen216 = [];
            if (function_exists('wpc_auto_state')) {
                $wpc_ast216 = wpc_auto_state();
                $wpc_frozen216 = is_array($wpc_ast216) && isset($wpc_ast216['frozen']) ? (array)$wpc_ast216['frozen'] : [];
            }
            update_option('wpc_r2_reentry_v', defined('WPC_PLUGIN_VERSION') ? WPC_PLUGIN_VERSION : '?', false);
            if (!in_array('R2', $wpc_frozen216, true)) {
                $set['used-css'] = '1';
                $healed++;
                if (function_exists('wpc_cache_first_log')) {
                    wpc_cache_first_log('r2-reentry', '', '', ['via' => 'reconciler-thaw']);
                }
            }
        }
        if ($healed > 0) {
...
if (!function_exists('wpc_lite_is_pillar148')) {
    function wpc_lite_is_pillar148($key)
    {
        if (is_array($key)) {
