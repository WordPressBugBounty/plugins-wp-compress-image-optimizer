...
if (!function_exists('wpc_crit_meta_files')) {
    ...
    function wpc_crit_meta_files()
    {
        return ['url.txt', 'uuid.txt', 'land_uuid.txt', 'dispatch_ts.txt', 'land_ts.txt',
            ...
            'crit_combined_src.txt', 'font-preload.txt', 'cmbkey-in.txt', 'dom_ids54.txt'];
    }
}
if (!function_exists('wpc_dom_ids54')) {
    function wpc_dom_ids54($html)
    {
        if (!is_string($html) || $html === '' || !preg_match_all('/data-id="([a-f0-9]{6,8})"/', $html, $wpc_m54)) {
            return [];
        }
        return array_values(array_unique($wpc_m54[1]));
    }
}
if (!function_exists('wpc_dom_ids_union54')) {
    function wpc_dom_ids_union54($dir, $ids, $mobile)
    {
        $wpc_f54 = rtrim((string) $dir, '/') . '/dom_ids54.txt';
        $wpc_flags54 = '';
        $wpc_have54 = [];
        if (@is_file($wpc_f54)) {
            $wpc_lines54 = preg_split('/\s+/', trim((string) @file_get_contents($wpc_f54)), -1, PREG_SPLIT_NO_EMPTY);
            if (!empty($wpc_lines54) && substr((string) $wpc_lines54[0], 0, 6) === 'flags:') {
                $wpc_flags54 = substr((string) array_shift($wpc_lines54), 6);
            }
            $wpc_have54 = $wpc_lines54;
        }
        $wpc_dev54 = $mobile ? 'm' : 'd';
        $wpc_nflags54 = strpos($wpc_flags54, $wpc_dev54) === false ? $wpc_flags54 . $wpc_dev54 : $wpc_flags54;
        $wpc_set54 = array_fill_keys($wpc_have54, 1);
        $wpc_add54 = 0;
        foreach ((array) $ids as $wpc_i54) {
            if (!isset($wpc_set54[$wpc_i54])) {
                $wpc_set54[$wpc_i54] = 1;
                $wpc_add54++;
            }
        }
        if (($wpc_add54 > 0 || $wpc_nflags54 !== $wpc_flags54) && count($wpc_set54) <= 3000 && function_exists('wpc_fs_put')) {
            wpc_fs_put($wpc_f54, 'flags:' . $wpc_nflags54 . "\n" . implode("\n", array_keys($wpc_set54)));
        }
        return ['ids' => $wpc_set54, 'both' => strpos($wpc_nflags54, 'm') !== false && strpos($wpc_nflags54, 'd') !== false];
    }
}
if (!function_exists('wpc_artifact_foreign54')) {
    function wpc_artifact_foreign54($css, $union)
    {
        if (!is_string($css) || strlen($css) < 1024 || !is_array($union) || empty($union)
            || !preg_match_all('/\.elementor-element-([a-f0-9]{6,8})\b/', $css, $wpc_m54)) {
            return [];
        }
        $wpc_cids54 = array_values(array_unique($wpc_m54[1]));
        $wpc_abs54 = [];
        foreach ($wpc_cids54 as $wpc_c54) {
            if (!isset($union[$wpc_c54])) {
                $wpc_abs54[] = $wpc_c54;
            }
        }
        if (count($wpc_abs54) >= 3 && count($wpc_abs54) * 10 >= count($wpc_cids54)) {
            return $wpc_abs54;
        }
        return [];
    }
...
function wpc_render_slot_acquire()
{
    ...
    $wpc_cores29 = function_exists('wpc_box_cores') ? max(1, (int) wpc_box_cores()) : 4;
    $n = (int) (function_exists('apply_filters') ? apply_filters('wpc_render_slots', min(4, $wpc_cores29)) : min(4, $wpc_cores29));
    if ($n <= 0) {
        return true;
    }
    for ($i = 1; $i <= $n; $i++) {
...
function wpc_variant_miss_302_belt258()
{
    try {
        if (is_admin() || !function_exists('is_404') || !is_404()
            || !apply_filters('wpc_variant_miss_302', true)) {
            return;
        }
        $uri = (string) ($_SERVER['REQUEST_URI'] ?? '');
        $path = (string) parse_url($uri, PHP_URL_PATH);
        if (!preg_match('~^(.*/wp-content/uploads/.+)\.(avif|webp)$~i', $path, $pm)
            || strpos($path, '..') !== false) {
            return;
        }
        $stem = (string) $pm[1];
        ...
        foreach (['jpg', 'jpeg', 'png'] as $e258) {
            if (!in_array($e258, $exts, true)) { $exts[] = $e258; }
        }
...
if (!function_exists('wpc_font_carrier_host_ok22')) {
    function wpc_font_carrier_host_ok22($url)
    {
...
if (!function_exists('wpc_font_carrier_filter22')) {
    function wpc_font_carrier_filter22($css)
    {
        ...
        $out = preg_replace_callback('/@font-face\s*\{[^{}]*\}/is', function ($m) use (&$dropped) {
            if (preg_match_all('/url\(\s*["\']?(https?:\/\/[^)"\'\s]+)/i', $m[0], $u)) {
                foreach ($u[1] as $one) {
                    if (!wpc_font_carrier_host_ok22($one)) {
                        $dropped++;
                        if (preg_match('/font-family\s*:\s*["\']?([^"\';}]+)/i', $m[0], $f)) {
                            $GLOBALS['wpc_fc_tp_fams22'][strtolower(trim($f[1]))] = 1;
                        }
                        return '';
                    }
                }
        ...
        if ($dropped > 0 && function_exists('wpc_cache_first_log') && function_exists('get_transient') && !get_transient('wpc_fc_tp22')) {
            set_transient('wpc_fc_tp22', 1, 600);
            wpc_cache_first_log('font-carrier-thirdparty-dropped', '', '', ['n' => $dropped]);
        }
        return $out;
    }
}
...
if (!function_exists('wpc_font_carrier_record602')) {
    function wpc_font_carrier_record602($css)
    {
        try {
            ...
            if (function_exists('wpc_font_carrier_filter22')) {
                $css = wpc_font_carrier_filter22($css);
...
if (!function_exists('wpc_font_carrier_emit602')) {
    function wpc_font_carrier_emit602()
    {
        ...
        try {
            ...
            if (function_exists('wpc_font_carrier_filter22')) {
                $css = (string) wpc_font_carrier_filter22($css);
                if ($css === '' || stripos($css, '@font-face') === false) { return; }
            }
            if (class_exists('wps_cdn_rewrite') && method_exists('wps_cdn_rewrite', 'wpc_host_twin_css42')) {
            ...
            $wpc_off602 = is_array($wpc_fo602) && !empty($wpc_fo602['font-display'])
                && strtolower((string) $wpc_fo602['font-display']) === 'off';
            ...
            if (preg_match_all('/@font-face\s*\{[^{}]*\}/is', $css, $wpc_fb32)) {
                $wpc_pl22 = [];
                $wpc_seen32 = [];
                foreach ($wpc_fb32[0] as $wpc_blk32) {
                    if (!preg_match('/url\(\s*["\']?(https?:\/\/[^)"\'\s]+\.woff2[^)"\'\s]*)/i', $wpc_blk32, $wpc_um32)) { continue; }
                    $wpc_u32 = $wpc_um32[1];
                    if (isset($wpc_seen32[$wpc_u32]) || preg_match('/icon|awesome|fa-(?:solid|regular|brands|light|duotone)|fa[- 0-9]|material|dashicon|glyphicon|icomoon|ionicon|line.?awesome|themify|elegant|feather|simple.?line|eicons|happy-icons/i', $wpc_u32)) { continue; }
                    $wpc_seen32[$wpc_u32] = 1;
                    $wpc_fam32 = preg_match('/font-family\s*:\s*["\']?([^"\';}]+)/i', $wpc_blk32, $wpc_fm32) ? trim($wpc_fm32[1]) : '';
                    $wpc_pl22[] = [$wpc_u32, 'font/woff2', $wpc_fam32];
                }
                $wpc_fpb689 = wpc_font_preload_postpaint_tag(array_slice($wpc_pl22, 0, 4));
                if ($wpc_fpb689 !== '') {
                    echo "\n" . $wpc_fpb689;
                }
            }
...
if (!function_exists('wpc_lcp_bg_preloaded')) {
    function wpc_lcp_bg_preloaded($html)
    {
        return is_string($html)
            && (strpos($html, 'wpc-lcp-bg-preload') !== false || strpos($html, 'wpc-lcp-hero-preload') !== false);
    }
}
if (!function_exists('wpc_crit_sanity_mark617')) {
    function wpc_crit_sanity_mark617($dir, $critBytes, $ids = [])
    {
        ...
        wpc_fs_put(rtrim((string) $dir, '/') . '/sanity_bad.txt',
...
if (!function_exists('wpc_ucss_sanity_mark617')) {
    function wpc_ucss_sanity_mark617($dir, $path)
    {
        ...
        wpc_fs_put(rtrim((string) $dir, '/') . '/ucss_sanity_bad.txt',
...
if (!function_exists('wpc_crit_sanity_stall621')) {
    function wpc_crit_sanity_stall621($dir, $critBytes)
    {
        ...
        $wpc_old621 = @is_file($wpc_mf621) ? trim((string) strtok((string) @file_get_contents($wpc_mf621), "\n")) : '';
        ...
        wpc_fs_put($wpc_sf621, $wpc_n621 . ':' . (int) ($wpc_st621[1] ?? 0 ?: time()));
        return $wpc_n621;
    }
}
...
if (!function_exists('wpc_crit_sanity_stall_tick622')) {
    function wpc_crit_sanity_stall_tick622($dir)
    {
        ...
        if (isset($GLOBALS['wpc_stall_once55'][$wpc_sf622])) {
            return (int) $GLOBALS['wpc_stall_once55'][$wpc_sf622];
        }
...
if (!function_exists('wpc_finish_request39')) {
    function wpc_finish_request39()
    {
        if (function_exists('fastcgi_finish_request')) {
            $GLOBALS['wpc_released39'] = true;
...
if (!function_exists('wpc_render_guard39_lane')) {
    function wpc_render_guard39_lane($sapi = null)
    {
...
if (!function_exists('wpc_render_guard39_active')) {
    function wpc_render_guard39_active()
    {
        return !empty($GLOBALS['wpc_rg39']) && empty($GLOBALS['wpc_released39']);
    }
}
if (!function_exists('wpc_net_defer39')) {
    function wpc_net_defer39($id, $fn)
    {
        if (!isset($GLOBALS['wpc_net_q39']) || !is_array($GLOBALS['wpc_net_q39'])) {
            $GLOBALS['wpc_net_q39'] = [];
        }
        $id = (string) $id;
        if (count($GLOBALS['wpc_net_q39']) >= 16 && !isset($GLOBALS['wpc_net_q39'][$id])) {
            return false;
        }
        $GLOBALS['wpc_net_q39'][$id] = $fn;
        return true;
    }
}
if (!function_exists('wpc_net_deferred39')) {
    function wpc_net_deferred39($resp, $id, $fn)
    {
        if (!is_object($resp) || !is_a($resp, 'WP_Error') || $resp->get_error_code() !== 'wpc_render_guard39') {
            return false;
        }
        wpc_net_defer39($id, $fn);
        return true;
    }
}
if (!function_exists('wpc_render_guard39_own_frame')) {
    function wpc_render_guard39_own_frame($trace)
    {
...
if (!function_exists('wpc_render_guard39_filter')) {
    function wpc_render_guard39_filter($pre, $args, $url)
    {
        if ($pre !== false || !wpc_render_guard39_active()) {
            return $pre;
        }
        ...
        $own = wpc_render_guard39_own_frame(debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 18));
        if ($own === '') {
            return $pre;
        }
        $g = is_array($GLOBALS['wpc_rg39']) ? $GLOBALS['wpc_rg39'] : ['mode' => 'block', 'n' => 0, 'seen' => []];
        ...
        return new WP_Error('wpc_render_guard39', 'WP Compress: outbound call deferred past the response');
    }
}
if (!function_exists('wpc_render_guard39_arm')) {
    function wpc_render_guard39_arm($sapi = null)
    {
        try {
            if (!empty($GLOBALS['wpc_rg39'])) {
                return;
            }
            ...
            if (wpc_render_guard39_lane(is_string($sapi) ? $sapi : null) !== 'visitor') {
                return;
            }
            ...
            $GLOBALS['wpc_rg39'] = ['mode' => $mode === 'observe' ? 'observe' : 'block', 'n' => 0, 'seen' => []];
            add_filter('pre_http_request', 'wpc_render_guard39_filter', -2147483647, 3);
            add_action('shutdown', 'wpc_render_guard39_release', 2);
        } catch (\Throwable $e) {
        }
    }
}
if (!function_exists('wpc_net_ceiling57_filter')) {
    ...
    function wpc_net_ceiling57_filter($pre, $args, $url)
    {
        if ($pre !== false) {
            return $pre;
        }
        ...
        $own = function_exists('wpc_render_guard39_own_frame') ? wpc_render_guard39_own_frame(debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 18)) : '';
        if ($own === '') {
            return $pre;
        }
        $u = (string) $url;
        $path = (string) parse_url($u, PHP_URL_HOST) . (string) parse_url($u, PHP_URL_PATH);
        $c = (isset($GLOBALS['wpc_nc57']) && is_array($GLOBALS['wpc_nc57'])) ? $GLOBALS['wpc_nc57'] : ['n' => 0, 'p' => [], 'hit' => 0];
        $c['n'] = (int) $c['n'] + 1;
        ...
        $wpc_host29 = (string) parse_url($u, PHP_URL_HOST);
        $wpc_hcap29 = (int) apply_filters('wpc_net_hourly_host_cap', 3000, $wpc_host29);
        if ($wpc_hcap29 > 0 && $wpc_host29 !== '' && function_exists('get_transient')) {
            $wpc_hk29 = 'wpc_neth29_' . md5($wpc_host29 . '|' . floor(time() / 3600));
            $wpc_hn29 = (int) get_transient($wpc_hk29) + 1;
            set_transient($wpc_hk29, $wpc_hn29, 3600);
            if ($wpc_hn29 > $wpc_hcap29) {
                if (function_exists('wpc_cache_first_log') && !get_transient('wpc_neth29_log_' . md5($wpc_host29))) {
                    set_transient('wpc_neth29_log_' . md5($wpc_host29), 1, 600);
                    wpc_cache_first_log('net-hourly-cap', $wpc_host29, '', ['n' => $wpc_hn29, 'cap' => $wpc_hcap29]);
                }
                return new WP_Error('wpc_net_hourly_cap', 'WP Compress: hourly outbound cap for ' . $wpc_host29);
            }
        }
        ...
        $GLOBALS['wpc_nc57'] = $c;
        return new WP_Error('wpc_net_ceiling57', 'WP Compress: per-request outbound ceiling reached (' . $over . ')');
    }
    function wpc_net_ceiling57_arm()
    {
        ...
        add_filter('pre_http_request', 'wpc_net_ceiling57_filter', -2147483646, 3);
    }
    if (function_exists('add_action')) {
        add_action('init', 'wpc_net_ceiling57_arm', -2147483646);
    }
}
if (!function_exists('wpc_render_guard39_release')) {
    function wpc_render_guard39_release()
    {
        if (empty($GLOBALS['wpc_rg39'])) {
            return;
        }
        $g = $GLOBALS['wpc_rg39'];
        $q = (isset($GLOBALS['wpc_net_q39']) && is_array($GLOBALS['wpc_net_q39'])) ? $GLOBALS['wpc_net_q39'] : [];
        $GLOBALS['wpc_net_q39'] = [];
        if (!$q) {
            $GLOBALS['wpc_rg39'] = false;
            return;
        }
        $rel = wpc_finish_request39();
        $GLOBALS['wpc_rg39'] = false;
        if (!$rel) {
...
if (function_exists('add_action')) {
    add_action('init', 'wpc_render_guard39_arm', -2147483647);
}
...
if (!function_exists('wpc_delay_v3_excl_seed55')) {
    function wpc_delay_v3_excl_seed55()
    {
        ...
        $wpc_x55['delay_js_v3'] = (isset($wpc_x55['delay_js_v2']) && is_array($wpc_x55['delay_js_v2']))
            ? array_values($wpc_x55['delay_js_v2']) : array();
...
if (!function_exists('wpc_strip_livefaces_fams329')) {
    ...
    {
        try {
            if (!is_string($facesCss) || $facesCss === '' || !is_string($doc)
                || stripos($facesCss, '@font-face') === false
                || stripos($doc, 'wpc-live-faces') === false
                || !apply_filters('wpc_livefaces_strip', true)) {
                return $facesCss;
            }
            $wpc_fams329 = [];
            ...
            $wpc_out329 = preg_replace_callback('/@font-face\s*\{[^{}]*\}/is', function ($m) use ($wpc_fams329) {
                if (preg_match('/font-family\s*:\s*["\']?([^"\';}]+)/i', $m[0], $f)
                    && !empty($wpc_fams329[strtolower(trim($f[1]))])) {
                    return '';
                }
...
if (!function_exists('wpc_ucss_divi5_guard')) {
        ...
        if ($wpc_g321 === null) {
            ...
            try {
                if (function_exists('wp_get_theme') && function_exists('get_template')) {
                    ...
                    if ($wpc_t321 && stripos((string) $wpc_t321->get('Name'), 'divi') !== false
                        && (int) $wpc_t321->get('Version') >= 5) {
                        $wpc_g321 = true;
                    }
                }
            } catch (\Throwable $e) {
    ...
    if (function_exists('add_action')) {
        ...
        try {
            ...
            if (is_array($wpc_s321) && !empty($wpc_s321['used-css']) && $wpc_s321['used-css'] == '1'
...
if (!function_exists('wpc_cdn_cname_reachable117')) {
    function wpc_cdn_cname_reachable117($host)
    {
        try {
...
if (!function_exists('wpc_visitor_kind111')) {
    function wpc_visitor_kind111()
    {
        try {
...
if (!function_exists('wpc_views_24h_key111')) {
    function wpc_views_24h_key111($url_key)
    {
        return 'wpc_v24_' . substr(md5((string) $url_key), 0, 20);
    }
}
if (!function_exists('wpc_views_24h_bump111')) {
    function wpc_views_24h_bump111($url_key)
    {
        try {
            if ((string) $url_key === '' || !function_exists('get_transient')) {
                return;
            }
            $wpc_hu46 = !(function_exists('wpc_visitor_kind111') && wpc_visitor_kind111() !== 'human');
            if (function_exists('wpc_site_views_bump46')) {
                wpc_site_views_bump46($wpc_hu46);
            }
            if (!$wpc_hu46) {
                return;
            }
            $k = wpc_views_24h_key111($url_key);
...
if (!function_exists('wpc_site_views_bump46')) {
    function wpc_site_views_bump46($human)
    {
        try {
    ...
    function wpc_site_counts_due46()
    {
        $d = gmdate('Ymd');
    ...
    function wpc_rum_sample_land46($data)
    {
        ...
        if ((int) get_option('wpc_rum_sample46', 0) !== $n) {
            update_option('wpc_rum_sample46', $n, false);
            if (function_exists('wpc_cache_first_log')) {
    ...
    function wpc_rum_sample46()
    {
        $n = function_exists('get_option') ? (int) get_option('wpc_rum_sample46', 0) : 0;
        return $n >= 1 ? $n : 50;
    }
}
if (!function_exists('wpc_views_24h111')) {
    function wpc_views_24h111($url_key)
    {
        try {
            if ((string) $url_key === '' || !function_exists('get_transient')) {
                return 0;
            }
            return (int) get_transient(wpc_views_24h_key111($url_key));
        } catch (\Throwable $e) {
            return 0;
        }
    }
...
if (!function_exists('wpc_doctrine_reconcile66')) {
    function wpc_ucss_invalidate313()
    {
        ...
        foreach ((array) @glob($wpc_root313 . '/used-css/*') as $wpc_f313) {
            if (is_file($wpc_f313) && @unlink($wpc_f313)) { $wpc_n313++; }
        }
        foreach ((array) @glob($wpc_root313 . '/*/used_css*_url.txt') as $wpc_f313) {
            if (@unlink($wpc_f313)) { $wpc_n313++; }
        }
        if (function_exists('wpc_cache_first_log')) {
    ...
    function wpc_doctrine_reconcile66($wpc_force66 = false)
    {
        ...
        if (function_exists('wpc_pillar_rider_heal215')) {
            ...
            if ($wpc_prh215 > 0) {
                ...
                if (function_exists('wpc_css_host_twin_heal231') && defined('WPS_IC_CRITICAL')
                    && get_option('wpc_twin_sweep_v') !== WPC_PLUGIN_VERSION) {
                    $wpc_th231 = 0;
                    $wpc_tf231 = array_merge(
                        (array) glob(rtrim(WPS_IC_CRITICAL, '/') . '/used-css/*.css'),
                        (array) glob(rtrim(WPS_IC_CRITICAL, '/') . '/*/critical_*.css')
                    );
                    foreach (array_slice($wpc_tf231, 0, 400) as $wpc_tp231) {
                        $wpc_tb231 = (string) @file_get_contents($wpc_tp231);
                        if ($wpc_tb231 === '') { continue; }
                        $wpc_tn231 = wpc_css_host_twin_heal231($wpc_tb231);
                        if (is_string($wpc_tn231) && $wpc_tn231 !== $wpc_tb231
                            && wpc_fs_put($wpc_tp231 . '.tmp', $wpc_tn231, LOCK_EX) !== false
                            && @rename($wpc_tp231 . '.tmp', $wpc_tp231)) {
                        $wpc_th231++;
                        }
                    }
                    if ($wpc_th231 > 0) { $wpc_did66[] = 'host-twin-sweep:' . $wpc_th231; }
        ...
        if (function_exists('wpc_park_reset52')) {
            $wpc_pr52 = (int) wpc_park_reset52();
            if ($wpc_pr52 > 0) {
                $wpc_did66[] = 'park-reset:' . $wpc_pr52;
            }
        }
        ...
        if (get_option('wpc_css_passthrough') === '1'
            && is_array($wpc_as143) && !empty($wpc_as143['armed_v'])
            && get_option('wpc_ff_sentinel', '0') === '1'
            && defined('WPC_PLUGIN_VERSION')
            && ($wpc_as143['probe_v'] ?? '') !== WPC_PLUGIN_VERSION
            && apply_filters('wpc_autosafe_probation', true)) {
            $wpc_as143['probe_v'] = WPC_PLUGIN_VERSION;
    ...
    function wpc_doctrine_stamp66()
    {
        ...
        $wpc_claim57 = 'wpc_dclaim66_' . preg_replace('/[^0-9a-z]/i', '_', (string) WPC_PLUGIN_VERSION);
        if (function_exists('add_option') && !add_option($wpc_claim57, (string) time(), '', 'no')) {
            if ((time() - (int) get_option($wpc_claim57, 0)) < 600) {
                return;
            }
        }
        $wpc_prev57 = (string) get_option('wpc_doctrine_ver');
        ...
        if (get_option('wpc_doctrine_ver') !== false
            && apply_filters('wpc_update_purges_html', true)
            && class_exists('wps_ic_cache_integrations')
            && method_exists('wps_ic_cache_integrations', 'purgeAll')) {
            wps_ic_cache_integrations::purgeAll(false, true, false, false, true);
            if (function_exists('wpc_cache_first_log')) {
        ...
        if ($wpc_prev57 !== '' && $wpc_prev57 !== WPC_PLUGIN_VERSION && function_exists('delete_option')) {
            delete_option('wpc_dclaim66_' . preg_replace('/[^0-9a-z]/i', '_', $wpc_prev57));
        }
    }
    if (function_exists('add_action')) {
    ...
    function wpc_disable_emoji233()
    {
        ...
        $on233 = class_exists('wps_ic_js_delay_v3') && method_exists('wps_ic_js_delay_v3', 'wpc_delay_master_on')
            ? (bool) wps_ic_js_delay_v3::wpc_delay_master_on(is_array($s233) ? $s233 : [])
            : (is_array($s233) && !empty($s233['delay-js']) && $s233['delay-js'] == '1');
        if (!$on233) {
            return;
        }
    ...
    function wpc_update_auto_refresh187()
    {
        if (!defined('WPC_PLUGIN_VERSION')
            || !apply_filters('wpc_update_auto_refresh', !defined('WPC_DISABLE_AUTO_REFRESH'))) {
            return;
        }
    ...
    function wpc_li_debug_receipt669()
    {
        ...
        $bits = [
            ...
            'css-passthrough=' . (string) get_option('wpc_css_passthrough', '0')
                . (is_array($wpc_as669 = get_option('wpc_autosafe102')) && !empty($wpc_as669['armed_ts'])
                    ? ' auto-safe-armed=' . gmdate('Y-m-d H:i', (int) $wpc_as669['armed_ts']) . ' hits=' . count((array) ($wpc_as669['hits'] ?? []))
                    : ' auto-safe=watching'),
        ];
        if (function_exists('admin_url') && function_exists('wp_create_nonce')) {
            $wpc_pt88 = get_option('wpc_css_passthrough', '0') === '1' ? '0' : '1';
            $bits[] = 'toggle-passthrough: ' . admin_url('admin-post.php?action=wpc_css_passthrough88&on=' . $wpc_pt88
                . '&_wpnonce=' . wp_create_nonce('wpc_css_passthrough88'));
            $wpc_sn91 = get_option('wpc_ff_sentinel', '0') === '1' ? '0' : '1';
            $bits[] = 'ff-sentinel=' . get_option('wpc_ff_sentinel', '0')
                . ' toggle: ' . admin_url('admin-post.php?action=wpc_ff_sentinel91&on=' . $wpc_sn91
                . '&_wpnonce=' . wp_create_nonce('wpc_ff_sentinel91'));
        }
        ...
        $safe77 = function ($v) { return str_replace('--', '- -', esc_html((string) $v)); };
        $st77 = [];
        foreach (['wpc_update_window_until', 'wpc_delay_aggr_off', 'wpc_delay_aggr_fails',
            'wpc_artifact_refresh_v', 'wpc_delay_v3_auto', 'delay_js_v3', 'delay_js_v2'] as $o77) {
        ...
        $probe81 = function ($url) use ($safe77) {
            ...
            $o[] = 'crit=' . (strpos($h, 'wpc-critical-css') !== false ? 'yes' : 'NO')
                 . ' stamp=' . (preg_match('/wpc-budget-final:[^*]{0,60}/', $h, $bm) ? $bm[0] : '-')
            ...
            $o[] = 'carriers: late-faces=' . (int) (strpos($h, 'wpc-late-faces') !== false)
                 ...
                 . ' vars-guard=' . (int) (strpos($h, 'wpc-vars-guard') !== false);
            $parked = [];
...
if (!function_exists('wpc_css_passthrough_toggle88')) {
    ...
    {
        if (!function_exists('current_user_can') || !current_user_can('manage_options')
            || empty($_GET['_wpnonce']) || !wp_verify_nonce((string) $_GET['_wpnonce'], 'wpc_css_passthrough88')) {
            wp_die('denied');
        }
...
if (!function_exists('wpc_ff_sentinel_head91')) {
    function wpc_ff_sentinel_head91()
    {
        ...
        echo '<script id="wpc-ff-sentinel">(function(){if(window.top!==window)return;var W="wpc-";'
            . 'function chk(tag){try{var h=document.querySelector("h1");if(!h)return;var cs=getComputedStyle(h);'
            . 'var fs=parseFloat(cs.fontSize)||0;var fam=(cs.fontFamily||"").toLowerCase();'
            . 'var marked=!!document.querySelector("[id^="+W+"],[class*="+W+"]");'
            . 'var fam1=(cs.fontFamily||"").split(",")[0].replace(/["\x27]/g,"").trim();'
            . 'var notRendering=false;try{var cx=document.createElement("canvas").getContext("2d");'
            . 'cx.font="32px \'"+fam1+"\', Times New Roman";var wA=cx.measureText("Falknerei GgQq").width;'
            . 'cx.font="32px Times New Roman";var wB=cx.measureText("Falknerei GgQq").width;'
            . 'notRendering=(fam1.toLowerCase().indexOf("times")!==0&&Math.abs(wA-wB)<0.5);}catch(e){}'
            . 'var remBad=false;try{var rs=getComputedStyle(document.documentElement);'
            . 'remBad=(parseFloat(rs.fontSize)===16&&(rs.getPropertyValue("--fs-h1")||rs.getPropertyValue("--fluid"))!=="");}catch(e){}'
            . 'var bad=(fs>0&&fs<24)||(marked&&(notRendering||remBad||fam.indexOf("times")===0||fam.indexOf("serif")===0));'
            . 'if(!bad)return;var links=[];var ls=document.querySelectorAll("link[rel]");'
            . 'for(var i=0;i<ls.length&&links.length<15;i++){var rl=ls[i].getAttribute("rel");'
            . 'if(rl&&(rl.indexOf("stylesheet")>-1)){links.push(rl+"|"+String(ls[i].href).slice(-60)+"|"+(ls[i].media||""));}}'
            . 'var st="";try{var nv=performance.getEntriesByType("navigation")[0];'
            . '(nv.serverTiming||[]).forEach(function(x){st+=x.name+"="+x.description+";";});}catch(e){}'
            . 'var fd=new FormData();fd.append("action","wpc_ff_beacon91");fd.append("d",JSON.stringify('
            . '{tag:tag,fs:fs,fam:fam.slice(0,40),nr:notRendering?1:0,rb:remBad?1:0,href:location.pathname,st:st,ua:navigator.userAgent.slice(0,70),links:links}).slice(0,6000));'
            . 'try{navigator.sendBeacon("' . esc_js($wpc_aj91) . '",fd);}catch(e){}}'
            . 'setTimeout(function(){chk("1500")},1500);setTimeout(function(){chk("3500")},3500);'
            . '})();</script>' . "\n";
    }
    function wpc_ff_beacon_rx91()
    {
        $wpc_n91 = (int) get_transient('wpc_ffb91_n');
        if ($wpc_n91 >= 30) { wp_die('', '', 200); }
    ...
    {
        try {
            if (!function_exists('apply_filters') || !apply_filters('wpc_auto_safe', true)
                || get_option('wpc_css_passthrough') === '1'
                || empty($wpc_j102['tag'])) {
                return;
            }
            ...
            if (($wpc_as102['armed_v'] ?? '') === $wpc_ver102
                || (time() - (int) ($wpc_as102['manual_off_ts'] ?? 0)) < 7 * 86400) {
                return;
            }
            ...
            $wpc_hits102[] = [time(),
                substr(md5((string) ($wpc_j102['href'] ?? '')), 0, 8),
                substr(md5((string) ($wpc_j102['ua'] ?? '')), 0, 8)];
            ...
            if (count($wpc_as102['hits']) >= 3 && (count($wpc_hrefs102) >= 2 || count($wpc_uas102) >= 2)) {
                ...
                if (function_exists('wpc_cache_first_log')) {
                    wpc_cache_first_log('auto-safe-armed', '', '', ['hits' => count($wpc_as102['hits']),
                        'pages' => count($wpc_hrefs102), 'uas' => count($wpc_uas102)]);
                }
            }
    ...
    {
        if (!function_exists('current_user_can') || !current_user_can('manage_options')
            || empty($_GET['_wpnonce']) || !wp_verify_nonce((string) $_GET['_wpnonce'], 'wpc_ff_sentinel91')) {
            wp_die('denied');
        }
...
if (!function_exists('wpc_ff_debug_head86')) {
    function wpc_ff_debug_head86()
    {
        ...
        echo '<script id="wpc-ff-debug">(function(){if(window.top!==window)return;var T0=performance.now(),S=[],EV=[],DONE=0;'
            ...
            . 'function carriers(){var W="wpc-",ids=[W+"critical-css",W+"type-guard",W+"vars-guard",W+"late-faces",W+"crit-faces",W+"missing-faces75"],o={};'
            ...
            . 'function render(){var rep={ver:"' . esc_js($wpc_v86) . '",url:location.pathname+location.search,ua:navigator.userAgent.slice(0,60),carriers:carriers(),frames:S,events:EV};'
...
if (!function_exists('wpc_purge_soft43')) {
    function wpc_purge_soft43($explicit = null)
    {
        try {
            if ($explicit === 'hard') {
                return false;
    ...
    function wpc_purge_all_coalesce10()
    {
        if (!function_exists('get_option') || !function_exists('update_option')) {
            return false;
        }
    ...
    function wpc_stale43_hard_done()
    {
        if (defined('WPS_IC_CACHE')) {
...
if (!function_exists('wpc_delivery_config_sig146')) {
    function wpc_delivery_config_sig146()
    {
        $wpc_parts146 = [
            ...
            function_exists('get_option') ? get_option('wpc-excludes') : null,
            function_exists('get_option') ? get_option('wpc_css_passthrough') : null,
            defined('WPC_PLUGIN_VERSION') ? WPC_PLUGIN_VERSION : '',
        ];
    ...
    function wpc_dcv_flip146()
    {
        try {
            ...
            if (defined('WPS_IC_CACHE')) {
                ...
                wpc_fs_put(rtrim(WPS_IC_CACHE, '/') . '/dcv.txt', $wpc_sig146);
            }
...
if (!function_exists('wpc_pillar_rider_heal215')) {
    function wpc_pillar_rider_heal215()
    {
        ...
        if (!empty($set['critical']['css']) && $set['critical']['css'] == '1' && !$wpc_g321r
            && isset($set['used-css']) && (string) $set['used-css'] === '0'
            && get_option('wpc_r2_reentry_v') !== (defined('WPC_PLUGIN_VERSION') ? WPC_PLUGIN_VERSION : '?')) {
            $wpc_frozen216 = [];
